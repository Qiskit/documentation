...
4. Add the package's Box artifact URL to `scripts/config/api-html-artifacts.json` (see how to get the shareable link [here](#Final-steps-for-the-rc1-release))
3. Add the package name to `Pkg.ADDON_NAMES` in `scripts/js/lib/api/Pkg.ts` and add a `new Pkg(...)` with the correct `title`, `githubSlug`, and `language` in that file as well.
5. Run the pipeline for the new addon (`gen-api`/`gen-addon`)
6. Verify content with `npm run check:internal-links -- --current-apis` and `npm run check:spelling` 
