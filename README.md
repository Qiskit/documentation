...
6. Verify the content by running `npm run check:internal-links -- --current-apis`, `npm run check:spelling`, and `npm run check:markdown -- --apis`  Fix any issues directly in the generated content.  Note any problems with link generation etc. and let the frontent team know, in case they need to fix the pipeline.
