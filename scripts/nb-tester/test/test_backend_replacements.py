...
def in_notebook(cell_source: str, outputs: list[dict] | None = None) -> NotebookNode:
...
def test_simple_open_to_internal():
    nb = in_notebook("backend = service.backend(\"ibm_brisbane\")")
...
def test_simple_internal_to_open():
    nb = in_notebook("backend = service.backend(\"alt_brisbane\")")
