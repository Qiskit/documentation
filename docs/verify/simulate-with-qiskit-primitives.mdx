...
The reference primitives in Qiskit&reg; can perform local statevector simulations, which is useful for quickly prototyping algorithms. 
...
<Tabs>
  <TabItem value="EstimatorV2" label="EstimatorV2">
    Now that you have defined your `estimator`, you can run your estimation by calling the [`qiskit.primitives.StatevectorEstimator.run`](../api/qiskit/qiskit.primitives.StatevectorEstimator#run) method, which returns an instance of [`qiskit.providers.JobV1`](../api/qiskit/qiskit.providers.JobV1). 
...
You can get the results from the job (as a [`qiskit.primitives.PrimitiveResult`](../api/qiskit/qiskit.primitives.PrimitiveResult) object)
with the [`qiskit.primitives.PrimitiveJob.result`](../api/qiskit/qiskit.pprimitives.PrimitiveJob.result) method.
