...
job = estimator.run([(isa_circuit, observable, parameter_values)])
result = job.result()
...
job = estimator.run([isa_circuit]*3, [observable]*3, parameter_values)
result = job.result()
print(f" > Result class: {type(result)}")
```
```text
...
<Tabs>
  <TabItem value="V2" label="StatevectorEstimator (V2)">
    ...
    estimator.run([(isa_circuit1, obs_array1), (isa_circuit2, obs_array_2)], precision=0.05)
    ```
  ...
  <TabItem value="V1" label="Estimator (V1)">
    ...
    job = estimator.run(isa_circuit, observable, shots=2048, seed=123)
    result = job.result()
    print(result)
    ```
  </TabItem>
...
See more examples on the [Primitives examples](/run/primitives-examples#estimator-examples) page.
...
<Tabs>
  ...
  <TabItem value="V1" label="Sampler (V1)">
    ...
    job = sampler.run(isa_circuit, observable, shots=2048, seed=123)
    result = job.result()
    print(result)
    ```
  </TabItem>
...
See more examples on the [Primitives examples](/run/primitives-examples#sampler-examples) page.
## Next steps
