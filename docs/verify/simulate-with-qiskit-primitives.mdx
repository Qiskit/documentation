...
The `Estimator` primitive can compute an expectation value, and the `Sampler` primitive can compute circuit output probabilities. For import instructions, see the topic [Where should I import the primitive from?](../run/primitives#import-primitives)
