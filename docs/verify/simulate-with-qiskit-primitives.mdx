...
TODO: fix circuit image (doesn't correspond to code)
...
<Tabs>
  <TabItem value="EstimatorV2" label="StatevectorEstimator (V2)">
    ...
    with the [`qiskit.primitives.PrimitiveResult.result`](../api/qiskit/qiskit.primitives.PrimitiveResult#result) method.
...
<Tabs>
  <TabItem value="EstimatorV2" label="StatevectorEstimator (V2)">
      ...
      The primitives V2 result outputs an array of `PubResult`s, where each item of the array is a `PubResult` object
...
these examples run a single circuit, and the circuit is parametrized, but you can also run the Sampler
