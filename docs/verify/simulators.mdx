...
The following simulation methods support a maximum of 300 circuits and 8192 shots per job. Find more information on each simulator below, including its type, a description, the number of qubits it simulates, whether it includes noise modeling, a list of supported gates, and a minimum code example. The `simulator_statevector` is a good default choice since it is a general-purpose solution method. 
