...
Whether you will work locally or in a cloud environment, the first step for all users is to install Qiskit&reg;. For those wanting to run on a real system, your next step is to choose one of two channels in order to access IBM Quantum systems: IBM Quantum&reg; Platform or IBM Cloud&reg;.

<Admonition type="caution" title="Upgrade from Qiskit 0.x to Qiskit 1.0 and beyond">
  (If you do not have a Qiskit installation already, skip ahead to the [Install and set up](#local) section.)

  If you are upgrading from version 0.x to 1.0 or later, see the [Qiskit 1.0 migration guide](/api/migration-guides/qiskit-1.0) for details and instructions. Note that because Qiskit 1.0 uses a new packaging structure, you **cannot** use `pip install -U qiskit` to upgrade from any Qiskit 0.x version to 1.0. 
