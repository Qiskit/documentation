...
1. Install Python. Check the "Programming Language" section on the [Qiskit PyPI project page](https://pypi.org/project/qiskit/) to determine which Python versions are supported by the most recent release. For download instructions, see the [Python Beginners Guide.](https://wiki.python.org/moin/BeginnersGuide/Download)
    ...
    Choose a preferred location in which to store information about your virtual environments. Commonly they're stored in a directory named `.venv` in a user's local directory. To create a virtual environment named `qiskit-stable` in this location, run this command:
    ```shell
      python3 -m venv ~/.venv/qiskit-stable
    ```
    To activate this environment within a terminal window, run this command:
    ```shell
      source ~/.venv/qiskit-stable/bin/activate
    ```
    </details>
