...
<span id="qiskit-1-upgrade-venv"></span>
## For current Qiskit users: Upgrade to Qiskit 1.0

<Admonition type="info" title="Current users must create a new virtual environment to install Qiskit 1.0">
  Because Qiskit 1.0 uses a new packaging structure, you cannot use `pip install -U qiskit` to upgrade from any Qiskit 0.x version to 1.0. Instead, create a new virtual environment following the following steps. 
  
  Future updates starting with Qiskit 1.0 will allow for in-place upgrades.  
</Admonition>

(If you do not have a Qiskit installation already, skip ahead to the [Install and set up](#local) section.) To upgrade to Qiskit 1.0, create a new virtual environment. Here is an example, using the standard tool *venv*:

```shell
# Create a new virtual environment (you only do this once)
python -m venv ~/qiskit-1.0-env

# Activate the new virtual environment (you do this every time you start a new session)
source ~/qiskit-1.0-env/bin/activate

# Install Qiskit 1.0 (you only do this once for this environment)
pip install ‘qiskit>=1.0’
```

Note that you may need to adapt your code to work with Qiskit 1.0, as there will be some breaking changes with respect to 0.x; see the [Qiskit 1.0 migration guide](/api/migration-guides/qiskit-1.0) for details.
