...
## Set up account using REST APIs
...
2. Obtain a temporary Access token by supplying the API token obtained above. This is especially useful if you would like control over tokens, such as token invalidation. Alternatively, you can work directly with your IBM Quantum Platform API token.
...
You can then [transpile circuits using REST APIs](rest-api-transpilation) and run them using either the [Sampler](rest-api-sampler) Primitive or the [Estimator](rest-api-estimator) Primitive.   
