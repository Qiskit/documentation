...
## Access IBM Quantum Systems using REST API

Alternatively, you can also access IBM Quantum Systems using REST APIs, enabling you to work with quantum systems from any programming language or framework. 

1. Retrieve your IBM Quantum token from the [IBM Quantum account page](https://quantum.ibm.com/account) and save it to a file named `token`.

2. Optionally obtain a temporary access token by supplying the API token obtained above. This is especially useful if you would like control over tokens, such as token invalidation. Alternatively, you can work directly with your IBM Quantum Platform API token.
...
3. Let's view your available backends
...
You can then [transpile circuits using REST API](/transpile/rest-api-transpilation) and run them using either the [Sampler or the Estimator Primitives](/run/rest-api-primitives).   

4. After your experiments are complete, you can proceed to invalidate your token followed by testing it's invalidation.
