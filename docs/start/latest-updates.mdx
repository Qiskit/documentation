...
The following endpoints are deprecated and will be removed on or after 1 October, 2024: `GET /stream/jobs` and `GET /stream/jobs/{id}` This removal has the following impacts:

- After the endpoints are removed, `job.stream_results()` and `job.interim_results()` will be removed.
