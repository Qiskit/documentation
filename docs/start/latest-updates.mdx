...
**New features for the v0.24.0 release**
...
Qiskit Runtime primitives now support QASM 3 as the input format for circuits and standard JSON output format. See [Qiskit IBM Runtime REST API](https://docs.quantum.ibm.com/api/runtime) for examples.

### 3 June 2024

Probabilistic error amplification (PEA) error mitigation method is now available as an experimental option for Estimator V2. See the [`EstimatorOptions` API reference](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/qiskit_ibm_runtime.options.EstimatorOptions) for more details.
