...
To replace cloud simulators, you can now use local testing mode instead. Depending on your needs, there are several ways to use local testing mode. 

### Fake backends
 
To use local testing mode, specify one of the fake backends in ``qiskit_ibm_runtime.fake_provider`` or specify a Qiskit Aer backend when instantiating a primitive or a session.  For details on fake backends, see the [fake_provider API reference.](/api/qiskit-ibm-runtime/qiskit_ibm_runtime.fake_provider)
...
You can use local testing mode to use simulators from [Qiskit Aer](../verify/simulate-with-qiskit-aer), which allow you to model noise.
...
To simulate with noise, specify a system (quantum hardware) and submit it to Aer.  Aer builds a noise model based on the calibration data from that system, and instantiates an Aer backend with that model.  If you prefer, you can [build a noise model](../verify/build-noise-models).
