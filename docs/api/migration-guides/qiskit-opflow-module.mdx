...
preparing a circuit to execute with a `backend.run()` type of method, the algorithms can leverage the [`qiskit.primitives.Sampler`](../qiskit/qiskit.primitives.Sampler) and
...
Similarly to [`qiskit.opflow.primitive_ops.PrimitiveOp`](../qiskit/qiskit.opflow.primitive_ops.PrimitiveOp), [`qiskit.opflow.state_fns.StateFn`](../qiskit/qiskit.opflow.state_fns.StateFn)
...
In the case of the [`qiskit.opflow.converters.CircuitSampler`](../qiskit/qiskit.opflow.converters.CircuitSampler), it traversed an operator and outputted
...
<Admonition type="note">
  ...
  constructing [`qiskit.circuit.library.PauliEvolutionGate`](../qiskit/qiskit.circuit.library.PauliEvolutionGate)s out of the operator primitives, and supplying one of
  ...
  The different Trotterization methods that extend [`qiskit.opflow.evolutions.TrotterizationBase`](../qiskit/qiskit.opflow.evolutions.TrotterizationBase) were migrated to
  ...
  standalone use, but are now designed to be plugged into the [`qiskit.circuit.library.PauliEvolutionGate`](../qiskit/qiskit.circuit.library.PauliEvolutionGate) and called via `.synthesize()`.
  In this context, the job of the [`qiskit.opflow.evolutions.PauliTrotterEvolution`](../qiskit/qiskit.opflow.evolutions.PauliTrotterEvolution) class can now be handled directly by the algorithms
  (for example, [`qiskit.algorithms.time_evolvers.trotterization.TrotterQRTE`](../qiskit/qiskit.algorithms.time_evolvers.trotterization.TrotterQRTE)).

  In a similar manner, the [`qiskit.opflow.evolutions.MatrixEvolution`](../qiskit/qiskit.opflow.evolutions.MatrixEvolution) class performs evolution by classical matrix exponentiation,
  constructing a circuit with [`qiskit.extensions.UnitaryGate`](../qiskit/qiskit.extensions.UnitaryGate)s or [`qiskit.extensions.HamiltonianGate`](../qiskit/qiskit.extensions.HamiltonianGate)s containing the exponentiation of the operator.
  This class is no longer necessary, as the [`qiskit.extensions.HamiltonianGate`](../qiskit/qiskit.extensions.HamiltonianGate)s can be directly handled by the algorithms.
</Admonition>
