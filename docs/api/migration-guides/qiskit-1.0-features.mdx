...
right-hand side to jump to the module you are interested in.
...
`qiskit.providers.basicaer` module has
...
[`quantum_info`](/api/qiskit/quantum_info) classes. This is not a drop-in replacement,
...
pulse libray have been superseded by
...
performance, flexibility and QPY support:
...
and attach it to `QuantumCircuit` through the
...
For more information, see  the [qiskit.synthesis](#qiskit.synthesis).
...
The `qiskit.tools` module was removed in Qiskit 1.0.0. Most of this
functionality was either replaced by similar functionality in other packages or
removed with no alternative. The primary exception was the
`qiskit.tools.parallel_map()` function has been relocated to the
[`qiskit.utils`](/api/qiskit/utils) module. It can be used from this new
...
use `qiskit.visualization.circuit.qcstyle`
