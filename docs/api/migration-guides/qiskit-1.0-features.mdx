...
and the corresponding pulse library. `SymbolicPulse` provides [QPY](/api/qiskit/qpy) support:
...
Complex-valued pulse amplitude (``amp``) is replaced with a (``amp``, ``angle``) duo.
This representation is more intuitive especially for some calibration tasks, e.g. angle calibration:
