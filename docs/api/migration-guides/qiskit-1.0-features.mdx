...
The unitary and statevector simulators can be replaced with different
...
pulse library have been superseded by `qiskit.pulse.SymbolicPulse` and the
corresponding pulse library. `SymbolicPulse` provides better performance,
flexibility, and [QPY](../qiskit/qpy) support:
...
We recommend writing a minimum pulse program with the builder
...
The `qiskit.visualization.qcstyle` module has been removed. Use [`qiskit.visualization.circuit.qcstyle`](/api/qiskit/qiskit.visualization.circuit.qcstyle.DefaultStyle)
