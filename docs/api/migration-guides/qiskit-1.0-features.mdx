...
* The `QuantumCircuit.qasm` method is deprecated. Instead, use
...
* The ``qiskit.converters.ast_to_dag`` function has been deprecated and
  ...
  that directly to a `.DAGCircuit`. As the legacy OpenQASM 2 parser
  ...
  converting that `.QuantumCircuit` into a `.DAGCircuit`
...
* The `~qiskit.pulse.library.parametric_pulses.ParametricPulse` base class and pulses are now
  deprecated, and will be removed in Qiskit 1.0. This includes:
  * `~qiskit.pulse.library.parametric_pulses.ParametricPulse`
  * `~qiskit.pulse.library.parametric_pulses.Constant`
  * `~qiskit.pulse.library.parametric_pulses.Drag`
  * `~qiskit.pulse.library.parametric_pulses.Gaussian`
  * `~qiskit.pulse.library.parametric_pulses.GaussianSquare`
  The class has been superseded by `~qiskit.pulse.SymbolicPulse` and the corresponding pulse library.
...
* Injecting circuit gate operations into the pulse builder context is now deprecated. The deprecation
  ...
  * :func:`~qiskit.pulse.builder.x`
  As well as input arguments of type `.QuantumCircuit` type in :func:`~qiskit.pulse.builder.call`.
  ...
  .. code-block:: python
    ...
    with pulse.build() as only_pulse_scheds:
      pulse.call(sched)
  Similarly, `.QuantumCircuit` can be injected in the builder context
...
*  The `.NoiseAdaptiveLayout` transpiler pass and the corresponding
...
*  The `.CrosstalkAdaptiveSchedule` transpiler pass has been deprecated
