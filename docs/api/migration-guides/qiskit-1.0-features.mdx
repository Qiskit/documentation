...
This guide shows migration paths for the most important feature changes in Qiskit&reg;  1.0, organized by module.
Use the navigation bar on the right-hand side to navigate to the module you are interested in.
...
[`run`](/api/qiskit/qiskit.providers.BackendV2#run) functionalities in Qiskit.
...
The following two `QuantumCircuit` methods are also deprecated.
...
1. `Sampler and Estimator`: The new V2 primitives are designed to accept **vectorized inputs**, where single circuits can be grouped with array-valued specifications. That is, one circuit can be executed
  for arrays of `n` parameter sets and/or `n` observables (in the case of the estimator). Each group is called a **primitive unified bloc (pub)**, and can be represented as a tuple: `(1 x circuit, [n x observables], [n x parameters])`. The V1 interface didn't allow for the same flexibility, as there number of input circuits had to match the number of observables and
...
* Implicit conversion from a dense `BaseOperator` to a
  ...
  `Estimator` observable arguments is no longer allowed. You should explicitly
  convert to a `SparsePauliOp` using `SparsePauliOp.from_operator(operator)`
  instead.
...
* Using a [`PauliList`](/api/qiskit/qiskit.quantum_info.PauliList) in
  ...
  explicitly convert the argument using `SparsePauliOp(pauli_list)` first.
...
classes; their functionality was already contained in the
[`quantum_info`](/api/qiskit/quantum_info) module.

The migration to the new paths should be straightforward. You can replace most
classes in `qiskit.providers.basicaer`
with their
[`qiskit.providers.basic_provider`](/api/qiskit/providers_basic_provider)
counterpart (drop-in replacement). Note that the following classes have not
only changed paths, but also changed names:
...
As for the unitary and statevector simulators, they can be replaced with
different [`quantum_info`](/api/qiskit/quantum_info) classes. This is
not a drop-in replacement, but the changes are minimal, see the migration
examples below:
...
<Tabs>
  <TabItem value="statevec" label="Statevector simulator" default>
    ```python
    from qiskit import QuantumCircuit
...
migration path consists of using the new
...
`pulse.library.parametric_pulses.Constant` |  [`pulse.library.symbolic_pulses.Constant`](/api/qiskit/qiskit.pulse.library.Constant_class.rst)
`pulse.library.parametric_pulses.Drag` |  [`pulse.library.symbolic_pulses.Drag`](/api/qiskit/qiskit.pulse.library.Drag_class.rst)
`pulse.library.parametric_pulses.Gaussian` |  [`pulse.library.symbolic_pulses.Gaussian`](/api/qiskit/qiskit.pulse.library.Gaussian_class.rst)
...
Complex-valued pulse amplitude (``amp``) is replaced with a (``amp``,
...
We recommend writing a minimum pulse program with the builder and attach it to
...
[`Square`](/api/qiskit/qiskit.pulse.library.Square_fun.rst) is defined such
...
It is no longer possible to load library `qiskit.pulse.ScalableSymbolicPulse` objects with complex `amp`
parameter from qpy files of version 5 or lower (Qiskit Terra < 0.23.0). No migration action is required, as complex `amp` will be
...
For more information, see [qiskit.synthesis](#qiskit.synthesis).
...
nor to be used outside of Qiskit's own test suite.  All functionality was specific to Qiskit and no
...
it exposed have been deprecated and will be removed in the Qiskit 1.0
release. This module's functionality was not widely used and is better covered
...
`qiskit.synthesis.unitary.aqc.AQCSynthesisPlugin` | [`qiskit.transpiler.passes.AQCSynthesisPlugin`](/api/qiskit/qiskit.transpiler.passes.AQCSynthesisPlugin)
...
codebase. This pass was no longer usable because its internal operation
...
[`FlowControllerLinear`](/api/qiskit/qiskit.passmanager.FlowControllerLinear)
[`DoWhileController`](/api/qiskit/qiskit.passmanager.DoWhileController) classes
are deprecated. Instead, all tasks must be provided when the controller objects
are constructed.
...
`qiskit.opflow` modules, and have no replacement.
