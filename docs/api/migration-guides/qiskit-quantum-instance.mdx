...
<Admonition type="caution">
  **Background on the Qiskit primitives**
  ...
  The Qiskit primitives are algorithmic abstractions that encapsulate QPU or simulator access for easy integration into algorithm workflows.
  ...
  - *`Backend` primitives* - Instances of [`qiskit.primitives.BackendSampler`](../qiskit/qiskit.primitives.BackendSampler) and [`qiskit.primitives.BackendEstimator`](../qiskit/qiskit.primitives.BackendEstimator). These allow any QPU to implement primitive interfaces.
...
2. How do you want to run your circuits?
   ...
   [`qiskit.utils.QuantumInstance`](../qiskit/0.46/qiskit.utils.QuantumInstance) with either a QPU from a provider, or a simulator.
   For this migration, this "QPU selection" process is translated to **where** do you import your primitives from:
   ...
   * Accessing **runtime-enabled QPUs** (or cloud simulators): **Qiskit Runtime primitives**
   * Accessing **non runtime-enabled QPUs** : **`Backend` primitives**
...
the result data was QPU-dependent (it could be a counts `dict`, a `numpy.array` for
