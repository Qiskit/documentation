...
Currently, the only option Sampler accepts is `dynamical_decoupling`.
...
|            | resilience_level         | How much resilience to build against errors.                                                                                                                                                                                            | 1              |
|            | dynamical_decoupling     | The dynamical decoupling sequence to use.                                                                                                                                                                                               | None           |
|            | seed_estimator           | Seed used to control sampling.                                                                                                                                                                                                          | None           |
| resilience | measure_noise_mitigation | Whether to enable measurement error mitigation method.                                                                                                                                                                                  | True           |
| resilience | zne_mitigation           | Whether to turn on Zero Noise Extrapolation error mitigation method.                                                                                                                                                                    | False          |
| resilience | zne_noise_factors        | An list of real valued noise factors that determine by what amount the circuits' noise is amplified.                                                                                                                                    |                |
| resilience | zne_extrapolator         | An extrapolation strategy. One or more of ``"multi_exponential"``,  ``"single_exponential"``, ``"double_exponential"``, ``"linear"``.                                                                                                   |                |
| resilience | zne_stderr_threshold     | A standard error threshold for accepting the ZNE result of Pauli basis expectation values when using ZNE mitigation.                                                                                                                    |                |
| resilience | pec_mitigation           | Whether to turn on Probabilistic Error Cancellation error mitigation method.                                                                                                                                                            | False          |
| resilience | pec_max_overhead         | Specify a maximum sampling overhead for the PEC sampling noise model. If None the full learned model will be sampled from.                                                                                                              |                |
...
| execution  | samples                  | The number of samples of each measurement circuit to run. This is used when twirling or resilience levels 1, 2 is enabled. If None it will  be calculated automatically based on the ``shots`` and ``shots_per_sample`` (if specified). |                |
| execution  | shots_per_sample         | The number of shots per sample of each measurement  circuit to run. This is used when twirling or resilience levels 1, 2, 3.  If None it will be calculated automatically based on the ``shots`` and ``samples`` (if specified).        |                |
| execution  | interleave_samples       | If True interleave samples from different measurement circuits when running.                                                                                                                                                            | False          |
| twirling   | gates                    | Whether to apply 2-qubit gate twirling.                                                                                                                                                                                                 | False          |
| twirling   | measure                  | Whether to apply measurement twirling.                                                                                                                                                                                                  | True           |
| twirling   | strategy                 | Specify the strategy of twirling qubits in identified layers of  2-qubit twirled gates.                                                                                                                                                 | “active-accum" |
...
V2 primitives support only ISA circuits. Because the primitives will no longer perform layout, routing, and translation operations, the corresponding transpilation options from V1 are not supported.  You can, however, still use `optimization_level` to indicate how much optimization the primitives should apply to the ISA circuits. The valid values are 0 and 1. 
