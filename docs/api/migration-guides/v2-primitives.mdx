...
* SamplerV2 PUB format: (`<circuit>`, `<parameter values>`, `<shots>`), where `<parameter values>`and `<shots>` are optional.
* EstimatorV2 PUB format: (`<circuit>`, `<observables>`, `<parameter values>`, `<precision>`), where `<parameter values>`and `<precision>` are optional.
...
* EstimatorV2 has gained a `precision` argument in the `run()` method that specifies the targeted precision of the expectation value estimates. 
* SamplerV2 has the `shots` argument in its `run()` method. 
* EstimatorV2 does not support resilience level 3.  This is because resilience level 3 in V1 Estimator uses Probabilistic Error Cancellation (PEC), which is proven to give unbiased results at the cost of exponential processing time. You can, however, still use PEC as the error mitigation method by specifying the `pec_mitigation` option.
* Because SamplerV2 returns samples without postprocessing, it does not support resilience levels. 

##### Examples

{/*From the boxnote.*/}

EstimatorV2 example that uses precision in `run()`:
...
SamplerV2 example that uses shots in `run()`:
...
* SamplerV2 returns per-shot measurements in the form of **bitstrings** or
...
* SamplerV2 has convenience methods like `get_counts()` to help with migration.    
...
sampler.options.optimization_level = 0
# Turn on dynamical decoupling with sequence XpXm.
sampler.options.dynamical_decoupling = "XpXm"

print(f">> optimization level to use: {sampler.options.optimization_level}")
print(f">> dynamical decoupling sequence to use: {sampler.options.dynamical_decoupling}")
```    
  </TabItem>
...
For examples of using options, see the [Error mitigation](#error-mit) and [Transpilation](#transpilation) sections. 

**Estimator Advanced options:**


| Category                          | qiskit-ibm-runtime       | Type                                              | Default                   |
|-----------------------------------|--------------------------|---------------------------------------------------|---------------------------|
...
| dynamical_decoupling              | enable                   | bool                                              | False                     |
| dynamical_decoupling              | sequence_type            | Literal[`XX`, `XpXm`, `XY4`]                      | `XX`                      |
| dynamical_decoupling              | extra_slack_distribution | Literal[`middle`, `edges`]                        | middle                    |
| dynamical_decoupling              | scheduling_method        | Literal[`alap`, `asap`]                           | alap                      |
| execution                         | rep_delay                | float                                             | backend.default_rep_delay |
| execution                         | init_qubits              | bool                                              | true                      |
| resilience                        | measure_mitigation       | bool                                              | true                      |
| resilience                        | zne_mitigation           | bool                                              | false                     |
| resilience                        | pec_mitigation           | bool                                              | false                     |
| resilience.zne                    | noise_factors            | list[float]                                       | (1, 3, 5)                 |
| resilience.zne                    | extrapolator             | `exponential\|linear\|polynomial_degree_[1-7]`    | (`exponential`, `linear`) |
| resilience.pec                    | max_overhead             | float                                             | None                      |
| resilience.pec                    | noise_gain               | float \| `auto`                                   | `auto`                    |
| resilience.layer_noise_learning   | max_layers_to_learn      | int                                               | 20                        |
| resilience.layer_noise_learning   | shots_per_randomization  | int                                               | 128                       |
| resilience.layer_noise_learning   | num_randomizations       | int                                               | 32                        |
| resilience.layer_noise_learning   | layer_pair_depths        | list[int]                                         | (0, 1, 2, 4, 16, 32)      |
| resilience.measure_noise_learning | num_randomizations       | int                                               | 32                        |
| resilience.measure_noise_learning | shots_per_randomization  | int \| `auto`                                     | `auto`                    |
| twirling                          | enable_gates             | bool                                              | False                     |
| twirling                          | enable_measure           | bool                                              | True                      |
| twirling                          | num_randomizations       | int \| `auto`                                     | `auto`                    |
| twirling                          | shots_per_randomization  | int \| `auto`                                     | `auto`                    |
| twirling                          | strategy                 | `active`, `active-accum`, `active-circuit`, `all` | `active-accum`            |
| simulator                         | coupling_map             |                                                   | None                      |
| simulator                         | basis gates              |                                                   | None                      |
| simulator                         | noise_model              |                                                   | None                      |
| simulator                         | seed_simulator           |                                                   | None                      |

**Sampler advanced options**
...
| dynamical_decoupling | extra_slack_distribution | Literal[`middle`, `edges`]    | middle                                    |
| dynamical_decoupling | scheduling_method        | Literal[`alap`, `asap`]       | alap                                      |
| execution            | rep_delay                | float                         | backend.configuration().default_rep_delay |
| execution            | init_qubits              | bool                          | true                                      |
...
print(f">>> gate twirling is turned on: {sampler.options.twirling.gates}")
```
  </TabItem>
...
The V2 primitives have a new `RuntimeJobV2` class, which inherits from `BasePrimitiveJob`. The `status()` method of this new class returns a string instead of JobStatus enum from Qiskit. See the [API reference](../qiskit-ibm-runtime/qiskit_ibm_runtime.RuntimeJobV2) for details. [TEST](../qiskit-ibm-runtime/qiskit_ibm_runtime.RuntimeJob)
...
2. Remove any `from qiskit_ibm_runtime import Options` statements, since the `Options` class is not used by V2 primitives. You can instead pass options as a dictionary when initializing the `Estimator` class (for example `estimator = Estimator(backend, options={“twirling”: {“gates”: True}})`), or set them after initialization:
   ```python
   estimator = Estimator(backend)
   estimator.options.twirling.gates = True
   ```

3. Review all the [supported options](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options) and make updates accordingly.
...
6. EstimatorV2 job results are grouped by PUBs. You can see the expectation value and standard error for each PUB by indexing to it. For example:
  ```python
...
pub_result_3 = job.result()[2]
print(f">>> Expectation values 1: {pub_result.data.evs}")
print(f">>> Standard errors 1: {pub_result.data.stds}")
print(f">>> Expectation values 2: {pub_result_2.data.evs}")
print(f">>> Standard errors 2: {pub_result_2.data.stds}")
print(f">>> Expectation values 3: {pub_result_3.data.evs}")
print(f">>> Standard errors 3: {pub_result_3.data.stds}")
```  
  </TabItem>
...
2. Remove any `from qiskit_ibm_runtime import Options` statements, since the `Options` class is not used by V2 primitives. You can instead pass options as a dictionary when initializing the `Sampler` class (for example `sampler = Sampler(backend, options={“optimization_level”: 1})`), or set them after initialization:
   ```python
   sampler = Sampler(backend)
...
6. SamplerV2 job results are grouped by PUBs. You can see the output data for each PUB by indexing to it. While SamplerV2 returns unweighted samples, the result class has a convenience method to get counts instead. For example:
  ```python
  pub_result = job.result()[0]
  print(f">>> Counts: {pub_result.data.meas.get_counts()}")
  print(f">>> Per-shot measurement: {pub_result.data.meas.get_bitstrings()}")
  ```
