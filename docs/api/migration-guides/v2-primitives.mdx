...
- For V2 primitives, the `options` attribute is the `dataclass` Python type.  You , and you can use the built-in `asdict` method to convert it to a dictionary. 
...
estimator = Estimator(backend)
job = estimator.run([(circuits[0], observables[0]),(circuits[1], observables[1]),(circuits[2], observables[2])])
