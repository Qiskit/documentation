...
* The ability to turn on or off individual error mitigation / suppression methods.
* SamplerV2 is simplified to focus on its core task of sampling the output register from execution of quantum circuits. This has resulted the the following specific changes: 
  - It returns the samples, whose type is defined by the program, without weights and therefore no longer supports resilience levels. 
  - The output data is separated by the output register names and has convenience methods like `get_counts()` to help with migration. This also enables future support for circuits with classical control flow.
  ...
  - Has a new `shots` argument in the `run()` method that enables you to specify different shots for each input block. 
* EstimatorV2 
  - Has a new `precision` argument in the `run()` method that specifies the targeted precision of the expectation value estimates. This value replaced `shots` from V1 to give a more accurate description of its function. 
  - Does not support resilience level 3.  Resilience level 3 of V1 Estimator uses Probabilistic Error Cancellation (PEC), which is proven to give unbiased results at the cost of exponential processing time. To make this trade-off more obvious, EstimatorV2 does not support resilience level 3. You can, however, still use PEC as the error mitigation method by using the `pec_mitigation` option.
