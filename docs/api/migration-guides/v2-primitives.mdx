...
* SamplerV2 is simplified to focus on its core task of sampling the output register from execution of quantum circuits. This has resulted the the following specific changes: 
  ...
  - The output data is separated by the output register names and has connivence methods like `get_counts()` to help with migration. This also enables future support for circuits with classical control flow.
...
* EstimatorV2 
  - Has a new `precision` argument in the `run()` method that specifies the targeted precision of the expectation value estimates. This value replaced `shots` from V1 to give a more accuate description of its function. 
