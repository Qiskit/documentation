...
For backward compatibility, you must explicity import the V2 primitives. Specifying `import` *`prmitiveV2`* `as` *`primitive`* is not required, but makes it easier to transition code to V2. 
...
CHECKING WHAT THIS WOULD LOOK LIKE:

<Tabs>
  <TabItem value="SamplerV2" label="SamplerV2 simple">
    ```python 
circuit = IQP(mat)
observable = SparsePauliOp("Z" * n_qubits)
...
```
  ...
  <TabItem value="SamplerV1" label="Sampler simple">
    ```python 
circuit = IQP(mat)
observable = SparsePauliOp("Z" * n_qubits)
...
```
  ...
  <TabItem value="SamplerV2m" label="SamplerV2 complex">
    ```python 
theta = Parameter("θ")
...
```
  ...
  <TabItem value="SamplerV1m" label="Sampler complex">
    ```python 
theta = Parameter("θ")
