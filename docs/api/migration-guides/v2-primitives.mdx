...
* V2 Sampler PUB format: (`<circuit>`, `<parameter values>`, `<shots>`), where `<parameter values>`and `<shots>` are optional.
* V2 Estimator PUB format: (`<circuit>`, `<observables>`, `<parameter values>`, `<precision>`), where `<parameter values>`and `<precision>` are optional.
...
* V2 Sampler returns per-shot measurements in the form of **bitstrings** or
...
* V2 Sampler does not support `resilience_level`. However, you can apply gate / measurement twirling to your circuits using the `dynamical_decoupling` and `twirling` methods.
* V2 Estimator supports `resilience_level` 0-2, as described in the following table. You can also explicitly turn on / off individual error mitigation / suppression methods, including gate and measurement twirling, measurement error mitigation, and ZNE.
