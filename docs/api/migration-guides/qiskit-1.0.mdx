...
Qiskit 1.0 uses a different packaging structure than previous Qiskit versions and may cause problems in environments thta use packages that are not ready for Qiskit 1.0.
...
3. Install packages as desired.
   ...
   If you encounter issues, consult the [troubleshooting](#troubleshooting) section, ask on [Qiskit Slack](https://qisk.it/join-slack), or if you think there is a Qiskit bug you can [create an issue against Qiskit](https://github.com/Qiskit/qiskit/issues/new/choose).
...
If neither your package, nor any of its transitive dependencies, have a requirement pin on `qiskit<1`, you should create a testing virtual environment as you normally would, in a single `pip install` command, and directly specify `qiskit==1.0.0rc1` or `qiskit==git+https://github.com/Qiskit/qiskit.git@main` as appropriate.
...
<Admonition type="caution">
This process deliberately creates an invalid environment.  Therefore, any test using it is less valid. Tests might appear to pass, but this does not guarantee that the package is compatible with Qiskit 1.0.  This could happen because the environment is not self-consistent and there could be files in it that do not exist in a valid environment, or the behavior of an overridden package might change with Qiskit 1.0.
</Admonition>
...
2. Uninstall both `qiskit` and `qiskit-terra` to make sure that neither is present:
  ...
  At this point, the environment's `site-packages` should not contain a `qiskit` directory. You don't need to verify this on every CI run, but if you are debugging a script locally, follwo these steps to verify: 
...
For a package that has unavoidable transitive dependencies that pin `qiskit<1`, we have to build an invalid environment:
...
We know that this is inconvenient, but this allows us to restore Qiskit to the simple package structure that most Python packages use, which will be easier for users, developers, and library authors after the Qiskit 1.0 transition is complete.
...
* *package*: A directory containing an `__init__.py` and other files or packages that Python can read.
  ...
  Python considers any directory that is on the search path to be something you can import (and will import quite a few additional items).
...
* *submodule*, *subpackage*: These are somewhat imprecise terms, but are commonly used.
  The _sub_ part just means "contained inside a package".
...
* *distribution*: The compressed Python files, data files, and metadata that are downloaded when you run `pip install something`.
  ...
  Most sources talking about Python packaging use the term _package_ to mean both distributions and packages, and you have to understand from context what is meant.
...
Python is highly dynamic, and it's actually possible for a module or package to not correspond to files on disk, or to be compiled extensions, or many other things.
The search path is not only a search over directories, but files on disk are all that matters for this discussion.
...
In our experience, while it's annoying to get the error message, it's much better to be aware of the problem right away than for things to silently look like they're working, only to fail in subtle ways further down the line.
...
Close out of Jupyter, activate the Qiskit virtual environment on the command line, then run `pip install jupyterlab` (or whichever notebook interface you use), then reopen Jupyter.
...
Please be mindful that this takes time; we're happy you're excited about Qiskit 1.0, but please give the developers a month or so to prepare new versions of their distributions!
...
Sometimes there will be true conflicts between dependencies and you might need multiple virtual environments to separate out dependencies that have incompatible requirements.
...
<Admonition type="note">
Don't worry about having multiple virtual environments.
...
When you're done with a virtual environment, you can simply delete its directory; you don't need to have several environments that stay around forever.
</Admonition>
...
<Admonition type="tip">
This example is a quite flat dependency structure.
You might see a much deeper tree than this.
The packages that are directly dependent on `qiskit-terra` (lowest indentation) are most likely to be the problematic ones, but one further down the tree could be problematic if it depends on a specific old version of some other package that has already been updated.
</Admonition>
...
If you constructed this environment out of several `pip install` commands (such as if the environment is old and has been updated), when you build a new environment, first try to install all of your packages by using a single `pip install` command.
...
Assuming you have at least one package that does not yet support Qiskit 1.0, you will have to exclude that package when constructing a Qiskit 1.0 environment, or  continue to use a prior version of Qiskit.
See [Creating the new environment](#creating-a-venv) for instructions.
...
It's not possible to backdate requirements changes to previously released versions, and `pip` will search arbitrarily far back trying to find something that works when building an environment.
</Admonition>
...
If you have a broken environment, but you were only trying to install Qiskit 0.45 or 0.46, the most likely situation is that `pip` installed Qiskit 1.0 because it tried to pick the latest versions of packages, even though it was not required.
...
You can also use the `pipdeptree` commands listed in [Getting a working environment for Qiskit 1.0](#debug-venv-for-1.0) from within the broken environment to determine which distributions have an explicit requirement on `qiskit>=1`.
