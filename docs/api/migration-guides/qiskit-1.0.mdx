...
Do not try to upgrade an existing Python&reg; virtual environment to Qiskit 1.0 in-place.
</Admonition>
...
For Linux&reg; and macOS&reg; commands, a bash-like syntax is used.
PowerShell&reg; is used for Windows commands.
</Admonition>
...
3. Install packages as desired.
   ...
   We have provided such a file in a GitHub&reg; Gist at [https://qisk.it/1-0-constraints](https://qisk.it/1-0-constraints), which you can use like this:
...
* *distribution*: The compressed Python files, data files, and metadata that are downloaded when you run `pip install something`.
  ...
  If you are familiar with "package managers" outside the context of Python, such as `apt` from Debian&reg;/Ubuntu&reg; or Homebrew on macOS, then what they call a "package", Python calls a distribution, and there is no exact match for what Python calls a package.
...
The `qiskit-terra` distribution on PyPI&reg; will never be updated to version 1.0 or beyond; it is entirely superseded by `qiskit`.
...
If you are using Jupyter&reg; and see this, ensure that Jupyter is installed into the same virtual environment as Qiskit.
