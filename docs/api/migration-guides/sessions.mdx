...
| Batch mode   |                        | Quantum computation only.               | The entire batch of jobs is scheduled together and there is no additional queueing time for each. Jobs in a batch are run close together.                                                                             |
...
Submit multiple primitive jobs simultaneously. When batching, classical processing is done in parallel. No session jobs can start when batch jobs are being processed, however, individual jobs might run between batch jobs.
...
* Session usage is the entire active session time.
