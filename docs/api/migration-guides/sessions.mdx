...
Sessions are designed for iterative workloads to avoid queueing delays between each iteration. When running a session, you have near exclusive access to the backend.  With this release, sessions are now thread safe. That is, you can run multiple workloads within a session. By default, all sessions run in this dedicated mode. 
...
```python
# Example: dividing a 500-circuit job into 5 100-circuit jobs and run them in batch
