...
You need the classical register name to get the results.  By default, it is named `meas`.  However, as shown in the previous image, the classical register name for this example is `c`.  You can also find the classical register name by running `<circuit_name>.cregs`. For example, `qc.cregs`.
</Admonition>
