...
Qiskit 1.0 uses a different packaging structure to previous Qiskit versions, and may cause problems in environments using packages that are not ready for Qiskit 1.0.
...
You do not need to read this entire page to start using Qiskit 1.0.

If you are a user, read the [For users](#for-users) section for the getting started guide.
If you develop or maintain a package that depends on Qiskit, read the [For developers](#for-developers) section for recommendations on managing the transition.
There is [a lot of detail on what exactly is changing and why it had to change](#why-did-this-happen) that you need only read if you are interested.
There is a large [Troubleshooting section](#troubleshooting) which you can consult if you encounter problems installing or importing Qiskit 1.0.
...
For Linux and macOS commands, we will use a bash-like syntax for the command-line syntax.
For Windows we will use PowerShell.
</Admonition>

<span id="creating-a-venv"></span>
### Creating the new environment

1. Create a new virtual environment, using your preferred version of Python 3.8 or higher:
   ...
   You can use any path you like in place of `/path/to/qiskit-1.0-venv`.
...
3. Install all the packages you want to install.
   ...
   You can use a separate virtual environment with Qiskit 0.45 or 0.46 for those packages.
   If you have an existing environment, you could use [`pipdeptree`](https://github.com/tox-dev/pipdeptree/blob/main/README.md#pipdeptree) to query the requirements of what you have installed, to see if they require `qiskit<1` specifically (though be aware, there might be updates available).

   If you encounter issues, consult [the troubleshooting section](#troubleshooting) below, ask on [the Qiskit Slack](https://qisk.it/join-slack), or if you think there is a Qiskit bug you can [file an issue against Qiskit](https://github.com/Qiskit/qiskit/issues/new/choose).

   The Qiskit team are working with the greater ecosystem to help other developers prepare their packages, and we hope that any problems will lessen over time.
   </Admonition>
...
You should test your package against this, and as soon as possible after the release candidate, release a new version of your package that is compatible with Qiskit 1.0 with its upper requirement unpinned.
...
Things might appear to work, but this does not *guarantee* that you are compatible with Qiskit 1.0, because the environment is not self-consistent, and there could be files around that would not exist with a proper environment, or the behavior of an overridden package might change with Qiskit 1.0.
</Admonition>
...
The upgrade is not likely to work if any have been installed, and you should work to remove any remnant dependencies on old packages like this.
...
It doesn't matter if `qiskit` was not installed; `pip` will show a warning, but all we cared about was making certain neither of the two were present.

At this point, the environment's `site-packages` should not contain a `qiskit` directory.
...
Second, for a package that has unavoidable transitive dependencies that are pinning `qiskit<1`, we have to build a deliberately invalid environment:
...
First, the case where there's nothing preventing Qiskit 1.0 from being installed in a valid environment:
...
Historically, Qiskit comprised of many Python distributions: `qiskit-terra`, the compiler core; `qiskit-aer`, the high-performance simulator; the original IBM Quantum provider; and several now-obsolete packages providing particular exploratory algorithmic or experiment-running features.
...
The *code* of the core of Qiskit lived in `qiskit-terra`, which was the owner of the root of the Python package `qiskit` (in other words, `qiskit-terra` controlled what happened when you ran `import qiskit`).
...
Starting with version 1.0, Qiskit comprises a single distribution, called `qiskit`, which in turn installs one single package, also called `qiskit`, which owns all the code contained in its directory.
This is the normal structure of Python code, and is by far the simplest and least error-prone for everybody moving forwards.

The `qiskit-terra` distribution on PyPI will never be updated to version 1.0 or beyond; it is entirely superseded by `qiskit`.
We do not want the `qiskit-terra` name to be involved in the installation any more, for the reasons previously given.
However, we will not remove the `qiskit-terra` package from PyPI, and we will leave its most recent version in a fine working state, so old scientific code and legacy packages can more easily continue to be used with it.

Unfortunately, because of the metapackage legacy and deficiencies in `pip` as a package manager, it is not possible for us to make a completely smooth upgrade path for users through the transition to Qiskit 1.0, especially while some packages depend on earlier versions of Qiskit, and some require only Qiskit 1.0+.
These problems will improve over time, as more and more of the ecosystem migrates to Qiskit 1.0.

<Admonition type="note">
We will not be making breaking packaging changes like this in the future.
This is a one-time event, at the turn of Qiskit 1.0, specifically so that our packaging story will be the easiest for everyone for all the years ahead of us in Qiskit.
</Admonition>
...
If you keep having problems with conflicts after setting up environments, you should strongly avoid running any further `pip install` or `pip uninstall` commands; `pip` does not guarantee to keep the environment coherent on subsequent commands.
...
In both of these cases, there is no guarantee that `pip` will complain, because we could not tell it about the conflict.


<span id="debug-venv-for-1.0"></span>
#### Getting a working environment for Qiskit 1.0

No matter how this happened, it will be much easier to make a new virtual environment.
...
* The dependent is an old package, and will not be updated to support Qiskit 1.0.
  ...
  Typically this is characterized by the dependent being at its latest version (assuming the environment is new, and you didn't pin it lower), and it having a direct requirement on `qiskit-terra`.
...
* The dependent is a package that is actively maintained, but does not yet support Qiskit 1.0.
  ...
  Typically this is characterized by the installed distribution *not* being at its latest version, despite your installation command not specifying a version.
  ...
  This is what has happened in the example above; at the time of writing, `qiskit-dynamics==0.4.4` was the latest release version.

If you constructed this environment out of several `pip install` commands (such as if the environment is old and has been updated), when you build a new environment, first you try to install all your packages using a single `pip install` command.
...
Instead, use the `pipdeptree` commands above to identify which dependencies do not yet support Qiskit 1.0.
...
1. Please [file an issue on Qiskit](https://github.com/Qiskit/qiskit/issues/new/choose) explaining how this happened and why you are sure the environment is correct so we can fix it.
