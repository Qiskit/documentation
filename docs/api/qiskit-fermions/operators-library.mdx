...
The same `tril` and `full` vocabulary names the layout of arrays that are *returned* rather than consumed, such as the integral accessors of [`FCIDump`](/docs/api/qiskit-fermions/operators-library-fci-dump#qiskit_fermions.operators.library.FCIDump "qiskit_fermions.operators.library.FCIDump").
