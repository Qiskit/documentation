...
<Class id="qiskit_ibm_runtime.Batch" isDedicatedPage={true} github="https://github.com/Qiskit/qiskit-ibm-runtime/tree/stable/0.26/qiskit_ibm_runtime/batch.py#L24-L130" signature="Batch(service=None, backend=None, max_time=None)" modifiers="class">
  ...
  *   **backend** (`Union`\[`str`, [BackendV1](/api/qiskit/1.4/qiskit.providers.BackendV1 "(in Qiskit v1.1)"), [`BackendV2`](/api/qiskit/qiskit.providers.BackendV2 "(in Qiskit v1.1)"), `None`]) – Instance of `Backend` class or backend string name. Note that passing a backend name is deprecated.
