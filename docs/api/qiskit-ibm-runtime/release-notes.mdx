---
title: IBM Quantum Compute client release notes
description: Changes made to IBM Quantum Compute client
in_page_toc_max_heading_level: 2
---

<span id="ibm-quantum-compute-formerly-qiskit-runtime-client-release-notes" />

# IBM Quantum Compute client release notes
...
*   [`EstimatorV2`](/docs/api/qiskit-ibm-runtime/estimator-v2#qiskit_ibm_runtime.EstimatorV2 "qiskit_ibm_runtime.EstimatorV2") and [`SamplerV2`](/docs/api/qiskit-ibm-runtime/sampler-v2#qiskit_ibm_runtime.SamplerV2 "qiskit_ibm_runtime.SamplerV2") are deprecated. Please use the new client-side implementations of these primitive interfaces: [`Estimator`](/docs/api/qiskit-ibm-runtime/executor-estimator-estimator#qiskit_ibm_runtime.executor_estimator.Estimator "qiskit_ibm_runtime.executor_estimator.Estimator") and [`Sampler`](/docs/api/qiskit-ibm-runtime/executor-sampler-sampler#qiskit_ibm_runtime.executor_sampler.Sampler "qiskit_ibm_runtime.executor_sampler.Sampler") instead. See the New Features section for more information about them.

    At the end of the deprecation period, the legacy server-side implementations will be removed, and the top-level imports (`qiskit_ibm_runtime.Sampler[V2]` and `qiskit_ibm_runtime.Estimator[V2]` will be updated to point to the client-side implementations. ([3362](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3362))
...
*   The following entities related to `QuantumProgram` have been moved to the `samplomatic` package:
    ...
    *   `qiskit_ibm_runtime.visualization.draw_chunk_timings`
    ...
    Importing these from `qiskit_ibm_runtime` is now deprecated. The imports will continue to work until they are fully removed in a later version. ([3290](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3290))

*   The `max_execution_time` field of `qiskit_ibm_runtime.options_models.EnvironmentOptions` has been deprecated. `max_execution_time` can now be set at top-level. ([3361](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3361))

### New Features

*   Added the client-side primitives [`qiskit_ibm_runtime.executor_sampler.Sampler`](/docs/api/qiskit-ibm-runtime/executor-sampler-sampler#qiskit_ibm_runtime.executor_sampler.Sampler "qiskit_ibm_runtime.executor_sampler.Sampler") and [`qiskit_ibm_runtime.executor_estimator.Estimator`](/docs/api/qiskit-ibm-runtime/executor-estimator-estimator#qiskit_ibm_runtime.executor_estimator.Estimator "qiskit_ibm_runtime.executor_estimator.Estimator").

    The client-side primitives are implementations of the Sampler and Estimator abstractions in Qiskit. Unlike the legacy server-side `qiskit_ibm_runtime.sampler.Sampler` and `qiskit_ibm_runtime.estimator.Estimator` classes, they perform pre- and post-processing on the client side and delegate execution to the [`qiskit_ibm_runtime.Executor`](/docs/api/qiskit-ibm-runtime/executor#qiskit_ibm_runtime.Executor "qiskit_ibm_runtime.Executor"). Running on the client side provides greater transparency, faster debugging feedback, and finer control.

    The client-side primitives share the same API as the legacy primitives, so switching from one to the other only requires updating the imports in your scripts in virtually all cases:
    ...
    ([3332](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3332))

*   Added local mode support to `Executor`.

    When specifying a `FakeBackendV2` or [`AerSimulator`](https://qiskit.github.io/qiskit-aer/stubs/qiskit_aer.AerSimulator.html#qiskit_aer.AerSimulator "(in Qiskit Aer v0.17.1)") as the backend, `run()` will operate as a qiskit\_aer simulator. Behavior can be controlled by the new `SimulatorOptions` class accessed via the `simulator` attribute.
    ...
    ([3288](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3288))

*   The primitives’ `run()` methods:

    *   [`run()`](/docs/api/qiskit-ibm-runtime/estimator-v2#qiskit_ibm_runtime.EstimatorV2.run "qiskit_ibm_runtime.EstimatorV2.run")
    *   [`run()`](/docs/api/qiskit-ibm-runtime/executor#qiskit_ibm_runtime.Executor.run "qiskit_ibm_runtime.Executor.run")
    *   `run()`
    *   `run()`
    *   [`run()`](/docs/api/qiskit-ibm-runtime/sampler-v2#qiskit_ibm_runtime.SamplerV2.run "qiskit_ibm_runtime.SamplerV2.run")

    now accept a new `dry_run` argument. It performs a dry run without executing the job on a QPU. This mode can be used to validate the job, estimate usage consumption, and retrieve circuit timing metadata. Returned results preserve the expected schema but contain randomized mock data rather than actual or simulated measurement results. Unlike the fake backends, the processing of this dry run happens on the server side, so the job might not finish immediately.

    Note that access to this feature may be restricted, and it is not widely available yet. ([3314](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3314))

*   Added [`MeasureReset`](/docs/api/qiskit-ibm-runtime/circuit-library-measure-reset#qiskit_ibm_runtime.circuit.library.MeasureReset "qiskit_ibm_runtime.circuit.library.MeasureReset") instruction to the [`qiskit_ibm_runtime.circuit.library`](/docs/api/qiskit-ibm-runtime/circuit-library#module-qiskit_ibm_runtime.circuit.library "qiskit_ibm_runtime.circuit.library") package. This instruction represents a reset operation that also exposes its intermediate measurement result to the user: it measures a qubit into a classical bit and then conditionally flips the qubit back to $|0\rangle$, so the measurement outcome is retained. The instruction name must start with `measure_reset` (for example: `measure_reset`, `measure_reset_2`), which allows it to be mapped to the corresponding hardware-level implementation. ([3199](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3199))

*   Added [`XSlowGate`](/docs/api/qiskit-ibm-runtime/circuit-library-x-slow-gate#qiskit_ibm_runtime.circuit.library.XSlowGate "qiskit_ibm_runtime.circuit.library.XSlowGate"), a single-qubit gate that represents the hardware-native `xslow` instruction. The gate can be appended to a [`QuantumCircuit`](/docs/api/qiskit/qiskit.circuit.QuantumCircuit) and passes through the Qiskit transpiler unchanged when the backend target includes the `xslow` instruction.
    ...
    ([3244](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3244))

*   Added `to_pauli_lindblad_maps()`, a method to return the list of [`qiskit.quantum_info.PauliLindbladMap`](/docs/api/qiskit/qiskit.quantum_info.PauliLindbladMap) of the items stored in a [`NoiseLearnerV3Results`](/docs/api/qiskit-ibm-runtime/results-noise-learner-v3-results#qiskit_ibm_runtime.results.NoiseLearnerV3Results "qiskit_ibm_runtime.results.noise_learner_v3.NoiseLearnerV3Results"). ([3257](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3257))

*   The following `QuantumProgram` related entities have been moved to the `samplomatic` package, and `qiskit_ibm_runtime` uses them as base classes, providing custom classes for the three of them:
    ...
    *   `qiskit_ibm_runtime.results.QuantumProgramItemResult`
    ...
    ([3290](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3290))

*   [`Executor`](/docs/api/qiskit-ibm-runtime/executor#qiskit_ibm_runtime.Executor "qiskit_ibm_runtime.Executor") and [`NoiseLearnerV3`](/docs/api/qiskit-ibm-runtime/noise-learner-v3-noise-learner-v3#qiskit_ibm_runtime.noise_learner_v3.NoiseLearnerV3 "qiskit_ibm_runtime.noise_learner_v3.NoiseLearnerV3") now have a `mode` property that returns the execution mode used by those primitives. ([3333](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3333))

### Bug Fixes

*   Jobs from retired backends can now be retrieved successfully via [`QiskitRuntimeService.job()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.job "qiskit_ibm_runtime.QiskitRuntimeService.job") and [`QiskitRuntimeService.jobs()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.jobs "qiskit_ibm_runtime.QiskitRuntimeService.jobs"). ([3020](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3020))
*   Fixed [`QiskitRuntimeService.backend()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.backend "qiskit_ibm_runtime.QiskitRuntimeService.backend") so that retrieving a backend without a `calibration_id` after retrieving it with a custom calibration returns the default backend configuration. Calibration-specific backend configurations are no longer cached, so custom calibration data does not replace the cached default backend configuration. ([3275](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3275))
*   The `fallback` extrapolator in [`qiskit_ibm_runtime.options.ZneOptions`](/docs/api/qiskit-ibm-runtime/options-zne-options#qiskit_ibm_runtime.options.ZneOptions "qiskit_ibm_runtime.options.ZneOptions") now requires at least two noise factors. Previously it only required one. ([3321](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3321))
*   `resolve_crn()` now takes into account the proxies and SSL verification arguments set in the account when resolving the CRN. ([3353](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3353))

### Other Notes

*   Bumped the lowest supported version of ibm-quantum-schemas to 0.12.20260921 and samplomatic to 0.21.0. ([3348](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3348))
*   The deprecated Python package `extras` (`common` and `documentation`) has been removed. ([3286](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3286))
...
*   The option classes from the `options_models` package are now based on Pydantic’s `BaseModel` instead of on `dataclass`. ([3042](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3042))
*   Switched default schema version for Executor to 2.0. ([3043](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3043))
*   The Qiskit Runtime service has been renamed to IBM Quantum Compute service, and the references to it in the documentation have been updated. The [`QiskitRuntimeService`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService "qiskit_ibm_runtime.QiskitRuntimeService") class is now available as `IBMQuantumComputeService`. ([3070](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3070))

<span id="id3" />

### Deprecation Notes

*   The usage of stdlib `asdict` with option classes from the `options_models` package is now deprecated, as they are now Pydantic models. Instead, use `options.model_dump()` directly for serializing them to a `dict`. ([3042](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3042))
*   The `post_selection` option [`NoiseLearnerV3`](/docs/api/qiskit-ibm-runtime/noise-learner-v3-noise-learner-v3#qiskit_ibm_runtime.noise_learner_v3.NoiseLearnerV3 "qiskit_ibm_runtime.noise_learner_v3.NoiseLearnerV3") is now deprecated and will be removed in a future release. ([3160](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3160))

<span id="id4" />

### New Features

*   Added [`MidCircuitReset`](/docs/api/qiskit-ibm-runtime/circuit-library-mid-circuit-reset#qiskit_ibm_runtime.circuit.library.MidCircuitReset "qiskit_ibm_runtime.circuit.library.MidCircuitReset") instruction and `ConvertToMidCircuitResetAndMeasure` transpiler pass. The new pass extends the existing [`ConvertToMidCircuitMeasure`](/docs/api/qiskit-ibm-runtime/transpiler-passes-convert-to-mid-circuit-measure#qiskit_ibm_runtime.transpiler.passes.ConvertToMidCircuitMeasure "qiskit_ibm_runtime.transpiler.passes.ConvertToMidCircuitMeasure") functionality by also replacing non-terminal `reset` instructions with hardware-specific mid-circuit reset implementations. ([2903](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2903))
*   Added bit\_flip\_checks field to NoiseLearnerV3Options, allowing users to request bit-flip checks to the results of noise learning circuits. ([2907](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2907))
*   Added an optional `persist` parameter to `FakeBackendV2.refresh()`. By default (`persist=True`) the behavior is unchanged: the refreshed configuration and properties overwrite the data files bundled with the installed package so the update persists across sessions. Passing `persist=False` instead writes the refreshed data to a temporary directory, which allows `refresh()` to succeed even when the package is installed in a read-only location (for example, a system-wide `site-packages` directory) where overwriting the bundled files would otherwise raise a `PermissionError`. In that mode, the backend is updated for the current session only and the changes are not persisted to disk. ([2977](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2977))
*   [`IBMBackend`](/docs/api/qiskit-ibm-runtime/ibm-backend#qiskit_ibm_runtime.IBMBackend "qiskit_ibm_runtime.IBMBackend") now has a `physical_qubits` attribute, which returns the number of physical qubits for the backend (programmable qubits, reset qubits, and couplers combined). The attribute is also available in `FakeBackendV2` for backends that are not retired. ([3080](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3080))
*   The IAM URL used by [`QiskitRuntimeService.instances()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.instances "qiskit_ibm_runtime.QiskitRuntimeService.instances") can now be overridden by the `IAM_URL` environment variable, following the same convention as the IBM SDK’s `GLOBAL_SEARCH_URL` and `GLOBAL_CATALOG_URL` variables. This allows routing IAM traffic through a private endpoint (for example, `https://private.iam.cloud.ibm.com`) in VPE environments without any code changes. ([3135](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3135))

<span id="id5" />

### Bug Fixes

*   Improve the warning shown when a retrieved job was serialized with a newer Qiskit release than is installed locally. Before calling `qiskit.qpy.load`, `RuntimeDecoder` now emits a clear message that suggests upgrading with `pip install -U qiskit` when the embedded QPY format version is newer than what the installed Qiskit supports. ([2426](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2426))
*   [`FakeProviderForBackendV2.backends()`](/docs/api/qiskit-ibm-runtime/fake-provider-fake-provider-for-backend-v2#qiskit_ibm_runtime.fake_provider.FakeProviderForBackendV2.backends "qiskit_ibm_runtime.fake_provider.FakeProviderForBackendV2.backends") now also returns eight fake backends that were importable from [`qiskit_ibm_runtime.fake_provider`](/docs/api/qiskit-ibm-runtime/fake-provider#module-qiskit_ibm_runtime.fake_provider "qiskit_ibm_runtime.fake_provider") but missing from the provider’s backend list, in addition to the ones it already returned: [`FakeAachen`](/docs/api/qiskit-ibm-runtime/fake-provider-fake-aachen#qiskit_ibm_runtime.fake_provider.FakeAachen "qiskit_ibm_runtime.fake_provider.FakeAachen"), [`FakeBerlin`](/docs/api/qiskit-ibm-runtime/fake-provider-fake-berlin#qiskit_ibm_runtime.fake_provider.FakeBerlin "qiskit_ibm_runtime.fake_provider.FakeBerlin"), [`FakeBoston`](/docs/api/qiskit-ibm-runtime/fake-provider-fake-boston#qiskit_ibm_runtime.fake_provider.FakeBoston "qiskit_ibm_runtime.fake_provider.FakeBoston"), [`FakeBrussels`](/docs/api/qiskit-ibm-runtime/fake-provider-fake-brussels#qiskit_ibm_runtime.fake_provider.FakeBrussels "qiskit_ibm_runtime.fake_provider.FakeBrussels"), [`FakeKingston`](/docs/api/qiskit-ibm-runtime/fake-provider-fake-kingston#qiskit_ibm_runtime.fake_provider.FakeKingston "qiskit_ibm_runtime.fake_provider.FakeKingston"), [`FakeMiami`](/docs/api/qiskit-ibm-runtime/fake-provider-fake-miami#qiskit_ibm_runtime.fake_provider.FakeMiami "qiskit_ibm_runtime.fake_provider.FakeMiami"), [`FakePittsburgh`](/docs/api/qiskit-ibm-runtime/fake-provider-fake-pittsburgh#qiskit_ibm_runtime.fake_provider.FakePittsburgh "qiskit_ibm_runtime.fake_provider.FakePittsburgh"), and [`FakeStrasbourg`](/docs/api/qiskit-ibm-runtime/fake-provider-fake-strasbourg#qiskit_ibm_runtime.fake_provider.FakeStrasbourg "qiskit_ibm_runtime.fake_provider.FakeStrasbourg"). Previously, calling `FakeProviderForBackendV2().backend(name="fake_kingston")` (and the other seven) raised a `QiskitBackendNotFoundError` even though the backend classes existed. ([3164](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3164))
...
*   The minimal version of Qiskit required is now `2.3.0`, and that of Samplomatic is `0.18.0`. Users of `Executor` can now twirl their circuits with the `"local_c1"` twirling group, which is supported by Samplomatic version `0.17.0` and above. ([2925](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2925))

<span id="id8" />

### Deprecation Notes

*   All classes that represent results are now available via the `qiskit_ibm_runtime.results` package; consequently, the following classes moved to the results package:

    *   `PauliLindbladError`, `LayerError`, `NoiseLearnerResult` (previously part of `qiskit_ibm_runtime.utils.noise_learner_result`)
    *   `EstimatorPubResult` (previously part of `qiskit_ibm_runtime.utils.estimator_pub_result`)

    Importing from the previous locations is deprecated, and the references will be removed in a future release. ([2835](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2835))

*   Running a [`SamplerV2`](/docs/api/qiskit-ibm-runtime/sampler-v2#qiskit_ibm_runtime.SamplerV2 "qiskit_ibm_runtime.SamplerV2") job with different `pub.shots` values across PUBs is deprecated. Similarly, running an [`EstimatorV2`](/docs/api/qiskit-ibm-runtime/estimator-v2#qiskit_ibm_runtime.EstimatorV2 "qiskit_ibm_runtime.EstimatorV2") job with different `pub.precision` values across PUBs is deprecated. ([2992](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2992))

*   Setting [`MeasureNoiseLearningOptions.shots_per_randomization`](/docs/api/qiskit-ibm-runtime/options-measure-noise-learning-options#qiskit_ibm_runtime.options.MeasureNoiseLearningOptions.shots_per_randomization "qiskit_ibm_runtime.options.MeasureNoiseLearningOptions.shots_per_randomization") to an `int` value is deprecated and will be removed in future releases. Please use `"auto"` instead. ([3004](https://github.com/Qiskit/qiskit-ibm-runtime/pull/3004))

<span id="id9" />

### New Features

*   The [`RuntimeJobV2.usage()`](/docs/api/qiskit-ibm-runtime/runtime-job-v2#qiskit_ibm_runtime.RuntimeJobV2.usage "qiskit_ibm_runtime.RuntimeJobV2.usage") method now supports an optional `partial` parameter ([2884](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2884)):

    *   if the `partial` flag is `False`, the method will return `0` until the job usage calculation is completed.
    *   if the `partial` flag is `True`, the method will return the incremential partial job usage regardless of the calculation being completed.

*   Added options to request circuit schedule timing and to return numeric resolutions of stretches for individual circuits when running Executor jobs. ([2790](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2790)) ([2789](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2789))

*   Added support for `instance="auto"` in [`QiskitRuntimeService`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService "qiskit_ibm_runtime.QiskitRuntimeService") and [`QiskitRuntimeService.save_account()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.save_account "qiskit_ibm_runtime.QiskitRuntimeService.save_account"). Passing `instance="auto"` explicitly opts in to automatic instance selection without triggering the “instance was not set” warning or the “Loading/Using instance” warnings during backend retrieval. The value can be saved to the account file so it takes effect on every subsequent instantiation. ([2854](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2854))

*   Added a `use_fractional_gates` parameter to [`QiskitRuntimeService.least_busy()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.least_busy "qiskit_ibm_runtime.QiskitRuntimeService.least_busy"), matching the behavior of the same parameter in [`QiskitRuntimeService.backend()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.backend "qiskit_ibm_runtime.QiskitRuntimeService.backend"). When set to `True`, only backends that include fractional gates in their basis gates are considered; backends without it are skipped in favor of the next least-busy qualifying backend. ([2855](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2855))

*   Support for passing multiple result decoders to [`RuntimeJobV2`](/docs/api/qiskit-ibm-runtime/runtime-job-v2#qiskit_ibm_runtime.RuntimeJobV2 "qiskit_ibm_runtime.RuntimeJobV2") and [`RuntimeJobV2.result()`](/docs/api/qiskit-ibm-runtime/runtime-job-v2#qiskit_ibm_runtime.RuntimeJobV2.result "qiskit_ibm_runtime.RuntimeJobV2.result") has been reintroduced. ([2899](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2899))

*   Added support for `QuantumProgram.meas_level = "both"`, which can be used to request that both classified and kerneled measurements be present in the quantum program results. ([2973](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2973))

<span id="id10" />

### Bug Fixes

*   Fixed a bug in the [`FoldRzzAngle`](/docs/api/qiskit-ibm-runtime/transpiler-passes-fold-rzz-angle#qiskit_ibm_runtime.transpiler.passes.FoldRzzAngle "qiskit_ibm_runtime.transpiler.passes.FoldRzzAngle") transpiler pass where `Rzz` gates whose angle wraps exactly onto the `+-pi` boundary (for example `3*pi`, `-3*pi`, `7*pi`) were folded into a circuit that differed from the original by a global phase of `pi`. The pass now derives the dropped `2*pi` winding parity directly from the wrapped angle, so the folded circuit preserves the full unitary (including global phase) for all angles. ([2856](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2856))
*   When using backends from different instances, the service used by [`run()`](/docs/api/qiskit-ibm-runtime/sampler-v2#qiskit_ibm_runtime.SamplerV2.run "qiskit_ibm_runtime.SamplerV2.run"), [`run()`](/docs/api/qiskit-ibm-runtime/estimator-v2#qiskit_ibm_runtime.EstimatorV2.run "qiskit_ibm_runtime.EstimatorV2.run"), and other primitives’ `.run()` methods now ensures that the API client used for submitting the job is the one belonging to the backend, instead of always using the active API client. ([2931](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2931))
*   Fixed the folding of out-of-bounds `rzz` angles during transpilation. With the previous setting there was a bug, where translation passes that ran after the FoldRzzAngle pass could modify angles and make them negative or greater than `pi/2`. The bug is fixed by setting the `angle_bounds` in the backend’s target to `(0, pi/2)` for the `rzz` gate, and registering an `rzz` wrapper with the transpiler’s `WrapAngles` pass. This mechanism replaces the usage of the `IBMDynamicFractionalTranslationPlugin` plugin and the [`FoldRzzAngle`](/docs/api/qiskit-ibm-runtime/transpiler-passes-fold-rzz-angle#qiskit_ibm_runtime.transpiler.passes.FoldRzzAngle "qiskit_ibm_runtime.transpiler.passes.FoldRzzAngle") transpiler pass. ([2960](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2960))

<span id="id11" />

### Other Notes

*   The library now uses the version `2026-04-15` of the Qiskit Runtime REST API. ([2845](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2845))
*   The deprecated transpiler plugin `IBMFractionalTranslationPlugin` has been removed. ([2880](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2880))
*   The default schema version for Executor is now `1.1`. ([2911](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2911))
...
*   Added the Executor and QuantumProgram classes, which allow taking full advantage of the set of capabilities offered by the [Samplomatic](https://github.com/Qiskit/samplomatic) package.
...
*   One new fake backend, `FakeKingston`, has been added. This is a 156-qubit Heron r2 device. ([2711](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2711))

*   The interactions with the IBM Cloud Platform services (which happen during `QiskitRuntimeService` instantation, with the service names `global_search`, `global_catalog` and `resource_controller`) now adhere to the [IBM Cloud SDK configuration mechanisms](https://github.com/IBM/ibm-cloud-sdk-common/blob/main/README.md#using-external-configuration), allowing for custom configuration to be passed via environment variables or credential files. ([2712](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2712))

*   Added [`draw_chunk_timings()`](/docs/api/qiskit-ibm-runtime/visualization-draw-chunk-timings#qiskit_ibm_runtime.visualization.draw_chunk_timings "qiskit_ibm_runtime.visualization.draw_chunk_timings") to [`qiskit_ibm_runtime.visualization`](/docs/api/qiskit-ibm-runtime/visualization#module-qiskit_ibm_runtime.visualization "qiskit_ibm_runtime.visualization") for plotting the execution timing of one or more instances of the new chunk timings container class, [`ChunkTiming`](/docs/api/qiskit-ibm-runtime/results-chunk-timing#qiskit_ibm_runtime.results.ChunkTiming "qiskit_ibm_runtime.results.ChunkTiming"), on an interactive barchart. A convenience [`ChunkTiming.draw()`](/docs/api/qiskit-ibm-runtime/results-chunk-timing#qiskit_ibm_runtime.results.ChunkTiming.draw "qiskit_ibm_runtime.results.ChunkTiming.draw") method is also available for the single-result case via `job.result().timing.draw()`. ([2727](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2727))

*   Building on top of the fake backend snapshot script introduced in [2711](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2736), the script now also generates `__init__.py` and `fake_<backend_name>.py` files for each backend, and adds a new `-a`/`--account` argument to select a named account when fetching backend data from different cloud regions (for example, us-east versus eu-de).

    Using this updated script, the following missing fake backends have been added: `FakeAachen`, `FakeBerlin`, `FakeBoston`, `FakeBrussels`, `FakeMiami`, `FakePittsburgh`, and `FakeStrasbourg`. ([2736](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2736))

<span id="id14" />

### Upgrade Notes

*   The `RuntimeJobV2.result()` and `RuntimeJobV2.wait_for_final_state()` methods now accept an optional `poll_interval` parameter, which allows customizing the interval between requests to the service. The default is now `0.5` seconds for non-session jobs, and `0.1` seconds for session jobs. ([2762](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2762))
...
*   The Python package `extras` have been refactored:
    ...
    *   The development dependencies now have finer granularity (`style`, `style_local`, `test`, `doc`), with the `dev` extra continuing to allow a full development suite to be installed. ([2707](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2707))

<span id="id16" />

### Bug Fixes

*   The `proxies` and `ssl_verification` arguments for `QiskitRuntimeService` are now propagated to the underlying HTTP requests to `` `GlobalSearchV2 ``, `GlobalCatalogV1`, and `IAMAuthenticator` services, allowing to instantiate `QiskitRuntimeService` correctly when using a proxy. ([2592](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2592))
*   Added ability to specify `calibration_id` and `use_fractional_gates` when initializing a session from a `session_id`. This fixes known bugs, namely that instantiating a `Session` object via `from_id` does not preserve the value of `calibration_id` set during the initial instantiation of the original `Session` nor the the value of `use_fractional_gates` set during the initial instantiation of the backend. ([2652](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2652))
*   `NoiseLearner.run()` now correctly supports generators, `map`, `filter`, generator expressions, and other single-pass iterators. Previously these were exhausted after the initial type check, causing jobs to be submitted with zero circuits. ([2759](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2759))
...
*   The minimal version of Qiskit required is now `2.2.0`. ([2689](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2689))
...
*   While creating backend objects, .QiskitRuntimeService previously checked for only `rzz` gates in the backend’s basis gates to decide whether to use cache or create a new backend object. Now .QiskitRuntimeService will check for both `rzz` and `rx` gates in the backend’s basis gates to decide whether to refresh the configuration or fetch from cache. ([2524](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2524))
*   The initialization of `QiskitRuntimeService` no longer fails if it is not possible to retrieve the catalog information of an instance (for example, due to the instance being archived). ([2603](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2603))
...
*   Submitting circuits with non-empty `circuit.calibrations` is deprecated and will be removed in `qiskit-ibm-runtime` v0.45. ([2574](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2574))
...
*   If a backend that does not support fractional gates is requested by setting the flag `use_fractional_gates=True` in [`QiskitRuntimeService.backend()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.backend "qiskit_ibm_runtime.QiskitRuntimeService.backend"), `IBMInputValueError` will be raised. ([2405](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2405))

*   The URL resolver was updated so that both `ibm_cloud` and `ibm_quantum_platform` channels now generate the same runtime entrypoint URL (`https://quantum.cloud.ibm.com/api/v[x]`). Previously, the `ibm_cloud` channel produced a decommissioned URL and relied on redirects. This change removes that dependency and aligns URL generation across channels. There are no user-facing changes. ([2486](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2486))

*   A y-axis channel ordering for `draw_circuit_schedule_timing()` has now been imposed, according to the order of channels in the `included_channels` input argument. ([2492](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2492))

*   Added a new `merge_common_instructions` input argument to `draw_circuit_schedule_timing()` that, if set to `True`, will merge instructions of the same type based on temporal continuity.

    Example ([2494](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2494)):
...
*   Added a “Fake Nighthawk” (`FakeNighthawk`) backend to the [`fake_provider`](/docs/api/qiskit-ibm-runtime/fake-provider#module-qiskit_ibm_runtime.fake_provider "qiskit_ibm_runtime.fake_provider") module.
...
*   The [`qiskit_ibm_runtime.transpiler.passes`](/docs/api/qiskit-ibm-runtime/transpiler#module-qiskit_ibm_runtime.transpiler.passes "qiskit_ibm_runtime.transpiler.passes") module has been extended with a new transpiler pass: `ConvertMidCircMeas`. This pass detects non-terminal [`Measure`](/docs/api/qiskit/circuit#qiskit.circuit.Measure) instructions in a circuit and converts them into mid-circuit measurement instructions supported by the target. The pass is constructed with two arguments:
    ...
    *   ## `mcm_name` (optional): The name of the mid-circuit measurement instruction to use.
    ...
    During initialization, the pass validates that the specified target supports the given mid-circuit measurement instruction on at least one qubit. If not, construction fails. When executed, the pass does the following:

    *   It runs on an already routed circuit.
    *   It replaces only non-terminal `` Measure` `` instructions with the specified mid-circuit measurement instruction, provided the target supports it on the corresponding physical qubit.

    It does not do the following:

    *   It does not replace terminal measurements.
    *   It does not replace existing mid-circuit measurement instructions (for example, `measure_2` to `measure_3`).
    *   It does not convert mid-circuit measurements back to terminal measurements.

    Example usage ([2317](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2317)):
...
*   When initializing [`QiskitRuntimeService`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService "qiskit_ibm_runtime.QiskitRuntimeService") with a saved account, the `warning` level log messages regarding the saved account name (or default account) have been lowered to the `info` level. ([2445](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2445))
...
*   Added a new `target` argument to the initializer following transpiler analysis and padding passes: [`ALAPScheduleAnalysis`](/docs/api/qiskit-ibm-runtime/transpiler-passes-scheduling-alap-schedule-analysis#qiskit_ibm_runtime.transpiler.passes.scheduling.ALAPScheduleAnalysis "qiskit_ibm_runtime.transpiler.passes.scheduling.ALAPScheduleAnalysis"), [`PadDelay`](/docs/api/qiskit-ibm-runtime/transpiler-passes-scheduling-pad-delay#qiskit_ibm_runtime.transpiler.passes.scheduling.PadDelay "qiskit_ibm_runtime.transpiler.passes.scheduling.PadDelay"), [`PadDynamicalDecoupling`](/docs/api/qiskit-ibm-runtime/transpiler-passes-scheduling-pad-dynamical-decoupling#qiskit_ibm_runtime.transpiler.passes.scheduling.PadDynamicalDecoupling "qiskit_ibm_runtime.transpiler.passes.scheduling.PadDynamicalDecoupling"), [`BlockBasePadder`](/docs/api/qiskit-ibm-runtime/transpiler-passes-scheduling-block-base-padder#qiskit_ibm_runtime.transpiler.passes.scheduling.BlockBasePadder "qiskit_ibm_runtime.transpiler.passes.scheduling.BlockBasePadder"). This change aligns these passes with the broader Qiskit transpiler architecture, and supersedes the use of the `durations` argument.

    The [`DynamicCircuitInstructionDurations`](/docs/api/qiskit-ibm-runtime/transpiler-passes-scheduling-dynamic-circuit-instruction-durations#qiskit_ibm_runtime.transpiler.passes.scheduling.DynamicCircuitInstructionDurations "qiskit_ibm_runtime.transpiler.passes.scheduling.DynamicCircuitInstructionDurations") class, used in custom scheduling passes, has been deprecated as of `qiskit-ibm-runtime` v0.43. This class was optimized for scheduling operations on Eagle processors, and it has fallen out of date with the current offering of Heron processors. This class was used to define the `` `durations `` argument in the scheduling passes listed above ([`ALAPScheduleAnalysis`](/docs/api/qiskit-ibm-runtime/transpiler-passes-scheduling-alap-schedule-analysis#qiskit_ibm_runtime.transpiler.passes.scheduling.ALAPScheduleAnalysis "qiskit_ibm_runtime.transpiler.passes.scheduling.ALAPScheduleAnalysis"), [`PadDelay`](/docs/api/qiskit-ibm-runtime/transpiler-passes-scheduling-pad-delay#qiskit_ibm_runtime.transpiler.passes.scheduling.PadDelay "qiskit_ibm_runtime.transpiler.passes.scheduling.PadDelay"), [`PadDynamicalDecoupling`](/docs/api/qiskit-ibm-runtime/transpiler-passes-scheduling-pad-dynamical-decoupling#qiskit_ibm_runtime.transpiler.passes.scheduling.PadDynamicalDecoupling "qiskit_ibm_runtime.transpiler.passes.scheduling.PadDynamicalDecoupling"), [`BlockBasePadder`](/docs/api/qiskit-ibm-runtime/transpiler-passes-scheduling-block-base-padder#qiskit_ibm_runtime.transpiler.passes.scheduling.BlockBasePadder "qiskit_ibm_runtime.transpiler.passes.scheduling.BlockBasePadder")). The argument is also deprecated and will be removed in a future release. Users are encouraged to migrate to the `target` argument. ([2403](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2403))
...
*   The [`qiskit_ibm_runtime`](/docs/api/qiskit-ibm-runtime/runtime-service#module-qiskit_ibm_runtime "qiskit_ibm_runtime") package has been extended with two new modules: `circuit` and [`circuit.library`](/docs/api/qiskit-ibm-runtime/circuit-library#module-qiskit_ibm_runtime.circuit.library "qiskit_ibm_runtime.circuit.library"). These modules are designed to mirror the structure of the corresponding `qiskit` SDK modules, while providing vendor-specific implementations of circuit objects and instructions.

    The first addition to this new circuit library is the [`MidCircuitMeasure`](/docs/api/qiskit-ibm-runtime/circuit-library-mid-circuit-measure#qiskit_ibm_runtime.circuit.library.MidCircuitMeasure "qiskit_ibm_runtime.circuit.library.MidCircuitMeasure") class. This class enables the creation of instructions that follow the naming convention `measure_<identifier>`, which are mapped to specific mid-circuit measurement hardware instructions matching that pattern. The default name for this instruction is `"measure_2"`.
...
*   It is now possible to retrieve the job tags of a job without having to actually fetch the job with [`QiskitRuntimeService.job()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.job "qiskit_ibm_runtime.QiskitRuntimeService.job"). ([2420](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2420))
...
*   [`RuntimeJobV2.properties()`](/docs/api/qiskit-ibm-runtime/runtime-job-v2#qiskit_ibm_runtime.RuntimeJobV2.properties "qiskit_ibm_runtime.RuntimeJobV2.properties") will now fetch the backend properties from when the job started running instead of when the job was created. ([2369](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2369))

*   Fixed an inconsistency in the unit conversion of `rep_times` in [`QasmBackendConfiguration`](/docs/api/qiskit-ibm-runtime/models-qasm-backend-configuration#qiskit_ibm_runtime.models.QasmBackendConfiguration "qiskit_ibm_runtime.models.QasmBackendConfiguration") to maintain consistency between initialization and serialization. The parameter is now properly converted from microseconds to seconds during initialization and back to microseconds when serialized through `to_dict()`, matching the behavior of other timing parameters: `rep_delay_range` and `default_rep_delay`. ([2386](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2386))

*   A consistent error will now be raised during service initialization if an invalid API token is given to [`QiskitRuntimeService`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService "qiskit_ibm_runtime.QiskitRuntimeService").
...
*   Fixed multiple bugs where having a default saved instance or passing in the instance name would result in an an `IBMInputValueError` when creating a [`Session`](/docs/api/qiskit-ibm-runtime/session#qiskit_ibm_runtime.Session "qiskit_ibm_runtime.Session") / [`Batch`](/docs/api/qiskit-ibm-runtime/batch#qiskit_ibm_runtime.Batch "qiskit_ibm_runtime.Batch")
...
> *   [`QiskitRuntimeService.least_busy()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.least_busy "qiskit_ibm_runtime.QiskitRuntimeService.least_busy") is significantly faster.
> *   [`QiskitRuntimeService.usage()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.usage "qiskit_ibm_runtime.QiskitRuntimeService.usage") has been updated to return information regarding the current instance usage limit, consumption, and time remaining.
...
*   With the migration to the new IBM Quantum Platform, there were a few inconsistencies with IBM Quantum Platform Classic that needed to be addressed:
    ...
    > *   [`QiskitRuntimeService.usage()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.usage "qiskit_ibm_runtime.QiskitRuntimeService.usage") now returns usage information for the current active IBM Cloud instance.
    > *   [`QiskitRuntimeService.delete_job()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.delete_job "qiskit_ibm_runtime.QiskitRuntimeService.delete_job") is not supported on the new platform, so it has been deprecated.
...
*   There is a new method, [`QiskitRuntimeService.active_instance()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.active_instance "qiskit_ibm_runtime.QiskitRuntimeService.active_instance") which returns the IBM Cloud CRN of the current active instance.

    The [`QiskitRuntimeService.jobs()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.jobs "qiskit_ibm_runtime.QiskitRuntimeService.jobs") method has been updated to properly accept the `instance` parameter, which can be used to filter jobs. ([2325](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2325))

*   [`QiskitRuntimeService.usage()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.usage "qiskit_ibm_runtime.QiskitRuntimeService.usage") has been updated to include a `usage_remaining_seconds` field. This field includes the difference between the reported `usage_limit_seconds` or `usage_allocation_seconds` (depending on how the instance is configured) and `usage_consumed_seconds`. Learn more about instance allocation limits [here](/docs/guides/allocation-limits). ([2329](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2329))
...
*   Class [`TwirledSliceSpan`](/docs/api/qiskit-ibm-runtime/execution-span-twirled-slice-span#qiskit_ibm_runtime.execution_span.TwirledSliceSpan "qiskit_ibm_runtime.execution_span.TwirledSliceSpan") has a new data member `data_slice_version` of type integer, with default value 1. When set to 2, the data slice tuples contain information about the PUB shots, used in [`TwirledSliceSpan.mask()`](/docs/api/qiskit-ibm-runtime/execution-span-twirled-slice-span#qiskit_ibm_runtime.execution_span.TwirledSliceSpan.mask "qiskit_ibm_runtime.execution_span.TwirledSliceSpan.mask") to shape the returned array. The last axis will be truncated, such that its length will be shortened to `pub_shots`. ([2312](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2312))
...
*   Fixed a bug in `BasePrimitive` where primitives instantiated inside a [`Session`](/docs/api/qiskit-ibm-runtime/session#qiskit_ibm_runtime.Session "qiskit_ibm_runtime.Session") or [`Batch`](/docs/api/qiskit-ibm-runtime/batch#qiskit_ibm_runtime.Batch "qiskit_ibm_runtime.Batch") context manager without the `mode` parameter would fetch the backend from the service (by name) instead of using the backend passed to the [`Session`](/docs/api/qiskit-ibm-runtime/session#qiskit_ibm_runtime.Session "qiskit_ibm_runtime.Session") or [`Batch`](/docs/api/qiskit-ibm-runtime/batch#qiskit_ibm_runtime.Batch "qiskit_ibm_runtime.Batch"). This could cause issues when the [`Session`](/docs/api/qiskit-ibm-runtime/session#qiskit_ibm_runtime.Session "qiskit_ibm_runtime.Session") or [`Batch`](/docs/api/qiskit-ibm-runtime/batch#qiskit_ibm_runtime.Batch "qiskit_ibm_runtime.Batch") backend was modified by users (for example, by removing a gate), because the primitives would instead fetch the unmodified backend object from the service. After the fix, the [`Session`](/docs/api/qiskit-ibm-runtime/session#qiskit_ibm_runtime.Session "qiskit_ibm_runtime.Session") or [`Batch`](/docs/api/qiskit-ibm-runtime/batch#qiskit_ibm_runtime.Batch "qiskit_ibm_runtime.Batch") backend object is used directly. ([2282](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2282))
*   Fixed an issue in [`IBMBackend`](/docs/api/qiskit-ibm-runtime/ibm-backend#qiskit_ibm_runtime.IBMBackend "qiskit_ibm_runtime.IBMBackend") where property changes, such as modifications to `basis_gates`, persisted even after the backend object was renewed. ([2283](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2283))
*   Fixed the serialization of execution spans, to ensure that executions of old versions of qiskit-ibm-runtime will not crash when trying to decode newly encoded execution spans. However the deserialization with old versions will now return the execution spans in the format of a dictionary, and not in the form of an [`ExecutionSpans`](/docs/api/qiskit-ibm-runtime/execution-span-execution-spans#qiskit_ibm_runtime.execution_span.ExecutionSpans "qiskit_ibm_runtime.execution_span.ExecutionSpans") object. As part of this work, we also removed the `data_slice_version` field from [`TwirledSliceSpan`](/docs/api/qiskit-ibm-runtime/execution-span-twirled-slice-span#qiskit_ibm_runtime.execution_span.TwirledSliceSpan "qiskit_ibm_runtime.execution_span.TwirledSliceSpan"); twirled slice spans that are aware of the pub shots are now managed by a new class `TwirledSliceSpanV2`. ([2347](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2347))

<span id="id50" />

### Other Notes

*   The deprecated [`IBMBackend`](/docs/api/qiskit-ibm-runtime/ibm-backend#qiskit_ibm_runtime.IBMBackend "qiskit_ibm_runtime.IBMBackend") attributes, `max_shots` and `max_experiments`, have been removed and the `IBMBackend.max_circuits()` method now returns `None`. See the [job limits guide](/docs/guides/job-limits#job-limits) for details. ([2235](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2235))
*   The deprecated [`IBMBackend`](/docs/api/qiskit-ibm-runtime/ibm-backend#qiskit_ibm_runtime.IBMBackend "qiskit_ibm_runtime.IBMBackend") and `FakeBackendV2` `defaults()` methods have been removed. They were deprecated in the v0.38.0 release. Pulse defaults have also been removed from all fake backends. ([2238](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2238))
*   Improved the error message returned when [`ExecutionSpan.mask()`](/docs/api/qiskit-ibm-runtime/execution-span-execution-span#qiskit_ibm_runtime.execution_span.ExecutionSpan.mask "qiskit_ibm_runtime.execution_span.ExecutionSpan.mask") is called for a PUB that’s not contained in the span. ([2311](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2311))
...
*   Fixed an issue where if there was no instance saved nor passed in at initialization, the service could not make any API calls until [`QiskitRuntimeService.backend()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.backend "qiskit_ibm_runtime.QiskitRuntimeService.backend") or [`QiskitRuntimeService.backends()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.backends "qiskit_ibm_runtime.QiskitRuntimeService.backends") is called first.
...
*   The following changes were made to support the upcoming [IBM Quantum platform migration](https://github.com/Qiskit/documentation/blob/00575e951e6b397cbd9cba016b653979369c8cd5/docs/migration-guides/classic-iqp-to-cloud-iqp.mdx):
    ...
    *   The [`QiskitRuntimeService.backend()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.backend "qiskit_ibm_runtime.QiskitRuntimeService.backend") and [`QiskitRuntimeService.backends()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.backends "qiskit_ibm_runtime.QiskitRuntimeService.backends") methods have been updated to accept an `instance` passed in explicitly when retrieving backends: `service.backend(name="...", instance="...")`.
...
*   Pub inputs to [`EstimatorV2`](/docs/api/qiskit-ibm-runtime/estimator-v2#qiskit_ibm_runtime.EstimatorV2 "qiskit_ibm_runtime.EstimatorV2") are now subject to a new validation step that checks that observables consist of Pauli operations that only contain \{I, X, Y, Z}. ([2254](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2254))

<span id="id59" />

### Bug Fixes

*   A new parameter `create_new` has been added to the [`Batch`](/docs/api/qiskit-ibm-runtime/batch#qiskit_ibm_runtime.Batch "qiskit_ibm_runtime.Batch") and [`Session`](/docs/api/qiskit-ibm-runtime/session#qiskit_ibm_runtime.Session "qiskit_ibm_runtime.Session") classes. This parameter allows users to control whether the POST session API endpoint should be called when initializing the class. It defaults to `True` as this is the case for most user-facing workflows. However, methods such as [`Session.from_id()`](/docs/api/qiskit-ibm-runtime/session#qiskit_ibm_runtime.Session.from_id "qiskit_ibm_runtime.Session.from_id") set it to `False` to avoid generating a new session when the original session is still active. This fixed issues where multiple sessions were generated simultaneously when calling `.from_id()`. ([2195](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2195))
*   Removed the incorrect `Optional` type hint for the `backend` parameter in [`Session`](/docs/api/qiskit-ibm-runtime/session#qiskit_ibm_runtime.Session "qiskit_ibm_runtime.Session") and [`Batch`](/docs/api/qiskit-ibm-runtime/batch#qiskit_ibm_runtime.Batch "qiskit_ibm_runtime.Batch"). ([2222](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2222))
*   [`Session.from_id()`](/docs/api/qiskit-ibm-runtime/session#qiskit_ibm_runtime.Session.from_id "qiskit_ibm_runtime.Session.from_id") now raises an error if the session does not have a known backend. This is usually the case if there haven’t been any jobs run in the session yet. ([2226](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2226))
...
*   `RuntimeJob` is now deprecated and will be removed no sooner than three months from this release. `RuntimeJob` was superseded by [`RuntimeJobV2`](/docs/api/qiskit-ibm-runtime/runtime-job-v2#qiskit_ibm_runtime.RuntimeJobV2 "qiskit_ibm_runtime.RuntimeJobV2") in all primitive implementations, so the deprecation should not have any user impact. The major difference between the two classes is that [`status()`](/docs/api/qiskit-ibm-runtime/runtime-job-v2#qiskit_ibm_runtime.RuntimeJobV2.status "qiskit_ibm_runtime.RuntimeJobV2.status") returns a string instead of Qiskit’s `JobStatus` enum. ([2170](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2170))
*   Since pulse gates are no longer supported, the [`IBMBackend`](/docs/api/qiskit-ibm-runtime/ibm-backend#qiskit_ibm_runtime.IBMBackend "qiskit_ibm_runtime.IBMBackend") and `FakeBackendV2` `defaults()` method has been deprecated and will be removed no sooner than three months from this release. While the method still exists, these pulse defaults are no longer used to construct the backend target. ([2186](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2186))
...
*   Added a new transpiler translation plugin `IBMFractionalTranslationPlugin` and a pass [`FoldRzzAngle`](/docs/api/qiskit-ibm-runtime/transpiler-passes-fold-rzz-angle#qiskit_ibm_runtime.transpiler.passes.FoldRzzAngle "qiskit_ibm_runtime.transpiler.passes.FoldRzzAngle"). This plugin is automatically applied for backends retrieved with the `use_fractional_gates` opt-in, and the folding pass is added when the backend target includes the `RZZ` gate.
...
*   Added [`draw_layer_errors_swarm()`](/docs/api/qiskit-ibm-runtime/visualization-draw-layer-errors-swarm#qiskit_ibm_runtime.visualization.draw_layer_errors_swarm "qiskit_ibm_runtime.visualization.draw_layer_errors_swarm") which draws a swarm plot of one or more [`LayerError`](/docs/api/qiskit-ibm-runtime/results-layer-error#qiskit_ibm_runtime.results.LayerError "qiskit_ibm_runtime.results.LayerError") objects. Also added the convenience method [`draw_swarm()`](/docs/api/qiskit-ibm-runtime/results-layer-error#qiskit_ibm_runtime.results.LayerError.draw_swarm "qiskit_ibm_runtime.results.LayerError.draw_swarm") to invoke the drawing function on a particular instance. ([1988](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1988))
*   Added [`TwirledSliceSpan`](/docs/api/qiskit-ibm-runtime/execution-span-twirled-slice-span#qiskit_ibm_runtime.execution_span.TwirledSliceSpan "qiskit_ibm_runtime.execution_span.TwirledSliceSpan"), an `ExecutionSpan` to be used when twirling is enabled in the Sampler. In particular, it keeps track of an extra shape axis corresponding to twirling randomizations, and also whether this axis exists at the front of the shape tuple, or right before the shots axis. ([2011](https://github.com/Qiskit/qiskit-ibm-runtime/pull/2011))
...
*   Added [`draw_execution_spans()`](/docs/api/qiskit-ibm-runtime/visualization-draw-execution-spans#qiskit_ibm_runtime.visualization.draw_execution_spans "qiskit_ibm_runtime.visualization.draw_execution_spans"), a function for creating a Plotly figure that visualizes one or more [`ExecutionSpans`](/docs/api/qiskit-ibm-runtime/execution-span-execution-spans#qiskit_ibm_runtime.execution_span.ExecutionSpans "qiskit_ibm_runtime.execution_span.ExecutionSpans") objects. Also added the convenience method [`draw()`](/docs/api/qiskit-ibm-runtime/execution-span-execution-spans#qiskit_ibm_runtime.execution_span.ExecutionSpans.draw "qiskit_ibm_runtime.execution_span.ExecutionSpans.draw") to invoke the drawing function on a particular instance. ([1923](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1923))
...
*   Added [`DoubleSliceSpan`](/docs/api/qiskit-ibm-runtime/execution-span-double-slice-span#qiskit_ibm_runtime.execution_span.DoubleSliceSpan "qiskit_ibm_runtime.execution_span.DoubleSliceSpan"), an `ExecutionSpan` for batching with two slices. ([1982](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1982))

*   Each of [`SamplerV2`](/docs/api/qiskit-ibm-runtime/sampler-v2#qiskit_ibm_runtime.SamplerV2 "qiskit_ibm_runtime.SamplerV2"), [`EstimatorV2`](/docs/api/qiskit-ibm-runtime/estimator-v2#qiskit_ibm_runtime.EstimatorV2 "qiskit_ibm_runtime.EstimatorV2"), and [`noise_learner.NoiseLearner`](/docs/api/qiskit-ibm-runtime/noise-learner-noise-learner#qiskit_ibm_runtime.noise_learner.NoiseLearner "qiskit_ibm_runtime.noise_learner.NoiseLearner") now has a `backend()` method that returns the backend that the class is configured with. ([1995](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1995))
...
*   Added `url_resolver` optional input to [`QiskitRuntimeService`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService "qiskit_ibm_runtime.QiskitRuntimeService") constructor to enable custom generation of the Qiskit Runtime API URL based on the provided `url`, `instance` and `private_endpoint`. If not specified, the default resolver will be used.
...
*   We added new classes, [`ExecutionSpan`](/docs/api/qiskit-ibm-runtime/execution-span-execution-span#qiskit_ibm_runtime.execution_span.ExecutionSpan "qiskit_ibm_runtime.execution_span.ExecutionSpan") and `ExecutionSpanSet`. These classes are used in the primitive result metadata, to convey information about start and stop times of batch jobs. ([1833](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1833))
...
*   The V1 Primitives `SamplerV1` and `EstimatorV1` have been completely removed. See the [Introduction to primitives](/docs/guides/primitives) to learn how to use the V2 Primitives. ([1857](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1857))
...
*   The `backend` argument in [Sampler](/docs/guides/get-started-with-sampler) and [Estimator](/docs/guides/get-started-with-estimator) has been deprecated. Please use `mode` instead. The `session` argument in [Sampler](/docs/guides/get-started-with-sampler) and [Estimator](/docs/guides/get-started-with-estimator) has also been deprecated. Please use `mode` instead. ([1556](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1556))

*   `qiskit_ibm_runtime.QiskitRuntimeService.get_backend()` is deprecated. Please [`qiskit_ibm_runtime.QiskitRuntimeService.backend()`](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#qiskit_ibm_runtime.QiskitRuntimeService.backend "qiskit_ibm_runtime.QiskitRuntimeService.backend") use instead. The V1 fake backends, `FakeBackend`, along with `FakeProvider` are also being deprecated in favor of the V2 fake backends and [`FakeProviderForBackendV2`](/docs/api/qiskit-ibm-runtime/fake-provider-fake-provider-for-backend-v2#qiskit_ibm_runtime.fake_provider.FakeProviderForBackendV2 "qiskit_ibm_runtime.fake_provider.FakeProviderForBackendV2"). ([1689](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1689))
...
*   You can now use the experimental option in [`qiskit_ibm_runtime.options.EstimatorOptions`](/docs/api/qiskit-ibm-runtime/options-estimator-options#qiskit_ibm_runtime.options.EstimatorOptions "qiskit_ibm_runtime.options.EstimatorOptions") to enable Probabilistic Error Amplification (PEA) error mitigation method for your estimator jobs. ([1728](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1728))
...
*   Add `block_ordering_callable` argument to [`BlockBasePadder`](/docs/api/qiskit-ibm-runtime/transpiler-passes-scheduling-block-base-padder#qiskit_ibm_runtime.transpiler.passes.scheduling.BlockBasePadder "qiskit_ibm_runtime.transpiler.passes.scheduling.BlockBasePadder"), [`PadDynamicalDecoupling`](/docs/api/qiskit-ibm-runtime/transpiler-passes-scheduling-pad-dynamical-decoupling#qiskit_ibm_runtime.transpiler.passes.scheduling.PadDynamicalDecoupling "qiskit_ibm_runtime.transpiler.passes.scheduling.PadDynamicalDecoupling"), [`PadDelay`](/docs/api/qiskit-ibm-runtime/transpiler-passes-scheduling-pad-delay#qiskit_ibm_runtime.transpiler.passes.scheduling.PadDelay "qiskit_ibm_runtime.transpiler.passes.scheduling.PadDelay"), and `BaseDynamicCircuitAnalysis`. This allows the user to construct blocks using an algorithm of their choosing. No assumptions or checks are made on the validity of the output that the `block_ordering_callable` produces. The motivation for this argument is that for some families of circuits, the existing function `block_order_op_nodes` can be very slow. ([1531](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1531))
...
*   [`SamplerV2`](/docs/api/qiskit-ibm-runtime/sampler-v2#qiskit_ibm_runtime.SamplerV2 "qiskit_ibm_runtime.SamplerV2") now supports twirling. Twirling will only be applied to those measurement registers not involved within a conditional logic. ([1557](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1557))
...
*   [`RuntimeJobV2`](/docs/api/qiskit-ibm-runtime/runtime-job-v2#qiskit_ibm_runtime.RuntimeJobV2 "qiskit_ibm_runtime.RuntimeJobV2") will no longer error when the API returns an unknown status. Instead, the status from the API will directly be returned. ([1476](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1476))
...
*   Fixed an issue where the [in\_final\_state()](/docs/api/qiskit-ibm-runtime/runtime-job-v2#in_final_state) method in [`RuntimeJobV2`](/docs/api/qiskit-ibm-runtime/runtime-job-v2#qiskit_ibm_runtime.RuntimeJobV2 "qiskit_ibm_runtime.RuntimeJobV2") would not update the status when called. ([1547](https://github.com/Qiskit/qiskit-ibm-runtime/pull/1547))
...
*   Custom programs are being deprecated as of qiskit-ibm-runtime 0.14.0 and will be removed on November 27, 2023. Users can instead convert their custom programs to use Qiskit Runtime primitives with Qiskit Serverless.
