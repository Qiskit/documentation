---
title: scheduling (v0.43)
description: API reference for qiskit_ibm_runtime.transpiler.passes.scheduling in qiskit-ibm-runtime v0.43
...
![Circuit diagram output by the previous code.](/docs/images/api/qiskit-ibm-runtime/0.43/qiskit_ibm_runtime-transpiler-passes-scheduling-1.avif)
...
![Circuit diagram output by the previous code.](/docs/images/api/qiskit-ibm-runtime/0.43/qiskit_ibm_runtime-transpiler-passes-scheduling-2.avif)

When compiling a circuit with Qiskit, it is more efficient and more robust to perform all the transformations in a single transpilation. This has been done above by extending Qiskit’s preset pass managers. For example, Qiskit’s [`transpile()`](/docs/api/qiskit/compiler#qiskit.compiler.transpile "(in Qiskit v2.2)") function internally builds its pass set by using `generate_preset_pass_manager()`. This returns instances of [`StagedPassManager`](/docs/api/qiskit/qiskit.transpiler.StagedPassManager "(in Qiskit v2.2)"), which can be extended.
