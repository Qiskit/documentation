...
When compiling a circuit with Qiskit, it is more efficient and more robust to perform all the transformations in a single transpilation. This has been done above by extending Qiskit’s preset pass managers. For example, Qiskit’s [`transpile()`](https://quantum.cloud.ibm.com/docs/api/qiskit/compiler#qiskit.compiler.transpile "(in Qiskit v2.0)") function internally builds its pass set by using `generate_preset_pass_manager()`. This returns instances of [`StagedPassManager`](https://quantum.cloud.ibm.com/docs/api/qiskit/qiskit.transpiler.StagedPassManager "(in Qiskit v2.0)"), which can be extended.
