...
A collection of scheduling passes for working with IBM Quantum’s next-generation backends that support advanced “dynamic circuit” capabilities. Ie., circuits with support for classical control-flow/feedback based off of measurement results. For more information on dynamic circuits, see our [Classical feedforward and control flow](/docs/en/guides/classical-feedforward-and-control-flow) guide.
