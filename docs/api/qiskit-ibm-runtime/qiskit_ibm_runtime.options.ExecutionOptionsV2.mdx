...
<Class id="qiskit_ibm_runtime.options.ExecutionOptionsV2" isDedicatedPage={true} github="https://github.com/Qiskit/qiskit-ibm-runtime/tree/stable/0.23/qiskit_ibm_runtime/options/execution_options.py#L20-L37" signature="ExecutionOptionsV2(*args, **kwargs)">
  ...
  *   **rep\_delay** – The repetition delay. This is the delay between a measurement and the subsequent quantum circuit. This is only supported on backends that have `backend.dynamic_reprate_enabled=True`. It must be from the range supplied by `backend.rep_delay_range`. Default is given by `backend.default_rep_delay`.
  ...
  <Attribute id="qiskit_ibm_runtime.options.ExecutionOptionsV2.init_qubits" name="init_qubits" attributeTypeHint="UnsetType | bool" attributeValue="Unset" />

  ### rep\_delay

  <Attribute id="qiskit_ibm_runtime.options.ExecutionOptionsV2.rep_delay" name="rep_delay" attributeTypeHint="UnsetType | float" attributeValue="Unset" />
