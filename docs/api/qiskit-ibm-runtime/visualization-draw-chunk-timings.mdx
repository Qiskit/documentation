...
<Function id="qiskit_ibm_runtime.visualization.draw_chunk_timings" isDedicatedPage={true} signature="draw_chunk_timings(*timings, names=None, common_start=False, normalize_y=False, line_width=4, show_legend=None, tz=None)">
  Draw one or more [`ChunkTiming`](results-chunk-timing "qiskit_ibm_runtime.results.ChunkTiming") on a bar plot.
  ...
  When comparing multiple [`ChunkTiming`](results-chunk-timing "qiskit_ibm_runtime.results.ChunkTiming") (e.g. from different jobs), use `common_start=True` to align traces at $t=0$ for direct comparison.

  <Admonition title="Note" type="note">
    For a simpler single-trace interface for data from an executor job, call [`draw()`](results-chunk-timing#draw "qiskit_ibm_runtime.results.ChunkTiming.draw") directly on `job.result().timings`.
  </Admonition>

  **Parameters**

  *   **timings** ([*ChunkTiming*](results-chunk-timing "samplomatic.quantum_program.quantum_program_result.ChunkTiming")) – One or more [`ChunkTiming`](results-chunk-timing "qiskit_ibm_runtime.results.ChunkTiming") collections.
