...
- Required: No
- Example: `{ "default_precision": 0.03, "max_execution_time": 3600, "transpilation_level": "standard"}`
...
## Outputs
The output of the QESEM function is a [PrimitiveResult](/docs/api/qiskit/qiskit.primitives.PrimitiveResult), which contains one [PubResult](/docs/api/qiskit/qiskit.primitives.PubResult) per input PUB and job-level metadata.
...
- The `metadata` field contains PUB-level metadata (`PubResult.metadata`), including the detailed QESEM results and execution metrics.
