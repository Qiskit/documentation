---
title: quimb_layers (v0.2)
description: API reference for qiskit_addon_mpf.backends.quimb_layers in qiskit-addon-mpf v0.2
...
![Output from the previous code.](/docs/images/api/qiskit-addon-mpf/0.2/qiskit_addon_mpf-backends-quimb_layers-2.avif)
...
![Output from the previous code.](/docs/images/api/qiskit-addon-mpf/0.2/qiskit_addon_mpf-backends-quimb_layers-3.avif)
...
To circumvent this problem, we can take any layer consisting of only single-qubit gates, and apply twice (once on the even and once on the odd bonds) while scaling each one with a factor of `0.5`.
