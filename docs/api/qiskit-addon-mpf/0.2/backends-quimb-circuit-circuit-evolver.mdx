...
<Class id="qiskit_addon_mpf.backends.quimb_circuit.CircuitEvolver" isDedicatedPage={true} github="https://github.com/Qiskit/qiskit-addon-mpf/tree/stable/0.2/qiskit_addon_mpf/backends/quimb_circuit/evolver.py#L24-L92" signature="CircuitEvolver(evolution_state, circuit, dt)" modifiers="class">
  ...
  *   **circuit** (*QuantumCircuit*) – the template circuit encoding the time-evolution of a single Trotter step. This circuit **must** be parameterized (see [`Parameter`](/docs/api/qiskit/qiskit.circuit.Parameter "(in Qiskit v1.3)") in place of the Trotter methods time step. This parameter must be named `dt`.
  *   **dt** ([*float*](https://docs.python.org/3/library/functions.html#float "(in Python v3.13)")) – the time step that will be used and later bound to the [`Parameter`](/docs/api/qiskit/qiskit.circuit.Parameter "(in Qiskit v1.3)") of the `circuit` object.
  ...
  <Attribute id="qiskit_addon_mpf.backends.quimb_circuit.CircuitEvolver.circuit">
    The parameterized [`QuantumCircuit`](/docs/api/qiskit/qiskit.circuit.QuantumCircuit "(in Qiskit v1.3)") describing the Trotter step.
  </Attribute>
