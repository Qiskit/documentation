---
title: JobStatus
description: API reference for qiskit.providers.JobStatus
in_page_toc_min_heading_level: 1
python_api_type: class
python_api_name: qiskit.providers.JobStatus
---

# qiskit.providers.JobStatus

<span id="qiskit.providers.JobStatus" />

`JobStatus(value)` [GitHub](https://github.com/qiskit/qiskit/tree/stable/0.17/qiskit/providers/jobstatus.py "view source code")

Class for job status enumerated type.

### \_\_init\_\_

<span id="qiskit.providers.JobStatus.__init__" />

`__init__()`

Initialize self. See help(type(self)) for accurate signature.

## Attributes

|                |   |
| -------------- | - |
| `CANCELLED`    |   |
| `DONE`         |   |
| `ERROR`        |   |
| `INITIALIZING` |   |
| `QUEUED`       |   |
| `RUNNING`      |   |
| `VALIDATING`   |   |

