...
<Class id="qiskit.transpiler.passes.SetIOLatency" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.23/qiskit/transpiler/passes/scheduling/scheduling/set_io_latency.py" signature="SetIOLatency(clbit_write_latency=0, conditional_latency=0)">
  ...
  *   **conditional\_latency** (`int`) – A control flow constraints. This value represents a latency of reading a classical register for the conditional operation. The gate operation occurs after this latency. This appears as a delay in front of the DAGOpNode of the gate. This defaults to 0 dt.
  ...
  <Function id="qiskit.transpiler.passes.SetIOLatency.name" name="name" signature="SetIOLatency.name()">
    Return the name of the pass.
  </Function>

  ### run

  <Function id="qiskit.transpiler.passes.SetIOLatency.run" name="run" signature="SetIOLatency.run(dag)">
