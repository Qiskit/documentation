...
<Class id="qiskit.algorithms.eigensolvers.NumPyEigensolver" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.23/qiskit/algorithms/eigensolvers/numpy_eigensolver.py" signature="NumPyEigensolver(k=1, filter_criterion=None)">
  ...
  *   **filter\_criterion** (*FilterType | None*) – Callable that allows to filter eigenvalues/eigenstates. Only feasible eigenstates are returned in the results. The callable has the signature `filter(eigenstate, eigenvalue, aux_values)` and must return a boolean to indicate whether to keep this value in the final returned result or not. If the number of elements that satisfies the criterion is smaller than `k`, then the returned list will have fewer elements and can even be empty.
  ...
  <Function id="qiskit.algorithms.eigensolvers.NumPyEigensolver.compute_eigenvalues" name="compute_eigenvalues" signature="NumPyEigensolver.compute_eigenvalues(operator, aux_operators=None)">
  ...
  <Function id="qiskit.algorithms.eigensolvers.NumPyEigensolver.supports_aux_operators" name="supports_aux_operators" signature="classmethod NumPyEigensolver.supports_aux_operators()">
