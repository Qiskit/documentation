...
*   Add qiskit library [`SXdgGate`](/api/qiskit/qiskit.circuit.library.SXdgGate "qiskit.circuit.library.SXdgGate") and [`CUGate`](/api/qiskit/qiskit.circuit.library.CUGate "qiskit.circuit.library.CUGate") to the supported basis gates for the Aer simulator backends. Note that the [`CUGate`](/api/qiskit/qiskit.circuit.library.CUGate "qiskit.circuit.library.CUGate") gate is only natively supported for the `statevector` and `unitary` methods. For other simulation methods it must be transpiled to the supported basis gates for that method.
...
*   The `PulseSimulator` can now take [`QuantumCircuit`](/api/qiskit/qiskit.circuit.QuantumCircuit "qiskit.circuit.QuantumCircuit") objects on the `run()`. Previously, it only would except [`Schedule`](/api/qiskit/qiskit.pulse.Schedule "qiskit.pulse.Schedule") objects as input to `run()`. When a circuit or list of circuits is passed to the simulator it will call [`schedule()`](/api/qiskit/compiler#qiskit.compiler.schedule "qiskit.compiler.schedule") to convert the circuits to a schedule before executing the circuit. For example:
