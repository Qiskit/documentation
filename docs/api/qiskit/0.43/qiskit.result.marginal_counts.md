...
`marginal_counts(result, indices=None, inplace=False, format_marginal=False, marginalize_memory=True)`[GitHub](https://github.com/qiskit/qiskit/tree/stable/0.43/qiskit/result/utils.py "view source code")

Marginalize counts from an experiment over some indices of interest.

**Parameters**

*   **result** (*dict |* [*Result*](qiskit.result.Result "qiskit.result.result.Result")) – result to be marginalized (a Result object or a dict(str, int) of counts).
...
*   **marginalize\_memory** (*bool | None*) – If True, then also marginalize the memory field (if present). If False, remove the memory field from the result. If None, leave the memory field as is.
