---
title: measure (v0.43)
description: API reference for qiskit.pulse.builder.measure in qiskit v0.43
...
<Function id="qiskit.pulse.builder.measure" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.24/qiskit/pulse/builder.py" signature="measure(qubits, registers=None)">
  ...
  *   **registers** (*List\[StorageLocation] | StorageLocation | None*) – Register to store result in. If not selected the current behavior is to return the `MemorySlot` with the same index as `qubit`. This register will be returned.
