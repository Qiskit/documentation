---
title: AskData (v0.43)
description: API reference for qiskit.algorithms.optimizers.AskData in qiskit v0.43
in_page_toc_min_heading_level: 1
python_api_type: class
python_api_name: qiskit.algorithms.optimizers.AskData
---

# AskData

<Class id="qiskit.algorithms.optimizers.AskData" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.24/qiskit/algorithms/optimizers/steppable_optimizer.py" signature="AskData(x_fun=None, x_jac=None)" modifiers="class">
  Bases: `ABC`

  Base class for return type of [`ask()`](qiskit.algorithms.optimizers.SteppableOptimizer#ask "qiskit.algorithms.optimizers.SteppableOptimizer.ask").

  **Parameters**

  *   **x\_fun** (*POINT | list\[POINT] | None*) – Point or list of points where the function needs to be evaluated to compute the next
  *   **optimizer.** (*the next state of the*) –
  *   **x\_jac** (*POINT | list\[POINT] | None*) – Point or list of points where the gradient/jacobian needs to be evaluated to compute
  *   **optimizer.** –

  ## Attributes

  ### x\_fun

  <Attribute id="qiskit.algorithms.optimizers.AskData.x_fun" attributeTypeHint="POINT | list[POINT] | None" attributeValue="None" />

  ### x\_jac

  <Attribute id="qiskit.algorithms.optimizers.AskData.x_jac" attributeTypeHint="POINT | list[POINT] | None" attributeValue="None" />
</Class>

