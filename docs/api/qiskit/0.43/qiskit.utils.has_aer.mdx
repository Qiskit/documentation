---
title: has_aer
description: API reference for qiskit.utils.has_aer
in_page_toc_min_heading_level: 1
python_api_type: function
python_api_name: qiskit.utils.has_aer
---

<span id="qiskit-utils-has-aer" />

# qiskit.utils.has\_aer

<Function id="qiskit.utils.has_aer" name="has_aer" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.24/qiskit/utils/backend_utils.py" signature="has_aer()">
  Check if Aer is installed.

  <Admonition title="Deprecated since version 0.24.0" type="danger">
    The function `qiskit.utils.backend_utils.has_aer()` is deprecated as of qiskit-terra 0.24.0. It will be removed no earlier than 3 months after the release date. For code migration guidelines, visit [https://qisk.it/qi\_migration](https://qisk.it/qi_migration).
  </Admonition>
</Function>

