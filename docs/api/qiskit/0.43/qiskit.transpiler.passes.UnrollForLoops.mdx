...
<Class id="qiskit.transpiler.passes.UnrollForLoops" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.24/qiskit/transpiler/passes/utils/unroll_forloops.py" signature="UnrollForLoops(*args, **kwargs)">
  ...
  **max\_target\_depth** (*int*) – Optional. Checks if the unrolled block is over a particular subcircuit depth. To disable the check, use `-1` (Default).
  ...
  <Function id="qiskit.transpiler.passes.UnrollForLoops.name" name="name" signature="UnrollForLoops.name()">
  ...
  <Function id="qiskit.transpiler.passes.UnrollForLoops.run" name="run" signature="UnrollForLoops.run(dag)">
