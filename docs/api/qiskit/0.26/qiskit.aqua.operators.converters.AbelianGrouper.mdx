...
<Class id="qiskit.aqua.operators.converters.AbelianGrouper" isDedicatedPage={true} github="https://github.com/qiskit-community/qiskit-aqua/tree/stable/0.9/qiskit/aqua/operators/converters/abelian_grouper.py" signature="AbelianGrouper(traverse=True)">
  ...
  **traverse** (`bool`) – Whether to convert only the Operator passed to `convert`, or traverse down that Operator.
  ...
  <Function id="qiskit.aqua.operators.converters.AbelianGrouper.group_subops" signature="classmethod group_subops(list_op, fast=None, use_nx=None)">
