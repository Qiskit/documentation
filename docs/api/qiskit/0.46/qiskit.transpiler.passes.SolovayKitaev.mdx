---
title: SolovayKitaev (v0.46)
description: API reference for qiskit.transpiler.passes.SolovayKitaev in qiskit v0.46
...
<Class id="qiskit.transpiler.passes.SolovayKitaev" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.46/qiskit/transpiler/passes/synthesis/solovay_kitaev_synthesis.py" signature="qiskit.transpiler.passes.SolovayKitaev(*args, **kwargs)" modifiers="class">
  ...
  *   **basic\_approximations** – The basic approximations for the finding the best discrete decomposition at the root of the recursion. If a string, it specifies the `.npy` file to load the approximations from. If a dictionary, it contains `{label: SO(3)-matrix}` pairs. If None, a default based on the H, T and Tdg gates up to combinations of depth 10 is generated.
  ...
  <Function id="qiskit.transpiler.passes.SolovayKitaev.execute" signature="execute(passmanager_ir, state, callback=None)">
  ...
  <Function id="qiskit.transpiler.passes.SolovayKitaev.update_status" signature="update_status(state, run_state)">
