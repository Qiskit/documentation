---
title: ValidatePulseGates (v0.46)
description: API reference for qiskit.transpiler.passes.ValidatePulseGates in qiskit v0.46
...
<Class id="qiskit.transpiler.passes.ValidatePulseGates" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.46/qiskit/transpiler/passes/scheduling/alignments/pulse_gate_validation.py" signature="qiskit.transpiler.passes.ValidatePulseGates(*args, **kwargs)" modifiers="class">
  ...
  In Qiskit SDK, we can define the pulse-level implementation of custom quantum gate instructions, as a [pulse gate](/docs/build/pulse), thus user gates should satisfy all waveform memory constraints imposed by the backend.
  ...
  *   **min\_length** – Integer number representing the minimum data point length to define the pulse gate in units of `dt`. This value depends on the control electronics of your quantum processor.
  ...
  <Function id="qiskit.transpiler.passes.ValidatePulseGates.execute" signature="execute(passmanager_ir, state, callback=None)">
  ...
  <Function id="qiskit.transpiler.passes.ValidatePulseGates.update_status" signature="update_status(state, run_state)">
