---
title: PVQD (v0.46)
description: API reference for qiskit.algorithms.PVQD in qiskit v0.46
...
<Class id="qiskit.algorithms.PVQD" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.46/qiskit/algorithms/time_evolvers/pvqd/pvqd.py" signature="qiskit.algorithms.PVQD(fidelity, ansatz, initial_parameters, estimator=None, optimizer=None, num_timesteps=None, evolution=None, use_parameter_shift=True, initial_guess=None)" modifiers="class">
  ...
  *   **num\_timesteps** ([*int*](https://docs.python.org/3/library/functions.html#int) *| None*) – The number of time steps. If `None` it will be set such that the timestep is close to 0.01.
  ...
  *   **use\_parameter\_shift** ([*bool*](https://docs.python.org/3/library/functions.html#bool)) – If True, use the parameter shift rule to compute gradients. If False, the optimizer will not be passed a gradient callable. In that case, Qiskit optimizers will use a finite difference rule to approximate the gradients.
  *   **initial\_guess** (*np.ndarray | None*) – The initial guess for the first VQE optimization. Afterwards the previous iteration result is used as initial guess. If None, this is set to a random vector with elements in the interval $[-0.01, 0.01]$.
  ...
  <Function id="qiskit.algorithms.PVQD.evolve" signature="evolve(evolution_problem)">
    ...
    *   [**ValueError**](https://docs.python.org/3/library/exceptions.html#ValueError) – If `aux_operators` provided in the time evolution problem but no estimator provided to the algorithm.
    *   [**NotImplementedError**](https://docs.python.org/3/library/exceptions.html#NotImplementedError) – If the evolution problem contains an initial state.
  ...
  <Function id="qiskit.algorithms.PVQD.get_loss" signature="get_loss(hamiltonian, ansatz, dt, current_parameters)">
    ...
    *   **dt** ([*float*](https://docs.python.org/3/library/functions.html#float)) – The time step.
    ...
    **Return type**
    ...
    [tuple](https://docs.python.org/3/library/stdtypes.html#tuple)\[Callable\[\[np.ndarray], [float](https://docs.python.org/3/library/functions.html#float)], Callable\[\[np.ndarray], np.ndarray]] | None
  </Function>
  ...
  <Function id="qiskit.algorithms.PVQD.step" signature="step(hamiltonian, ansatz, theta, dt, initial_guess)">
    ...
    **Return type**
    ...
    [tuple](https://docs.python.org/3/library/stdtypes.html#tuple)\[np.ndarray, [float](https://docs.python.org/3/library/functions.html#float)]
  </Function>
</Class>
