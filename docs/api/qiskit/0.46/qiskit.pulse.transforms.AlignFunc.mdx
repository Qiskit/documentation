---
title: AlignFunc (v0.46)
description: API reference for qiskit.pulse.transforms.AlignFunc in qiskit v0.46
...
<Class id="qiskit.pulse.transforms.AlignFunc" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.46/qiskit/pulse/transforms/alignments.py" signature="qiskit.pulse.transforms.AlignFunc(duration, func)" modifiers="class">
  ...
  *   **duration** ([*int*](https://docs.python.org/3/library/functions.html#int)  *|*[*ParameterExpression*](qiskit.circuit.ParameterExpression "qiskit.circuit.parameterexpression.ParameterExpression")) – Duration of this context. This should be larger than the schedule duration. If the specified duration is shorter than the schedule duration, no alignment is performed and the input schedule is just returned. This duration can be parametrized.
  *   **func** ([*Callable*](https://docs.python.org/3/library/typing.html#typing.Callable)) – A function that takes an index of sub-schedule and returns the fractional coordinate of of that sub-schedule. The returned value should be defined within \[0, 1]. The pulse index starts from 1.
