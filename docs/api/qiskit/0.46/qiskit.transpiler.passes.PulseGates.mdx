---
title: PulseGates (v0.46)
description: API reference for qiskit.transpiler.passes.PulseGates in qiskit v0.46
...
<Class id="qiskit.transpiler.passes.PulseGates" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.46/qiskit/transpiler/passes/calibration/pulse_gate.py" signature="qiskit.transpiler.passes.PulseGates(*args, **kwargs)" modifiers="class">
  ...
  *   **target** – The [`Target`](qiskit.transpiler.Target "qiskit.transpiler.Target") representing the target backend, if both `inst_map` and `target` are specified then it updates instructions in the `target` with `inst_map`.
  ...
  <Function id="qiskit.transpiler.passes.PulseGates.execute" signature="execute(passmanager_ir, state, callback=None)">
  ...
  <Function id="qiskit.transpiler.passes.PulseGates.update_status" signature="update_status(state, run_state)">
