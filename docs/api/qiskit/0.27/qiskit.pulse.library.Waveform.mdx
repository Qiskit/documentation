...
<Class id="qiskit.pulse.library.Waveform" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.17/qiskit/pulse/library/waveform.py" signature="Waveform(samples, name=None, epsilon=1e-07)">
  ...
  *   **epsilon** (`float`) – Pulse sample norm tolerance for clipping. If any sample’s norm exceeds unity by less than or equal to epsilon it will be clipped to unit norm. If the sample norm is greater than 1+epsilon an error will be raised.
  ...
  <Function id="qiskit.pulse.library.Waveform.__init__" name="__init__" signature="__init__(samples, name=None, epsilon=1e-07)">
    Create new sample pulse command.
