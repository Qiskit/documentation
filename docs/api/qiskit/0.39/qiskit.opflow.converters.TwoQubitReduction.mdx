...
<Class id="qiskit.opflow.converters.TwoQubitReduction" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.22/qiskit/opflow/converters/two_qubit_reduction.py" signature="TwoQubitReduction(num_particles)">
  ...
  **num\_particles** (`Union`\[`int`, `List`\[`int`], `Tuple`\[`int`, `int`]]) – number of particles, if it is a list, the first number is alpha and the second number if beta.
  ...
  <Function id="qiskit.opflow.converters.TwoQubitReduction.convert" name="convert" signature="TwoQubitReduction.convert(operator)">
