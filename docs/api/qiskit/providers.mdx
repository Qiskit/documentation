...
| [`QubitProperties`](qiskit.providers.QubitProperties "qiskit.providers.QubitProperties")(\[t1, t2, frequency]) | A representation of a `QubitProperties` object.              |
...
|                                                                                  |                                       |
| -------------------------------------------------------------------------------- | ------------------------------------- |
| [`JobStatus`](qiskit.providers.JobStatus "qiskit.providers.JobStatus")(\*values) | Class for job status enumerated type. |
...
If your backend has constraints on the allowed parameter values for any gate in the target you can model this with angle bounds on the [`Target`](qiskit.transpiler.Target "qiskit.transpiler.Target"). When you add the instruction with the [`add_instruction()`](qiskit.transpiler.Target#add_instruction "qiskit.transpiler.Target.add_instruction") you can use the `angle_bounds` keyword argument which takes a list of tuples for the upper and lower bound for the parameter of a gate.

For example, this code snippet instead of the example adding the [`PhaseGate`](qiskit.circuit.library.PhaseGate "qiskit.circuit.library.PhaseGate") in the example above:
...
will set the bounds on [`PhaseGate`](qiskit.circuit.library.PhaseGate "qiskit.circuit.library.PhaseGate") to be between 0 and $\pi$ (inclusive). This models the angle constraint in the [`Target`](qiskit.transpiler.Target "qiskit.transpiler.Target") on the angle values for the `lam` parameter on [`PhaseGate`](qiskit.circuit.library.PhaseGate "qiskit.circuit.library.PhaseGate"). The [`WrapAngles`](qiskit.transpiler.passes.WrapAngles "qiskit.transpiler.passes.WrapAngles") transpiler pass is used to transform any [`PhaseGate`](qiskit.circuit.library.PhaseGate "qiskit.circuit.library.PhaseGate") outside the specified angle bounds. You will need to write a function that takes in the angle values for the gate and returns a [`DAGCircuit`](qiskit.dagcircuit.DAGCircuit "qiskit.dagcircuit.DAGCircuit"). For example:
