...
<Class id="qiskit.algorithms.optimizers.GradientDescent" github="https://github.com/qiskit/qiskit/tree/stable/0.21/qiskit/algorithms/optimizers/gradient_descent.py" signature="GradientDescent(maxiter=100, learning_rate=0.01, tol=1e-07, callback=None, perturbation=None)">
  ...
  *   **perturbation** (`Optional`\[`float`]) – If no gradient is passed to `GradientDescent.optimize` the gradient is approximated with a symmetric finite difference scheme with `perturbation` perturbation in both directions (defaults to 1e-2 if required). Ignored if a gradient callable is passed to `GradientDescent.optimize`.
  ...
  <Function id="qiskit.algorithms.optimizers.GradientDescent.gradient_num_diff" signature="static GradientDescent.gradient_num_diff(x_center, f, epsilon, max_evals_grouped=1)">
