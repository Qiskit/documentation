...
<Class id="qiskit.algorithms.EvolutionResult" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.21/qiskit/algorithms/evolvers/evolution_result.py" signature="EvolutionResult(evolved_state, aux_ops_evaluated=None)">
  ...
  *   **aux\_ops\_evaluated** (`Union`\[`List`\[`Optional`\[`Tuple`\[`complex`, `complex`]]], `Dict`\[`str`, `Tuple`\[`complex`, `complex`]], `None`]) – Optional list of observables for which expected values on an evolved state are calculated. These values are in fact tuples formatted as (mean, standard deviation).
  ...
  <Function id="qiskit.algorithms.EvolutionResult.combine" name="combine" signature="EvolutionResult.combine(result)">
