...
<Class id="qiskit.transpiler.passes.ValidatePulseGates" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.21/qiskit/transpiler/passes/scheduling/alignments/pulse_gate_validation.py" signature="ValidatePulseGates(granularity=1, min_length=1)">
  ...
  *   **min\_length** (`int`) – Integer number representing the minimum data point length to define the pulse gate in units of `dt`. This value depends on the control electronics of your quantum processor.
  ...
  <Function id="qiskit.transpiler.passes.ValidatePulseGates.name" name="name" signature="ValidatePulseGates.name()">
    Return the name of the pass.
  </Function>

  ### run

  <Function id="qiskit.transpiler.passes.ValidatePulseGates.run" name="run" signature="ValidatePulseGates.run(dag)">
