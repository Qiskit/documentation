...
*   The fields [`QkObsTerm.bit_terms`](/docs/api/qiskit-c/2.4/qk-obs-term#QkObsTerm.bit_terms "QkObsTerm.bit_terms") and [`indices`](/docs/api/qiskit-c/2.4/qk-obs-term#QkObsTerm.indices "QkObsTerm.indices") can now be null pointers if their corresponding [`len`](/docs/api/qiskit-c/2.4/qk-obs-term#QkObsTerm.len "QkObsTerm.len") is zero.
...
*   Added a new transpiler pass, [`SynthesizeRZRotations`](/docs/api/qiskit/2.4/qiskit.transpiler.passes.SynthesizeRZRotations "qiskit.transpiler.passes.SynthesizeRZRotations"), which replaces all single-qubit [`RZGate`](/docs/api/qiskit/2.4/qiskit.circuit.library.RZGate "qiskit.circuit.library.RZGate") rotation gates with floating-point angles by equivalent Clifford+T sequences.
    ...
    ![\_images/release\_notes-1.png](/docs/images/api/qiskit/2.4/release_notes-1.avif)
...
*   Modified [`QkTargetOp`](/docs/api/qiskit-c/2.4/qk-target#QkTargetOp "QkTargetOp") to use `QkParam` as the type for its [`QkTargetOp.params`](/docs/api/qiskit-c/2.4/qk-target#QkTargetOp.params "QkTargetOp.params") attribute. Allowing users to access any free or fixed parameter in an operation.
...
*   Fixed a mismatch in [`QkTargetOp`](/docs/api/qiskit-c/2.4/qk-target#QkTargetOp "QkTargetOp") in which the length of the array stored in [`QkTargetOp.params`](/docs/api/qiskit-c/2.4/qk-target#QkTargetOp.params "QkTargetOp.params") did not match the number exposed by [`QkTargetOp.num_params`](/docs/api/qiskit-c/2.4/qk-target#QkTargetOp.num_params "QkTargetOp.num_params"). Wildcard parameters are now represented by `NAN`, and the length of the array will always be [`QkTargetOp.num_params`](/docs/api/qiskit-c/2.4/qk-target#QkTargetOp.num_params "QkTargetOp.num_params"). This bug was only present in 2.3.0rc1.
