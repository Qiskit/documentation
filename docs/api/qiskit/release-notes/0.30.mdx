...
This bugfix release fixes a few minor issues in 0.18, including a performance regression in [`assemble`](/docs/api/qiskit/0.46/compiler#qiskit.compiler.assemble "qiskit.compiler.assemble") when dealing with executing [`QuantumCircuit`](/docs/api/qiskit/0.46/qiskit.circuit.QuantumCircuit "qiskit.circuit.QuantumCircuit") objects on pulse-enabled backends.
...
*   Fixed [#7004](https://github.com/Qiskit/qiskit/issues/7004) where `AttributeError` was raised when executing [`ScheduleBlock`](/docs/api/qiskit/0.46/qiskit.pulse.ScheduleBlock "qiskit.pulse.ScheduleBlock") on a pulse backend. These blocks are now correctly treated as pulse jobs, like [`Schedule`](/docs/api/qiskit/0.46/qiskit.pulse.Schedule "qiskit.pulse.Schedule").
...
*   Update the 1-qubit gate errors in [`plot_error_map`](/docs/api/qiskit/0.46/qiskit.visualization.plot_error_map "qiskit.visualization.plot_error_map") to use the sx gate instead of the u2 gate, consistent with IBMQ backends.
...
*   Add qiskit library [`SXdgGate`](/docs/api/qiskit/0.46/qiskit.circuit.library.SXdgGate "qiskit.circuit.library.SXdgGate") and [`CUGate`](/docs/api/qiskit/0.46/qiskit.circuit.library.CUGate "qiskit.circuit.library.CUGate") to the supported basis gates for the Aer simulator backends. Note that the [`CUGate`](/docs/api/qiskit/0.46/qiskit.circuit.library.CUGate "qiskit.circuit.library.CUGate") gate is only natively supported for the `statevector` and `unitary` methods. For other simulation methods it must be transpiled to the supported basis gates for that method.
...
*   The `PulseSimulator` can now take [`QuantumCircuit`](/docs/api/qiskit/0.46/qiskit.circuit.QuantumCircuit "qiskit.circuit.QuantumCircuit") objects on the `run()`. Previously, it only would except [`Schedule`](/docs/api/qiskit/0.46/qiskit.pulse.Schedule "qiskit.pulse.Schedule") objects as input to `run()`. When a circuit or list of circuits is passed to the simulator it will call [`schedule()`](/docs/api/qiskit/0.46/compiler#qiskit.compiler.schedule "qiskit.compiler.schedule") to convert the circuits to a schedule before executing the circuit. For example:
