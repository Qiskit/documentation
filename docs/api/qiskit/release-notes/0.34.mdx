...
*   Fixes an issue with `LocalNoisePass` for noise functions that return a [`QuantumCircuit`](/docs/api/qiskit/0.46/qiskit.circuit.QuantumCircuit "qiskit.circuit.QuantumCircuit") for the noise op. These were appended to the DAG as an opaque circuit instruction that must be unrolled to be simulated. This fix composes them so that the circuit instructions are added to the new DAG and can be simulated without additional unrolling if all circuit instructions are supported by the simulator.
