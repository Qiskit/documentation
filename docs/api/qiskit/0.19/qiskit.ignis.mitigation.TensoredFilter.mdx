...
<Class id="qiskit.ignis.mitigation.TensoredFilter" isDedicatedPage={true} github="https://github.com/qiskit-community/qiskit-ignis/tree/stable/0.3/qiskit/ignis/mitigation/measurement/filters.py" signature="TensoredFilter(cal_matrices, substate_labels_list)">
  ...
  *   **substate\_labels\_list** (`list`) – for each calibration matrix a list of the states (as strings, states in the subspace)
  ...
  <Function id="qiskit.ignis.mitigation.TensoredFilter.apply" name="apply" signature="TensoredFilter.apply(raw_data, method='least_squares')">
