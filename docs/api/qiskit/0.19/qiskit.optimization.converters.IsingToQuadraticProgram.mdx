...
<Class id="qiskit.optimization.converters.IsingToQuadraticProgram" isDedicatedPage={true} github="https://github.com/qiskit-community/qiskit-aqua/tree/stable/0.7/qiskit/optimization/converters/ising_to_quadratic_program.py" signature="IsingToQuadraticProgram(linear=False)">
  ...
  **linear** (`bool`) – If linear is True, $x^2$ is treated as a linear term since $x^2 = x$ for $x \in \{0,1\}$. Else, $x^2$ is treat as a quadratic term. The default value is False.
  ...
  <Function id="qiskit.optimization.converters.IsingToQuadraticProgram.encode" name="encode" signature="IsingToQuadraticProgram.encode(qubit_op, offset=0.0)">
