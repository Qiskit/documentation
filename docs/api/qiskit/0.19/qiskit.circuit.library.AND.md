...
`AND(num_variable_qubits, flags=None, mcx_mode='noancilla')` [GitHub](https://github.com/qiskit/qiskit/tree/stable/0.14/qiskit/circuit/library/boolean_logic/quantum_and.py "view source code")
...
<span id="qiskit.circuit.library.AND.AND" />

`AND.AND(qr_variables, qb_target, qr_ancillae, flags=None, mct_mode='no-ancilla')`
...
`AND.append(instruction, qargs=None, cargs=None)`
...
`AND.assign_parameters(param_dict, inplace=False)`
...
`AND.cbit_argument_conversion(clbit_representation)`
...
`AND.ccx(control_qubit1, control_qubit2, target_qubit, *, ctl1=None, ctl2=None, tgt=None)`
...
`AND.ch(control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`AND.cnot(control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`AND.crx(theta, control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`AND.cry(theta, control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`AND.crz(theta, control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`AND.cswap(control_qubit, target_qubit1, target_qubit2, *, label=None, ctrl_state=None, ctl=None, tgt1=None, tgt2=None)`
...
`AND.cu1(theta, control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`AND.cu3(theta, phi, lam, control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`AND.cx(control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`AND.cy(control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`AND.cz(control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`AND.draw(output=None, scale=0.7, filename=None, style=None, interactive=False, line_length=None, plot_barriers=True, reverse_bits=False, justify=None, vertical_compression='medium', idle_wires=True, with_layout=True, fold=None, ax=None, initial_state=False, cregbundle=False)`
...
`AND.fredkin(control_qubit, target_qubit1, target_qubit2, *, ctl=None, tgt1=None, tgt2=None)`
...
`static AND.from_qasm_file(path)`
...
`static AND.from_qasm_str(qasm_str)`
...
`AND.iso(isometry, q_input, q_ancillas_for_output, q_ancillas_zero=None, q_ancillas_dirty=None)`
...
`AND.isometry(isometry, q_input, q_ancillas_for_output, q_ancillas_zero=None, q_ancillas_dirty=None)`
...
`AND.mcmt(gate, control_qubits, target_qubits, ancilla_qubits=None, mode='no-ancilla', *, single_control_gate_fun=None, q_controls=None, q_ancillae=None, q_targets=None)`
...
`AND.mcrx(theta, q_controls, q_target, use_basis_gates=False)`
...
`AND.mcry(theta, q_controls, q_target, q_ancillae, mode='basic', use_basis_gates=False)`
...
`AND.mcrz(lam, q_controls, q_target, use_basis_gates=False)`
...
`AND.mcx(control_qubits, target_qubit, ancilla_qubits=None, mode='noancilla')`
...
`AND.measure_active(inplace=True)`
...
`AND.num_connected_components(unitary_only=False)`
...
`AND.qasm(formatted=False, filename=None)`
...
`AND.qbit_argument_conversion(qubit_representation)`
...
`AND.rcccx(control_qubit1, control_qubit2, control_qubit3, target_qubit)`
...
`AND.rccx(control_qubit1, control_qubit2, target_qubit)`
...
`AND.remove_final_measurements(inplace=True)`
...
`AND.rx(theta, qubit, *, label=None, q=None)`
...
`AND.snapshot(label, snapshot_type='statevector', qubits=None, params=None)`
...
`AND.snapshot_density_matrix(label, qubits=None)`
...
`AND.snapshot_expectation_value(label, op, qubits, single_shot=False, variance=False)`
...
`AND.snapshot_probabilities(label, qubits, variance=False)`
...
`AND.squ(unitary_matrix, qubit, mode='ZYZ', up_to_diagonal=False, *, u=None)`
...
`AND.to_gate(parameter_map=None)`
...
`AND.to_instruction(parameter_map=None)`
...
`AND.toffoli(control_qubit1, control_qubit2, target_qubit, *, ctl1=None, ctl2=None, tgt=None)`
...
`AND.u3(theta, phi, lam, qubit, *, q=None)`
...
`AND.uc(gate_list, q_controls, q_target, up_to_diagonal=False)`
...
<span id="qiskit.circuit.library.AND.ucx" />
...
<span id="qiskit.circuit.library.AND.ucz" />
...
`AND.x(qubit, *, label=None, ctrl_state=None, q=None)`
