...
`visualize_transition(circuit, trace=False, saveas=None, fpg=100, spg=2)` [GitHub](https://github.com/qiskit/qiskit/tree/stable/0.14/qiskit/visualization/transition_visualization.py "view source code")
...
*   **fpg** (*int*) – Frames per gate. Finer control over animation smoothness and computiational needs to render the animation. Works well for tkinter GUI as it is, for jupyter GUI it might be preferable to choose fpg between 5-30.
