...
<Class id="qiskit.aqua.components.iqfts.Approximate" isDedicatedPage={true} github="https://github.com/qiskit-community/qiskit-aqua/tree/stable/0.7/qiskit/aqua/components/iqfts/approximate.py" signature="Approximate(num_qubits, degree=0)">
  ...
  *   **degree** (`int`) – The degree of approximation. 0 is the minimum value and causes no approximation so will in fact be the same as a `StandardIQFT`.
  ...
  <Function id="qiskit.aqua.components.iqfts.Approximate.construct_circuit" name="construct_circuit" signature="Approximate.construct_circuit(mode='circuit', qubits=None, circuit=None, do_swaps=True)">
