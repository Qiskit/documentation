...
`QuantumVolume(num_qubits, depth=None, seed=None, classical_permutation=True)` [GitHub](https://github.com/qiskit/qiskit/tree/stable/0.14/qiskit/circuit/library/quantum_volume.py "view source code")
...
<span id="qiskit.circuit.library.QuantumVolume.AND" />

`QuantumVolume.AND(qr_variables, qb_target, qr_ancillae, flags=None, mct_mode='no-ancilla')`
...
`QuantumVolume.append(instruction, qargs=None, cargs=None)`
...
`QuantumVolume.ch(control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`QuantumVolume.compose(other, qubits=None, clbits=None, front=False, inplace=False)`
...
`QuantumVolume.crx(theta, control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`QuantumVolume.cry(theta, control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`QuantumVolume.cswap(control_qubit, target_qubit1, target_qubit2, *, label=None, ctrl_state=None, ctl=None, tgt1=None, tgt2=None)`
...
`QuantumVolume.cu1(theta, control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`QuantumVolume.cy(control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`QuantumVolume.cz(control_qubit, target_qubit, *, label=None, ctrl_state=None, ctl=None, tgt=None)`
...
`static QuantumVolume.from_qasm_str(qasm_str)`
...
`QuantumVolume.iso(isometry, q_input, q_ancillas_for_output, q_ancillas_zero=None, q_ancillas_dirty=None)`
...
`QuantumVolume.isometry(isometry, q_input, q_ancillas_for_output, q_ancillas_zero=None, q_ancillas_dirty=None)`
...
<span id="qiskit.circuit.library.QuantumVolume.mcmt" />

`QuantumVolume.mcmt(gate, control_qubits, target_qubits, ancilla_qubits=None, mode='no-ancilla', *, single_control_gate_fun=None, q_controls=None, q_ancillae=None, q_targets=None)`
...
`QuantumVolume.mcrx(theta, q_controls, q_target, use_basis_gates=False)`
...
`QuantumVolume.mcrz(lam, q_controls, q_target, use_basis_gates=False)`
...
`QuantumVolume.mcx(control_qubits, target_qubit, ancilla_qubits=None, mode='noancilla')`
...
`QuantumVolume.measure_active(inplace=True)`
...
<span id="qiskit.circuit.library.QuantumVolume.mirror" />
...
`QuantumVolume.num_connected_components(unitary_only=False)`
...
`QuantumVolume.rcccx(control_qubit1, control_qubit2, control_qubit3, target_qubit)`
...
`QuantumVolume.remove_final_measurements(inplace=True)`
...
`QuantumVolume.rx(theta, qubit, *, label=None, q=None)`
...
`QuantumVolume.ry(theta, qubit, *, label=None, q=None)`
...
`QuantumVolume.rz(phi, qubit, *, q=None)`
...
`QuantumVolume.sdg(qubit, *, q=None)`
...
`QuantumVolume.snapshot(label, snapshot_type='statevector', qubits=None, params=None)`
...
`QuantumVolume.snapshot_density_matrix(label, qubits=None)`
...
`QuantumVolume.snapshot_probabilities(label, qubits, variance=False)`
...
`QuantumVolume.tdg(qubit, *, q=None)`
...
`QuantumVolume.u2(phi, lam, qubit, *, q=None)`
...
`QuantumVolume.u3(theta, phi, lam, qubit, *, q=None)`
...
`QuantumVolume.uc(gate_list, q_controls, q_target, up_to_diagonal=False)`
...
`QuantumVolume.ucrx(angle_list, q_controls, q_target)`
...
`QuantumVolume.x(qubit, *, label=None, ctrl_state=None, q=None)`
