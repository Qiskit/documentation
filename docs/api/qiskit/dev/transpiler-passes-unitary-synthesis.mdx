---
title: UnitarySynthesis (dev version)
description: API reference for qiskit.transpiler.passes.UnitarySynthesis in the dev version of qiskit
...
<Class id="qiskit.transpiler.passes.UnitarySynthesis" isDedicatedPage={true} github="https://github.com/Qiskit/qiskit/tree/main/qiskit/transpiler/passes/synthesis/unitary_synthesis.py#L315-L610" signature="qiskit.transpiler.passes.UnitarySynthesis(*args, **kwargs)" modifiers="class">
  ...
  *   **coupling\_map** ([*CouplingMap*](transpiler-coupling-map "qiskit.transpiler.CouplingMap")) – the coupling map of the backend in case synthesis is done on a physical circuit. The directionality of the coupling\_map will be taken into account if `pulse_optimize` is `True`/`None` and `natural_direction` is `True`/`None`.
  *   **backend\_props** ([*BackendProperties*](providers-models-backend-properties "qiskit.providers.models.BackendProperties")) – Properties of a backend to synthesize for (e.g. gate fidelities).
  ...
  *   **target** – The optional [`Target`](transpiler-target "qiskit.transpiler.Target") for the target device the pass is compiling for. If specified this will supersede the values set for `basis_gates`, `coupling_map`, and `backend_props`.
  ...
  **Raises**
  ...
  <Function id="qiskit.transpiler.passes.UnitarySynthesis.run" github="https://github.com/Qiskit/qiskit/tree/main/qiskit/transpiler/passes/synthesis/unitary_synthesis.py#L422-L527" signature="run(dag)">
    Run the UnitarySynthesis pass on `dag`.

    **Parameters**

    **dag** ([*DAGCircuit*](dagcircuit-dag-circuit "qiskit._accelerate.circuit.DAGCircuit")) – input dag.
