---
title: classical (dev version)
description: API reference for qiskit.circuit.classical in the dev version of qiskit
...
Currently, only simple expressions on bits and registers that result in a Boolean value are supported, and these are only valid for use in the conditions of [`QuantumCircuit.if_test()`](circuit-quantum-circuit#if_test "qiskit.circuit.QuantumCircuit.if_test") ([`IfElseOp`](circuit-if-else-op "qiskit.circuit.IfElseOp")) and [`QuantumCircuit.while_loop()`](circuit-quantum-circuit#while_loop "qiskit.circuit.QuantumCircuit.while_loop") ([`WhileLoopOp`](circuit-while-loop-op "qiskit.circuit.WhileLoopOp")), and in the target of [`QuantumCircuit.switch()`](circuit-quantum-circuit#switch "qiskit.circuit.QuantumCircuit.switch") ([`SwitchCaseOp`](circuit-switch-case-op "qiskit.circuit.SwitchCaseOp")).
...
<Function id="qiskit.circuit.classical.types.cast_kind" github="https://github.com/Qiskit/qiskit/tree/main/qiskit/circuit/classical/types/ordering.py#L203-L225" signature="qiskit.circuit.classical.types.cast_kind(from_, to_, /)">
