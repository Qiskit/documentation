...
<Admonition title="Warning" type="caution">
  In general, the visualization tooling in Qiskit may call system programs on arbitrary user input, and should only be used on trusted inputs. The visualization tooling provided in Qiskit is mostly intended for local visualization, and deliberately allows user code injection in several places, such as Graphviz node labels like register names in [`DAGCircuit.draw()`](qiskit.dagcircuit.DAGCircuit#draw "qiskit.dagcircuit.DAGCircuit.draw"), or LaTeX instruction labels from [`Instruction.label`](qiskit.circuit.Instruction#label "qiskit.circuit.Instruction.label") in the `latex` mode of [`QuantumCircuit.draw()`](qiskit.circuit.QuantumCircuit#draw "qiskit.circuit.QuantumCircuit.draw").
</Admonition>
