---
title: ValidatePulseGates (dev version)
description: API reference for qiskit.transpiler.passes.ValidatePulseGates in the dev version of qiskit
...
<Class id="qiskit.transpiler.passes.ValidatePulseGates" isDedicatedPage={true} github="https://github.com/Qiskit/qiskit/tree/main/qiskit/transpiler/passes/scheduling/alignments/pulse_gate_validation.py#L23-L107" signature="qiskit.transpiler.passes.ValidatePulseGates(*args, **kwargs)" modifiers="class">
  ...
  *   **target** – The [`Target`](transpiler-target "qiskit.transpiler.Target") representing the target backend, if `target` is specified then this argument will take precedence and `granularity` and `min_length` will be ignored.
  ...
  <Function id="qiskit.transpiler.passes.ValidatePulseGates.run" github="https://github.com/Qiskit/qiskit/tree/main/qiskit/transpiler/passes/scheduling/alignments/pulse_gate_validation.py#L71-L107" signature="run(dag)">
    Run the pulse gate validation attached to `dag`.

    **Parameters**

    **dag** ([*DAGCircuit*](dagcircuit-dag-circuit "qiskit._accelerate.circuit.DAGCircuit")) – DAG to be validated.
