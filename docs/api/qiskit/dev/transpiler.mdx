...
At optimization level 0, no abstract optimization is done. The default plugin simply “unrolls” operations with more than three qubits by accessing their hierarchical [`definition`](qiskit.circuit.Instruction#definition "qiskit.circuit.Instruction.definition") fields.
...
For a Clifford+T basis set, the single-qubit rotation gates are approximated using the [`UnitarySynthesis`](qiskit.transpiler.passes.UnitarySynthesis "qiskit.transpiler.passes.UnitarySynthesis") pass. By default (when `unitary_synthesis_method='default'`), this invokes the [`SolovayKitaevDecomposition`](qiskit.synthesis.SolovayKitaevDecomposition "qiskit.synthesis.SolovayKitaevDecomposition") algorithm. A custom synthesis method may be also specified, and it should either return the synthesized circuit in the Clifford+T basis set or return `None` in which case the default method would be called as fallback.
...
|                                                                                                               |                                                      |
| ------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| [`OptimizationMetric`](qiskit.transpiler.OptimizationMetric "qiskit.transpiler.OptimizationMetric")(\*values) | Optimization metric considered during transpilation. |
