---
title: evolved_operator_ansatz (dev version)
description: API reference for qiskit.circuit.library.evolved_operator_ansatz in the dev version of qiskit
...
$$
  ...
  **Examples**
  ...
  ![Circuit diagram output by the previous code.](/images/api/qiskit/dev/qiskit-circuit-library-evolved_operator_ansatz-1.avif)
  ...
  *   **evolution** ([*EvolutionSynthesis*](synthesis-evolution-synthesis "qiskit.synthesis.EvolutionSynthesis") *| None*) – A specification of which evolution synthesis to use for the [`PauliEvolutionGate`](circuit-library-pauli-evolution-gate "qiskit.circuit.library.PauliEvolutionGate"). Defaults to first order Trotterization. Note, that operators of type [`Operator`](quantum-info-operator "qiskit.quantum_info.Operator") are evolved using the [`HamiltonianGate`](circuit-library-hamiltonian-gate "qiskit.circuit.library.HamiltonianGate"), as there are no Hamiltonian terms to expand in Trotterization.
  ...
  *   **remove\_identities** ([*bool*](https://docs.python.org/3/library/functions.html#bool "(in Python v3.13)")) – If `True`, ignore identity operators (note that we do not check [`Operator`](quantum-info-operator "qiskit.quantum_info.Operator") inputs). This will also remove parameters associated with identities.
