...
<Class id="qiskit.transpiler.passes.PulseGates" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.45/qiskit/transpiler/passes/calibration/pulse_gate.py" signature="qiskit.transpiler.passes.PulseGates(*args, **kwargs)">
  ...
  *   **target** – The [`Target`](qiskit.transpiler.Target "qiskit.transpiler.Target") representing the target backend, if both `inst_map` and `target` are specified then it updates instructions in the `target` with `inst_map`.
  ...
  <Function id="qiskit.transpiler.passes.PulseGates.execute" name="execute" signature="execute(passmanager_ir, state, callback=None)">
  ...
  <Function id="qiskit.transpiler.passes.PulseGates.update_status" name="update_status" signature="update_status(state, run_state)">
