...
<Class id="qiskit.chemistry.components.bosonic_bases.HarmonicBasis" isDedicatedPage={true} github="https://github.com/qiskit-community/qiskit-aqua/tree/stable/0.9/qiskit/chemistry/components/bosonic_bases/harmonic_basis.py" signature="HarmonicBasis(watson_hamiltonian, basis, truncation_order=3)">
  ...
  *   **truncation\_order** (`int`) – where is the Hamiltonian expansion truncation (1 for having only 1-body terms, 2 for having on 1- and 2-body terms…)
  ...
  <Function id="qiskit.chemistry.components.bosonic_bases.HarmonicBasis.convert" name="convert" signature="HarmonicBasis.convert(threshold=1e-06)">
