---
title: pad (v0.29)
description: API reference for qiskit.pulse.builder.pad in qiskit v0.29
in_page_toc_min_heading_level: 1
python_api_type: function
python_api_name: qiskit.pulse.builder.pad
---

<span id="qiskit-pulse-builder-pad" />

# qiskit.pulse.builder.pad

<Function id="qiskit.pulse.builder.pad" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.18/qiskit/pulse/builder.py" signature="pad(*chs)">
  Deprecated. Pad all available timeslots with delays upon exiting context.

  **Parameters**

  **chs** (`Channel`) – Channels to pad with delays. Defaults to all channels in context if none are supplied.

  **Yields**

  None

  **Return type**

  `AbstractContextManager`\[`None`]
</Function>

