---
title: Drag (latest version)
description: API reference for qiskit.pulse.library.Drag in the latest version of qiskit
...
$$
  ...
  where $g(x)$ is a standard unlifted Gaussian waveform, $g'(x)$ is the lifted [`Gaussian`](pulse-library-gaussian "qiskit.pulse.library.Gaussian") waveform, and $\text{A} = \text{amp} \times \exp\left(i\times\text{angle}\right)$.
