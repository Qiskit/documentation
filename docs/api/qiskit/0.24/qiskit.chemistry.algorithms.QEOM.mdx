...
<Class id="qiskit.chemistry.algorithms.QEOM" isDedicatedPage={true} github="https://github.com/qiskit-community/qiskit-aqua/tree/stable/0.8/qiskit/chemistry/algorithms/excited_states_solvers/qeom.py" signature="QEOM(ground_state_solver, excitations='sd')">
  ...
  *   **excitations** (`Union`\[`str`, `List`\[`List`\[`int`]]]) – The excitations to be included in the eom pseudo-eigenvalue problem. If a string (‘s’, ‘d’ or ‘sd’) then all excitations of the given type will be used. Otherwise a list of custom excitations can directly be provided.
  ...
  <Function id="qiskit.chemistry.algorithms.QEOM.excitations" signature="property excitations">
