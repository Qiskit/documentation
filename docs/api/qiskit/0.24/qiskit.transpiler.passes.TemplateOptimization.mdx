...
<Class id="qiskit.transpiler.passes.TemplateOptimization" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.16/qiskit/transpiler/passes/optimization/template_optimization.py" signature="TemplateOptimization(*args, **kwargs)">
  ...
  *   **heuristics\_qubits\_param** (*list\[int]*) – \[length] The heuristics for the qubit choice make guesses from the dag dependency of the circuit in order to limit the number of qubit configurations to explore. The length is the number of successors or not predecessors that will be explored in the dag dependency of the circuit, each qubits of the nodes are added to the set of authorized qubits. We advice to use length=1. Check reference for more details.
  ...
  <Function id="qiskit.transpiler.passes.TemplateOptimization.is_analysis_pass" signature="property is_analysis_pass">
  ...
  <Function id="qiskit.transpiler.passes.TemplateOptimization.is_transformation_pass" signature="property is_transformation_pass">
