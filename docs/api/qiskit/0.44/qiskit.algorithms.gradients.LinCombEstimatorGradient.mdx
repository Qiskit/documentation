...
<Class id="qiskit.algorithms.gradients.LinCombEstimatorGradient" isDedicatedPage={true} github="https://github.com/qiskit/qiskit/tree/stable/0.25/qiskit/algorithms/gradients/lin_comb/lin_comb_estimator_gradient.py" signature="qiskit.algorithms.gradients.LinCombEstimatorGradient(estimator, derivative_type=DerivativeType.REAL, options=None)">
  ...
  *   **options** ([*Options*](qiskit.providers.Options "qiskit.providers.Options") *| None*) – Primitive backend runtime options used for circuit execution. The order of priority is: options in `run` method > gradient’s default options > primitive’s default setting. Higher priority setting overrides lower priority setting.
  ...
  <Function id="qiskit.algorithms.gradients.LinCombEstimatorGradient.run" name="run" signature="run(circuits, observables, parameter_values, parameters=None, **options)">
  ...
  <Function id="qiskit.algorithms.gradients.LinCombEstimatorGradient.update_default_options" name="update_default_options" signature="update_default_options(**options)">
