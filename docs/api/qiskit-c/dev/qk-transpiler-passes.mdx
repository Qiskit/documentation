...
### qk\_transpiler\_pass\_basis\_translator

<Function id="qk_transpiler_pass_basis_translator" signature="void qk_transpiler_pass_basis_translator(QkDag *dag, const QkTarget *target, size_t min_qubits)">
  Run the BasisTranslator transpiler pass on a DAG circuit.

  The BasisTranslator transpiler pass translates gates to a target basis by searching for a set of translations from the standard EquivalenceLibrary. The DAG will be mutated in-place, unless the DAG is already in the target basis, in which case the DAG remains unchanged.
  ...
  Behavior is undefined if `dag` and/or `target` are not valid, non-null pointers to a `QkDag` or `QkTarget`.

  **Parameters**

  *   **dag** – A pointer to the DAG to run BasisTranslator on.
...
<Function id="qk_transpiler_pass_remove_diagonal_gates_before_measure" signature="void qk_transpiler_pass_remove_diagonal_gates_before_measure(QkDag *dag)">
  ...
  <span id="group__QkTranspilerPasses_1autotoc_md278" />
