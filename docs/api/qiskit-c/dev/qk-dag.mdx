...
### qk\_dag\_view\_instruction

<Function id="qk_dag_view_instruction" signature="void qk_dag_view_instruction(const QkDag *dag, uint32_t index, QkCircuitInstructionView *out)">
  Write out direct views for an instruction in the circuit.

  This is a mirror of `qk_circuit_view_instruction`; consult its documentation for more detail and examples.
  ...
  *   `index` is not a valid instruction index in the circuit. An index is invalid if does not correspond to an “operation node”, i.e. calling `qk_dag_node_type(dag, index)` would be defined and return `QkDagNodeType_Operation`.
  *   `out` is misaligned or not valid for a single write.

  **Parameters**

  *   **dag** – The circuit to get the instruction from.
  *   **index** – The index of the instruction in `dag`.
  *   **out** – **\[out]** The memory location to write the result to.
</Function>
...
<Function id="qk_dag_to_python" signature="PyObject *qk_dag_to_python(QkDag *dag)">
  ...
  <span id="group__QkDag_1autotoc_md267" />
...
<Function id="qk_dag_borrow_from_python" signature="QkDag *qk_dag_borrow_from_python(PyObject *ob)">
  ...
  <span id="group__QkDag_1autotoc_md268" />
...
<Function id="qk_dag_convert_from_python" signature="int qk_dag_convert_from_python(PyObject *object, void *address)">
  ...
  <span id="group__QkDag_1autotoc_md269" />
