---
title: Transpiler Passes (v2.4)
description: API reference for Transpiler Passes in qiskit-c v2.4
...
In Qiskit, the transpiler is built up by executing as a series of passes that each perform a singular task to analyze or transform a quantum circuit. The Python [`transpiler`](/docs/api/qiskit-c/2.4/transpiler#module-qiskit.transpiler "qiskit.transpiler") documentation contains a more detailed explanation of the transpilation process.

The Qiskit C API provides transpiler pass functions in two forms: ones that operate on a `QkDag` and another set that operate on a `QkCircuit`. The DAG‑based functions, which follow the naming convention `qk_transpiler_pass_*`, accept a `QkDag` along with a `QkTarget` and any pass‑specific configuration parameters. These functions are the recommended choice when chaining multiple passes, e.g. when creating a custom transpilation pipeline, because they operate directly on the DAG object and allow it to be passed efficiently from one pass to the next within a transpilation session. By contrast, the circuit‑based functions, following the `qk_transpiler_pass_standalone_*` naming convention, operate on a `QkCircuit` and are intended for executing individual passes in isolation. While they can also be used to build custom workflows, each call incurs additional overhead because the input circuit must be converted to a DAG internally and, if a transformed circuit is returned, the resulting DAG must then be converted back to a circuit.

## DAG-based Functions

### qk\_transpiler\_pass\_elide\_permutations

<Function id="qk_transpiler_pass_elide_permutations" signature="QkTranspileLayout *qk_transpiler_pass_elide_permutations(QkDag *dag)">
  Run the ElidePermutations transpiler pass on a DAG.

  The ElidePermutations transpiler pass removes any permutation operations from a pre-layout DAG.
  ...
  *   **dag** – A pointer to the DAG to run ElidePermutations on. If there are changes made the object pointed to is changed in place. In case of gates being elided the original DAG’s allocations are freed by this function.

  **Returns**

  the layout object containing the output permutation induced by the elided gates in the DAG. If no elisions are performed this will be a null pointer and the input DAG is unchanged. The caller is responsible for freeing the returned layout by calling `qk_transpile_layout_free`.
</Function>

### qk\_transpiler\_pass\_check\_gate\_direction

<Function id="qk_transpiler_pass_check_gate_direction" signature="bool qk_transpiler_pass_check_gate_direction(const QkDag *dag, const QkTarget *target)">
  ...
  <span id="group__QkTranspilerPasses_1autotoc_md264" />
  ...
  <span id="group__QkTranspilerPasses_1autotoc_md265" />
  ...
  *   **dag** – A pointer to the DAG Circuit on which to run the CheckGateDirection pass.
...
### qk\_transpiler\_pass\_gate\_direction

<Function id="qk_transpiler_pass_gate_direction" signature="void qk_transpiler_pass_gate_direction(QkDag *dag, const QkTarget *target)">
  ...
  <span id="group__QkTranspilerPasses_1autotoc_md266" />
  ...
  <span id="group__QkTranspilerPasses_1autotoc_md267" />
  ...
  *   **dag** – A pointer to the DAG Circuit on which to run the GateDirection pass. The DAG will be modified in place by the pass.
  *   **target** – A pointer to the target used for checking gate directions.
</Function>

### qk\_transpiler\_pass\_optimize\_1q\_sequences

<Function id="qk_transpiler_pass_optimize_1q_sequences" signature="void qk_transpiler_pass_optimize_1q_sequences(QkDag *dag, const QkTarget *target)">
  ...
  *   If the original chain is an identity (chain gets removed).
  ...
  <span id="group__QkTranspilerPasses_1autotoc_md273" />
  ...
  <span id="group__QkTranspilerPasses_1autotoc_md274" />
  ...
  *   **dag** – A pointer to the `QkDag` object to transform.
...
### qk\_transpiler\_pass\_remove\_diagonal\_gates\_before\_measure

<Function id="qk_transpiler_pass_remove_diagonal_gates_before_measure" signature="void qk_transpiler_pass_remove_diagonal_gates_before_measure(QkDag *dag)">
  ...
  <span id="group__QkTranspilerPasses_1autotoc_md276" />
  ...
  <span id="group__QkTranspilerPasses_1autotoc_md277" />
  ...
  *   **dag** – A pointer to the DAG to run this pass on. The DAG will be modified in place by the pass.
</Function>

### qk\_transpiler\_pass\_remove\_identity\_equivalent

<Function id="qk_transpiler_pass_remove_identity_equivalent" signature="void qk_transpiler_pass_remove_identity_equivalent(QkDag *dag, const QkTarget *target, double approximation_degree)">
...
$$
  ...
  <span id="group__QkTranspilerPasses_1autotoc_md279" />
  ...
  <span id="group__QkTranspilerPasses_1autotoc_md280" />
  ...
  *   **dag** – A pointer to the DAG to run RemoveIdentityEquivalent on. This DAG pointed to will be updated with the modified DAG if the pass is able to remove any gates.
...
### qk\_transpiler\_pass\_split\_2q\_unitaries

<Function id="qk_transpiler_pass_split_2q_unitaries" signature="QkTranspileLayout *qk_transpiler_pass_split_2q_unitaries(QkDag *dag, double requested_fidelity, bool split_swaps)">
  Run the Split2QUnitaries transpiler pass on a DAG Circuit

  <span id="group__QkTranspilerPasses_1autotoc_md284" />
  ...
  *   **dag** – A mutable pointer to the DAG Circuit to run Split2QUnitaries on. This will be replaced with the new DAG if any gates are optimized.
  ...
  **Returns**
  ...
  If any swap equivalent unitaries are split this function returns a pointer to a `TranspileLayout` that contains the permutation induced by this DAG optimization. If no swap equivalent unitaries are split this will be a null pointer.
</Function>
...
<Function id="qk_transpiler_pass_standalone_basis_translator" signature="void qk_transpiler_pass_standalone_basis_translator(QkCircuit *circuit, const QkTarget *target, size_t min_qubits)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md253" />
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md254" />
...
<Function id="qk_transpiler_pass_standalone_commutative_cancellation" signature="QkExitCode qk_transpiler_pass_standalone_commutative_cancellation(QkCircuit *circuit, const QkTarget *target, double approximation_degree)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md255" />
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md256" />
...
<Function id="qk_transpiler_pass_standalone_consolidate_blocks" signature="void qk_transpiler_pass_standalone_consolidate_blocks(QkCircuit *circuit, const QkTarget *target, double approximation_degree, bool force_consolidate)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md257" />
...
### qk\_transpiler\_pass\_standalone\_convert\_to\_pauli\_rotations

<Function id="qk_transpiler_pass_standalone_convert_to_pauli_rotations" signature="void qk_transpiler_pass_standalone_convert_to_pauli_rotations(QkCircuit *circuit)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md258" />
  ...
  *   **circuit** – A pointer to the circuit on which to run the pass.
</Function>
...
<Function id="qk_transpiler_pass_standalone_elide_permutations" signature="QkTranspileLayout *qk_transpiler_pass_standalone_elide_permutations(QkCircuit *circuit)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md259" />
  ...
  **Returns**
  ...
  the layout object containing the output permutation induced by the elided gates in the circuit. If no elisions are performed this will be a null pointer and the input circuit is unchanged. The caller is responsible for freeing the returned layout by calling `qk_transpile_layout_free`.
</Function>
...
<Function id="qk_transpiler_pass_standalone_check_gate_direction" signature="bool qk_transpiler_pass_standalone_check_gate_direction(const QkCircuit *circuit, const QkTarget *target)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md262" />
  ...
  **Returns**
  ...
  True iff all two-qubit gate directions comply with target constraints.
</Function>
...
<Function id="qk_transpiler_pass_standalone_gate_direction" signature="void qk_transpiler_pass_standalone_gate_direction(QkCircuit *circuit, const QkTarget *target)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md263" />
  ...
  *   **circuit** – A pointer to the circuit to modify in place.
  *   **target** – A pointer to the target used for gate directions.
</Function>
...
<Function id="qk_transpiler_pass_standalone_inverse_cancellation" signature="void qk_transpiler_pass_standalone_inverse_cancellation(QkCircuit *circuit)">
  ...
  *   (QkGate\_CS, QkGate\_CSdg)
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md268" />
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md269" />
...
### qk\_transpiler\_pass\_standalone\_litinski\_transformation

<Function id="qk_transpiler_pass_standalone_litinski_transformation" signature="void qk_transpiler_pass_standalone_litinski_transformation(QkCircuit *circuit, bool fix_clifford)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md270" />
  ...
  *   **circuit** – A pointer to the circuit on which to run the pass.
  *   **fix\_clifford** – If `true`, leave the Clifford gates at the end of the circuit. If `false` they are omitted.
</Function>
...
<Function id="qk_transpiler_standalone_optimize_1q_sequences" signature="void qk_transpiler_standalone_optimize_1q_sequences(QkCircuit *circuit, const QkTarget *target)">
  ...
  <Admonition title="Deprecated since version 2.4.0" type="danger">
    use [`qk_transpiler_pass_standalone_optimize_1q_sequences()`](#qk_transpiler_pass_standalone_optimize_1q_sequences "qk_transpiler_pass_standalone_optimize_1q_sequences") instead.
  </Admonition>

  <span id="group__QkTranspilerPassesStandalone_1autotoc_md271" />
...
<Function id="qk_transpiler_pass_standalone_optimize_1q_sequences" signature="void qk_transpiler_pass_standalone_optimize_1q_sequences(QkCircuit *circuit, const QkTarget *target)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md272" />
...
<Function id="qk_transpiler_pass_standalone_remove_diagonal_gates_before_measure" signature="void qk_transpiler_pass_standalone_remove_diagonal_gates_before_measure(QkCircuit *circuit)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md275" />
...
<Function id="qk_transpiler_pass_standalone_remove_identity_equivalent" signature="void qk_transpiler_pass_standalone_remove_identity_equivalent(QkCircuit *circuit, const QkTarget *target, double approximation_degree)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md278" />
...
<Function id="qk_transpiler_pass_standalone_sabre_layout" signature="QkTranspileLayout *qk_transpiler_pass_standalone_sabre_layout(QkCircuit *circuit, const QkTarget *target, const QkSabreLayoutOptions *options)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md281" />
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md282" />
...
<Function id="qk_transpiler_pass_standalone_split_2q_unitaries" signature="QkTranspileLayout *qk_transpiler_pass_standalone_split_2q_unitaries(QkCircuit *circuit, double requested_fidelity, bool split_swaps)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md283" />
  ...
  *   **circuit** – A mutable pointer to the circuit to run Split2QUnitaries on.
  *   **requested\_fidelity** – Allowed tolerance for splitting two-qubit unitaries and gate decompositions.
  *   **split\_swaps** – Whether to attempt to split swap gates, resulting in a permutation of the qubits.
...
<Function id="qk_transpiler_pass_standalone_unitary_synthesis" signature="void qk_transpiler_pass_standalone_unitary_synthesis(QkCircuit *circuit, const QkTarget *target, size_t min_qubits, double approximation_degree)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md285" />
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md286" />
...
<Function id="qk_transpiler_pass_standalone_vf2_layout_average" signature="QkVF2LayoutResult *qk_transpiler_pass_standalone_vf2_layout_average(const QkCircuit *circuit, const QkTarget *target, const QkVF2LayoutConfiguration *config, bool strict_direction)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md298" />
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md299" />
...
<Function id="qk_transpiler_pass_standalone_vf2_layout_exact" signature="QkVF2LayoutResult *qk_transpiler_pass_standalone_vf2_layout_exact(const QkCircuit *circuit, const QkTarget *target, const QkVF2LayoutConfiguration *config)">
  ...
  *   there is no valid choice of virtual-to-physical mapping that results in an executable circuit (or at least, the pass failed to find one within its specified limits).
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md300" />
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md301" />
...
<Function id="qk_transpiler_pass_standalone_vf2_layout" signature="QkVF2LayoutResult *qk_transpiler_pass_standalone_vf2_layout(const QkCircuit *circuit, const QkTarget *target, bool strict_direction, int64_t call_limit, double time_limit, int64_t max_trials)">
  ...
  <span id="group__QkTranspilerPassesStandalone_1autotoc_md302" />
