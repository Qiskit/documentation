...
The length parameter $t$ is the number of terms in the sum and can be queried using `qk_obs_num_terms`. The parameter $s$ is the total number of non-identity single-qubit terms and can be queried using `qk_obs_len`. As illustrative examples:
...
| `qk_obs_new`      | Construct an observable from [the raw data arrays](#sparse-observable-arrays). |
