...
description: API reference for qiskit_fermions.mappers.library.jordan_wigner in the latest version of qiskit-addon-fermions
...
<Function id="qiskit_fermions.mappers.library.jordan_wigner" isDedicatedPage={true} signature="jordan_wigner(operator, num_qubits)">
  ...
  This is the type-agnostic entry point to the Jordan-Wigner transformation. It dispatches on the concrete type of `operator` to the appropriate direct implementation. Today only [`FermionOperator`](qiskit-fermions-operators-fermion-operator "qiskit_fermions.operators.FermionOperator") is supported, delegating to [`fermion_jordan_wigner()`](qiskit-fermions-mappers-library-fermion-jordan-wigner "qiskit_fermions.mappers.library.fermion_jordan_wigner"); any other operator type raises a [`TypeError`](https://docs.python.org/3/library/exceptions.html#TypeError) until a direct implementation exists for it.

  **Parameters**

  *   **operator** ([*OperatorTrait*](qiskit-fermions-operators-operator-trait "qiskit_fermions.operators.operator_trait.OperatorTrait")) – the operator to map.
