...
description: API reference for qiskit_fermions.operators.terms.group_terms_by_electronic_structure in the latest version of qiskit-addon-fermions
...
<Function id="qiskit_fermions.operators.terms.group_terms_by_electronic_structure" isDedicatedPage={true} signature="group_terms_by_electronic_structure(op, num_modes, *, two_body_physicist_order=False)">
  ...
  This function automatically populates the [`groups`](qiskit-fermions-operators-fermion-operator#groups "qiskit_fermions.operators.FermionOperator.groups") attribute of the provided [`FermionOperator`](qiskit-fermions-operators-fermion-operator "qiskit_fermions.operators.FermionOperator") such that terms satisfying a symmetric perturbation present in electronic-structure Hamiltonians are grouped.

  <Admonition title="Caution" type="note">
    The provided operator *must* be normal-ordered! This is an underlying assumption of the implementation that is *not* being verified! See [`normal_ordered()`](qiskit-fermions-operators-fermion-operator#normal_ordered "qiskit_fermions.operators.FermionOperator.normal_ordered") for how to get an operator of that form.
  </Admonition>
