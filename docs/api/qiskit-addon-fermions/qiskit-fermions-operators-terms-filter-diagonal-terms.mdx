...
description: API reference for qiskit_fermions.operators.terms.filter_diagonal_terms in the latest version of qiskit-addon-fermions
...
<Function id="qiskit_fermions.operators.terms.filter_diagonal_terms" isDedicatedPage={true} signature="filter_diagonal_terms(op)">
  ...
  This removes every term from the provided [`FermionOperator`](qiskit-fermions-operators-fermion-operator "qiskit_fermions.operators.FermionOperator") that is a product of number operators ($a^\dagger_i a_i$). This includes the constant term (a product of zero number operators), single number operators, as well as higher-order products such as $n_i n_j = a^\dagger_i a^\dagger_j a_j a_i$.
