...
<Class id="qiskit_ibm_provider.transpiler.passes.scheduling.PadDelay" isDedicatedPage={true} github="https://github.com/qiskit/qiskit-ibm-provider/tree/stable/0.8/qiskit_ibm_provider/transpiler/passes/scheduling/pad_delay.py" signature="PadDelay(fill_very_end=True, schedule_idle_qubits=False)" modifiers="class">
  ...
  *   **schedule\_idle\_qubits** (`bool`) – Set to true if you’d like a delay inserted on idle qubits. This is useful for timeline visualizations, but may cause issues for execution on large backends.
  ...
  <Function id="qiskit_ibm_provider.transpiler.passes.scheduling.PadDelay.run" signature="run(dag)">
