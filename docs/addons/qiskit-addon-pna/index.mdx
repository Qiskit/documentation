...
This package implements propagated noise absorption (PNA) [\[1\]](#ref1) for mitigating errors in the expectation value of observables.
...
This technique has been used to mitigate noise on 56-qubit Trotterized Ising models [\[1\]](#ref1).
...
It is possible to learn and efficiently characterize this gate noise as a Pauli-Lindblad model, and as shown in probabilistic error cancellation (PEC), we can mitigate the error by implementing the anti-noise, $\Lambda^{-1}$, with a QPU sampling protocol [\[2\]](#ref2). Other techniques, such as tensor-network error mitigation (TEM), implement the inverse noise channel as a classical post-processing step [\[3\]](#ref3).
