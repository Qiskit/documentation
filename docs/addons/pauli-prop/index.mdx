...
*   Lightcone shading to reduce the sampling overhead of probabilistic error cancellation (PEC) for mitigating expectation values in a 1- and 2D transverse field Ising model [\[6\]](#references) \[`qiskit-addon-slc` [docs](/docs/addons/qiskit-addon-slc)]
*   Absorbing noise model information into a target observable to mitigate expectation values in a 2D transverse field Ising model [\[7\]](#references) \[`qiskit-addon-pna` [docs](/docs/addons/qiskit-addon-pna)]
*   Trimming trailing gates to produce lower-depth Trotter circuits for the time-evolution of a 2D spin model [\[8\]](#references) \[`qiskit-addon-obp` [docs](/docs/addons/qiskit-addon-obp)]
...
*   Support for noisy simulations \[[guide](/docs/addons/pauli-prop/guides/simulate-noisy-expectation-values)]
...
*   Novel technique for approximating the conjugation of a Pauli-sum operator by another Pauli operator. This heuristic implementation greedily generates contributions to the product expected to be most significant. See Appendix B of [\[7\]](#references) for more information.
*   Single-threaded
