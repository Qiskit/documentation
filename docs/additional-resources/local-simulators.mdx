...
description: Migrate from using the IBM Quantum cloud simulators to using `qiskit-ibm-runtime` local testing mode.
---
...
<Accordion>
<AccordionItem title="Simulators have limitations">
...
<AccordionItem title="Using quantum hardware builds unique skills">
...
<AccordionItem title="Algorithms should be adapted for quantum hardware">
...
Local testing mode does not have built-in error suppression or mitigation.  Instead, you must specify those options explicitly. See [Configure error mitigation](/docs/guides/error-mitigation-and-suppression-techniques) for details.
</Admonition>
