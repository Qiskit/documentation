...
To find the maximum session timeout value for a session, follow the instructions in [Determine session details](run-jobs-in-session#determine-session-details).
