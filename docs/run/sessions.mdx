...
<video title="A line of users is shown. The one in the current session becomes larger than the rest and has rotating arrows around them, indicating that they are the priority. Boxes (jobs) are seen being sent to the QPU (represented by a chandelier) from the current user. Other users begin to pop up during the animation, and their jobs appear beneath them as they wait for the prioritized user to end their session. A progress bar pops above the prioritized user to represent the interactive TTL. Once the prioritized user finishes, the lines moves ahead toward the system, and the next person's jobs can begin processing." class="max-w-auto h-auto" controls>
