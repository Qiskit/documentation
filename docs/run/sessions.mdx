...
A session ends by reaching its maximum timeout value or when it is [closed](run-sessions#close). What happens to unfinished session jobs when the session ends depends on how it ended:
...
- When using Qiskit Runtime 13.0 or later releases:
...
- When using Qiskit Runtime releases before 13.0:
    -   Any jobs that are already running continue to run.
    -   Any queued jobs remaining in the session are put into a failed state.
    -   No further jobs can be submitted to the session.
    -   The session cannot be reopened.
...
<Admonition type="tip" title="Recommendation">
    - [Run a job in a session.](run-sessions)
