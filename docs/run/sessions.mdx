...
A session ends by reaching its maximum timeout value,  when it is [closed](run-sessions#close), or when it is cancelled by using the `session.cancel()` method. What happens to unfinished session jobs when the session ends depends on how it ended:
