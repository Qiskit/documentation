...
1. The first job in a session enters the normal queue. The job's owner become the priority user.
...
3. If the maximum timeout value is reached, the session ends and any remaining queued jobs fail.

The following video illustrates session basic workflow:
...
<Admonition type="tip" title="Recommendations">
  ...
  - Learn how to [run a job in a session.](run-jobs-in-session)
</Admonition>