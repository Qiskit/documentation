...
To find the maximum session timeout value for a session, follow the instructions in [Determine session details](#session-details).

<span id="ttl"></span>
## Interactive timeout value

Every session has an *interactive timeout value* (ITTL, or interactive time to live) of one minute for premium accounts and two seconds for open plan accounts. The ITTL cannot be configured. If there are no session jobs queued within the
...
To find the interactive timeout value for a session, follow the instructions in [Determine session details](#session-details).

<span id="active-ttl"></span>
## Active timeout value

The TTL for the active window in an extended session.

To find the active timeout value for a session, follow the instructions in [Determine session details](#session-details).


<span id="session-details"></span>
## Determine session details

You can find details about a session by using the `session.details()` method, from the [Quantum Platform Jobs page](https://quantum-computing.ibm.com/jobs), or from the IBM Cloud Jobs page, which you access from your [Instances page](https://cloud.ibm.com/quantum/instances). From the session details you can determine the [maximum](#max-ttl), [interactive](#ttl), and active time to live (TTL) values, its status, whether it's currently accepting jobs, and more. 
...
A session ends by reaching its maximum timeout value or by exiting the context manager. See [Close a session](run-sessions#close) for details. What happens when a session closes depends on how the session ended.

If the maximum timeout value was reached:
    -   Any jobs that are already running are canceled.
...
- When using Qiskit 12.3 or later releases:
    -   The session is put in `Not accepting new jobs` status.
    -   Any jobs that are already running continue to run.
    -   Any queued jobs remaining in the session continue to run.
    -   No further jobs can be submitted to the session.
    -   The session cannot be reopened.
    -   After all currently queued jobs are completed, the session is immediately closed and will not run any more jobs.

- When using Qiskit 12.2 or previous releases:
    -   Any jobs that are already running continue to run.
    -   Any queued jobs remaining in the session are put into a failed state.
    -   No further jobs can be submitted to the session.
    -   The session cannot be reopened.
