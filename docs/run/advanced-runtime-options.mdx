...
![The image shows the top-level options categories: resilience, execution, environment, and simulation.](/images/run/options.png "Option categories")
...
<Admonition type="info" title="Attention">
    This section focuses on Qiskit&reg; Runtime primitive [Options](../api/qiskit-ibm-runtime/qiskit_ibm_runtime.options.Options) (imported from `qiskit_ibm_runtime`). While most of the `primitives` interface is common across implementations, most `Options` are not. Consult the
...
Turning off all optional runtime compilation steps requires setting `optimization_level=0`.  V2 primitives do not support any advanced transpilation options. 
  </TabItem>
