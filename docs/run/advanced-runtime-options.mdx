...
When calling the primitives, you can pass in options by using an options class or when using the `run` method. In the options classes, commonly used options, such as `resilience_level`, are at the first level. Other options are grouped into different categories, such as `execution`. See all the available options in the [API reference](../api/qiskit-ibm-runtime/options).
...
### Primitive initialization (V2)

With the V2 primitives, you can specify options when initializing the primitive. 
