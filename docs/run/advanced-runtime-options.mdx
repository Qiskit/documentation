...
```python
estimator = Estimator(session=backend, options=options)
result = estimator.run(isa_circuit, isa_observable, shots=1024).result()
print(f">>> Metadata: {result.metadata[0]}")
```
...
estimator.run(isa_circuits=circuits, isa_observables=observables, shots=50)

# other logic

estimator.run(isa_circuits=circuits, isa_observables=observables, shots=100)
```
