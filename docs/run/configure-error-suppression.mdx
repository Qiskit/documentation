...
<Admonition>
  The primitives expect circuits in a form suitable to execute on the target system. You may use the Qiskit transpiler locally to translate abstract circuits into this target circuit form. For now, the primitives will attempt low-cost transformations if given a circuit that is not already in target form. In the future, primitives will error on such circuits.
