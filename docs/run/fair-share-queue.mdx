...
*   **Time used:** For every group and project, during the scheduling window, we account for all usage on all the QPU of the IBM Quantum Premium Plan. These include all successful jobs, as well as jobs returning known select errors. It does not account for canceled jobs, even when partially executed.
...
For example, assume only two groups maintain jobs in one queue. With all else being equal, a group with twice the number of shares will have twice the number jobs execute.
