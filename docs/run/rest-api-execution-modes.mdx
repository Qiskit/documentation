...
## Job mode with REST APIs

In job mode, a single primitive request of the estimator or the sampler made without a context manager. See how to run a quatum circuit using [Estimator](rest-api-primitives#estimator-primitive-with-rest-apis) and [Sampler](rest-api-primitives#sampler-primitive-with-rest-apis) for some examples. 

## Session mode with REST APIs
...
## Examples of jobs submitted in a session/batch

Once a session is setup: one or more `Estimator` jobs can be submitted to the same session by specifying the session ID. 
...
### Estimator jobs in session/batch mode
...
### Sampler jobs in session/batch mode
...
<Admonition type="tip" title="Recommendations">
    - Review detailed [Sampler and Estimator](rest-api-primitives) primitives examples using REST APIs.
