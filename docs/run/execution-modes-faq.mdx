...
<details>
  ...
  - Sessions have greater efficiency because multiple jobs from a single algorithm run can be run sequentially without interruptions.  Additionally, you can submit jobs, check results, and submit new jobs within an active session without needing to start a new one. You can use sessions for everything you can do in batch mode, such as submitting multiple jobs simultaneously to avoid drift and getting results from individual jobs. However, sessions can be more expensive to use.
