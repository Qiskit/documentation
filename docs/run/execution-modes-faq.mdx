...
The number of jobs running in parallel is based on the degree of parallelism configured for the backend, which is five for most backends.
</details>

<details>
  <summary>
    Are there reasons other than cost why I would choose session or batch mode?
  </summary>

  Yes.  Session and batch modes each have advantages.  Choose the mode that best fits your needs.

  - Sessions have greater efficiency because multiple jobs from a single algorithm run can be run sequentially without interruptions.  Additionally, you can submit jobs, check results, and submit new jobs within an active session without needing to start a new one.
  - Batch mode has minimal delay between jobs, which can help avoid drift. Also, if you partition your workload into multiple jobs and run them in batch mode, you can get results from individual jobs.
</details>
...
<details>
  <summary>
    Is compilation time counted as usage in sessions mode?
  </summary>

Yes.  In session mode, the entire time the session is active counts as usage.  The session becomes active when the first session job starts running.
</details>
...
<details>
  <summary>
    Do all of mybBatch jobs need to wait in the queue?
  </summary>
