...
IBM Quantum Network members gain reserved capacity on IBM QPUs. Usage is deducted from this capacity and hubs with lower capacity have longer queueing time.
</details>
...
<details>
  <summary>
    Can sessions be interrupted by QPU jobs?
  </summary>

No.  Sessions run in dedicated mode, which means that the user has total access to the backend.  Sessions are never interrupted by QPU jobs, such as calibrations or software upgrades.
</details>
...
- Because batch mode doesn't give you exclusive access to a backend, jobs inside a batch might run with other users' jobs or QPU calibration jobs.
...
- Your job might hit the QPU limit if it contains too many circuits or if the circuits are too large.
...
<details>
  <summary>
    Can batches be interrupted by QPU jobs?
  </summary>

Yes.  Batch workloads might be interrupted by QPU jobs, such as calibrations or software upgrades.
</details>
