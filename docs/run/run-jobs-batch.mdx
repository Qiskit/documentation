...
-  The jobs' classical computation, such as compilation, is run in parallel. Thus, running multiple jobs in a batch is significantly faster than running them serially.  Note that at most five jobs can be processed in parallel, which might include other users' jobs.
-  There is minimal delay between jobs, which can help avoid drift. However, you don't get exclusive access to the quantum system.  Therefore, if another user's job is already running on the system, your jobs have to wait.  Additionally, system calibration jobs could run between the batched jobs. 
