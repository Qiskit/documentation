...
A session automatically closes when it exits the context manager. With `qiskit-ibm-runtime` 0.13 or later releases, when the session context manager is exited, the session is put into "In progress, not accepting new jobs" status. This means that the session finishes processing all running or queued jobs until the maximum timeout value is reached. After all jobs are completed, the session is immediately canceled. This allows the scheduler to run the next job without waiting for the session interactive timeout, thereby reducing the average job queuing time. You cannot submit jobs to a closed session.
...
You can get more out of a session by running multiple workloads at once. The following example shows how you can run two VQE algorithms, each uses a different classical optimizer, simultaneously inside a single session. Job tags are also used to differentiate jobs from each workload. 
```python
