...
<Admonition>
  The primitives expect circuits in a form suitable to execute on the target system. You may use the Qiskit&reg; transpiler locally to translate abstract circuits into this target circuit form. 
...
Future versions of the Primitives will offer options to tune runtime compilation. At present, the only option with any effect is `skip_transpilation` to turn off all optimizations done with the Primitives.
...
| options.transpilation.initial\_layout(Union\[dict, List, None]) | (Deprecated) No effect                                                                                            |
| options.transpilation.layout\_method (Optional\[str])           | (Deprecated) No effect                                                                                            |
| options.transpilation.routing\_method (Optional\[str])          | (Deprecated) No effect                                                                                            |
| options.transpilation.approximation\_degree (Optional\[float])  | (Deprecated) No effect                                                                                            |
