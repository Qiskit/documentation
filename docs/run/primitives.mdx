...
Computing systems are built upon multiples layers of **abstraction**. Abstractions generalize certain
details of *how* the technology works, to allow us to focus our attention on the details that are relevant for a
specific task. The level of abstraction we choose to work with will depend on how we want to interact with the device.
The closer we get to the hardware, the lower-level we need the abstractions to be (for example, we could
want to manipulate electrical signals), and vice-versa, the more complex the task we want to perform,
the higher-level abstractions we need (for example, we could be using a programming library to perform
...
With the progress of quantum computing, we started to see the need to work at higher levels of abstraction.
...
The two most common tasks we need quantum computers for are sampling quantum states and calculating expectation values.
...
using Qiskit, we can find reference primitive implementations for local simulation in the `qiskit.primitives` module.
...
The Sampler primitive samples results from a series of measurements performed on states prepared by quantum circuits.
