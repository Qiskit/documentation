...
In this context, a _primitive_ is the smallest processing instruction, the simplest building block from which
...
* To reduce the total execution time, v2 primitives only support *lightweight transpilation*. If you need more advanced transpilation, you can use the [Qiskit transpiler](advanced-runtime-options#runtime-compilation) (manually) or the [AI-driven transpilation service](../transpile/qiskit-transpiler-service).

### Interface changes

The updated interface uses _primitive unified bloc_s (PUBs).  Each PUB is a tuple that contains a circuit and the data broadcasted to it. This greatly simplifies your ability to send complex data to a circuit.  Previously, you had to specify the same circuit multiple times to match the size of the data to be combined, as shown in the following table: 

<table style="width:100%">
  <tbody>
    <tr>
      <th style="width:50%">Estimator V1</th>
      <th style="width:50%">Estimator V2</th>
    </tr>
    <tr>
      <td>
        <ul>
          <li>Takes three parameters: circuits, observables, and parameter_values.</li>
          <li>Each parameter can be a single value or a list.</li>
          <li>The length of the parameters must match. </li>
          <li>Elements from each are aggregated.</li>
        </ul>
      </td>  
      <td>
        <ul>
          <li>Takes one parameter: PUBs</li>
          <li>Each PUB is a tuple containing the following:</li>
            <ul>
            <li>One circuit</li>
            <li>One or more observable for the circuit</li>
            <li>Optional: one or more parameter value sets for the circuit</li>
            </ul>
          <li>Elements from each are aggregated.</li>
        </ul>
      </td>
    ...
    <tr>
      <td>`estimator.run([circuit1, circuit2, ...],[observable1, observable2, ...],[param_values1, param_values2, ...] )`</td>
      <td>`estimator.run([(circuit1, observable1, param_values1),(circuit2, observable2, param_values2) ])`</td>
    </tr>
  </tbody>
</table>
...
It receives one or more _primitive unified blocs_ (PUBs) as the inputs. Each PUB is a tuple that contains a single circuit and data broadcasted to that circuit. This lets you specify a single circuit and multiple observables and parameter value sets for that circuit. It returns the computed expectation values per pair, along with their
...
<Tabs>
  <TabItem value="SamplerV2" label="SamplerV2">
    SamplerV2 samples from the classical output registers resulting from execution of quantum circuits.
It receives one or more _primitive unified blocs_ (PUBs) as the inputs. Each PUB is a tuple that contains a single circuit and data broadcasted to that circuit. This lets you specify a single circuit and multiple parameter value sets for that circuit. It returns quasi-probabilities, and optionally counts or per-shot measurements, as `PubResult`s.  Quasi-probability distributions are similar to regular
