...
It receives one or more PUBs as the inputs. It returns the computed expectation values per pair, along with their
...
It receives one or more PUBs as the inputs. This lets you specify a single circuit and multiple parameter value sets for that circuit. It returns quasi-probabilities, and optionally counts or per-shot measurements, as `PubResult`s.  Quasi-probability distributions are similar to regular
