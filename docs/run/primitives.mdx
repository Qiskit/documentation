...
* You can choose to **get counts or per-shot measurements** instead of quasi-probabilities from SamplerV2.
...
<Tabs>
  <TabItem value="SamplerV2" label="SamplerV2">
    SamplerV2 is simplified to focus on its core task of sampling the output register from execution of quantum circuits. It returns the samples, whose type is defined by the program, without weights and therefore does not support resilience levels. The result class, however, has methods to return weighted samples, such as counts and quasi-probabilities. 
    
    It receives one or more PUBs as the inputs and returns quasi-probabilities, and optionally counts or per-shot measurements, as `PubResult`s.  Quasi-probability distributions are similar to regular
...
<Admonition type="tip" title="Recommendations">
    ...
    - Read [Migrate to V2 primitives](/api/migration_guides/v2-primitives).
