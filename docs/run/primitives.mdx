...
details of *how* the technology works, to allow users to focus our attention on the details that are relevant for a
specific task. The level of abstraction you choose to work with will depend on how you want to interact with the device.
The closer you get to the hardware, the lower-level the abstractions (for example, you could
want to manipulate electrical signals), and vice versa, the more complex the task you want to perform,
the higher-level abstractions you need (for example, you could be using a programming library to perform
