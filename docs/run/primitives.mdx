...
For Estimator V2, observable and parameter values are made up of *nd-arrays*.  An nd-array is an object whose elements are indexed by a tuple of integers, where each integer must be bounded by the dimension of that axis. The tuple of dimensions, one for each axis, is called the *shape* of the nd-array. For example, a **1D** list of 10 objects (typically numbers) is a vector and has shape (10,), a matrix with five rows and two columns is **2D** and has shape (5, 2), a single number is **0D** and has an empty shape tuple (), and an **nd**-array with shape (20, 10, 5) can be interpreted as a length-20 list of 10x5 matrices.

The arrays of observables and parameter values that are input to Estimator must have shapes that are *broadcastable*. That is, the arrays must be able to be combined to create a valid result.  Note that a 0D array is compatible with any size of array, because the contents are duplicated as necessary. For example, a (6,) array and a (3,) array are not compatible because if you try to combine each element, three elements of the first array don't have corresponding elements in the second array.  Therefore, they are not a broadcastable set.  Similarly, a (4,3) array is not compatible with a (,4) array. However, a (4,3) array *is* compatible with a (,3) array. 
