...
* Takes one parameter: PUBs in the format (`<circuit>`, `<observables>`, `<parameter values>`, `<precision>`) (Note that there is only one circuit and optional `precision` value per PUB, but `parameter values` and `observables` can be lists or a single value.)
* Combines elements from multiple arrays by following NumPy broadcasting rules as described below. 
...
A1     (2d array):      2 x 1
A2     (3d array):  6 x 5 x 4 #This would work except for the middle dimensions (2 compared to 5).
```
...
parameter_values = np.random.uniform(size=(5,))  # this has shape (5,)
observables = SparsePauliOp("ZZZ")  # this has shape ()
>> pub result has shape (5,)

# Zip
parameter_values = np.random.uniform(size=(5,))  # this has shape (5,)
observables = [SparsePauliOp(pauli) for pauli in ["III", "XXX", "YYY", "ZZZ", "XYZ"]]  # this has shape (5,)
>> pub result has shape (5,)

# Outer/Product
parameter_values = np.random.uniform(size=(1, 6))  # this has shape (1, 6)
observables = [[SparsePauliOp(pauli)] for pauli in ["III", "XXX", "YYY", "ZZZ"]]  # this has shape (4, 1)
>> pub result has shape (4, 6)

# Standard nd generalization
parameter_values = np.random.uniform(size=(3, 6))  # this has shape (3, 6)
observables = [
    [[SparsePauliOp(['XII'])], [SparsePauliOp(['IXI'])], [SparsePauliOp(['IIX'])]],
    [[SparsePauliOp(['ZII'])], [SparsePauliOp(['IZI'])], [SparsePauliOp(['IIZ'])]]
]  # this has shape (2, 3, 1)
...
<admonition type="note" title="SparsePauliOp">
  Each `SparsePauliOp` counts as a single element in this context, regardless of the amount of Paulis contained in the `SparsePauliOp. So for the purpose of these broadcasting rules, all of the following elements have the same shape:
...
* Takes one parameter: PUBs in the format  (`<circuit>`, `<parameter values>`, `<shots>`) (Note that there is only one circuit per PUB and an optional value for `shots`, but `parameter values` can be a list or a single parameter)
