...
To see the instances to which you have access, look at the bottom of your [Account page](../account).

![../../../\_images/find-providers1.png](/images/migration/find-providers1.png)

## Find your instances

You can view a list of your instances on your [account settings page](../account), or you can use [the `instances()` method](../api/qiskit-ibm-runtime/qiskit_ibm_runtime.QiskitRuntimeService#instances).

## Switch instances

If you have access to run on multiple instances, the [IBM Quantum interface](../) menu bar contains a dropdown that lets you switch between instances.  The IBM Quantum Platform dashboard, Compute resources, and Jobs pages display information such as usage metrics, jobs, and systems based on the selected instance.  
...
Those using the Open plan instance have up to 10 minutes total of system execution time per month, which resets at 00:00:00 UTC on the first of each calendar month. Open plan users can track their system execution time on the [Platform dashboard,](../) [Jobs,](../jobs) and [Account](../account) pages.

<span id="connect-instance"></span>
## Connect to an instance

You can specify an instance when initializing the service or provider, or when choosing a backend.  You can copy the service-level code by clicking the three dots by the instance name on the Instances section of the [Account overview page](../account).
...
To leave an instance, visit the instance list on your [Account page.](../account) Select the instance you wish to leave, then select the overflow menu and choose *Leave instance*.
