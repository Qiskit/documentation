...
The `max_execution_time` value is based on _quantum time_, not wall clock time. Quantum time is the time spent by the QPU complex to process the job. However, simulator jobs use wall clock time because they do not have quantum time.
