...
*   save_account(): Save your account to disk for future use and optionally set it as the default.
...
If you do not set a default account, IBM Cloud is the default. Your default account can be overridden by passing in the account name or by having an account saved in an environment variable.

```python
from qiskit_ibm_runtime import QiskitRuntimeService

# Save an IBM Quantum account on disk and set it as the default.
...
When you initialize your account, the account used depends on what (if any) credentials you previously saved.  The account is chosen in this order:

1. The account specified in the `QiskitRuntimeService()` call  
1. The account saved in an environment variable
1. The account set as the default by using `save_account()`
1. Your IBM Cloud account
1. Your IBM Quantum Platform account

<Admonition type="note">
  If you send a job to a backend that the selected account cannot access, an account that can access the backend is used instead.  See the [backends page](backends) for details. 
</Admonition>
