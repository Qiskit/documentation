...
Each [processor family](processor-types) has a native gateset, and the systems in each family support running only those particular gates and operations. Transpilation translates an arbitrary circuit to the native gateset supported by a specified system.  You can view the native gates and operations for a system either [with Qiskit](#native-gates-with-qiskit) or on the IBM Quantum Platform [Compute resources page](#native-gates-on-platform).
