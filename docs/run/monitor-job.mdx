...
Call `service.job(\<job\how_toID>)` to retrieve a job you previously submitted. If you don’t have the job ID, or if you want to retrieve multiple jobs at once; including jobs from retired systems, call `service.jobs()` with optional filters instead. See [QiskitRuntimeService.jobs](https://quantum-computing.ibm.com/api/qiskit-ibm-runtime/qiskit_ibm_runtime.QiskitRuntimeService#jobs).

<Admonition type="note">
  service.jobs() returns only Qiskit Runtime jobs. To retrieve other jobs, use [qiskit-ibm-provider](https://quantum-computing.ibm.com/api/qiskit-ibm-provider/qiskit_ibm_provider.IBMBackend#ibmbackend) instead.
</Admonition>
...
For more information, see the [Qiskit tools documentation](/api/qiskit/tools) in the Qiskit Terra API reference.
