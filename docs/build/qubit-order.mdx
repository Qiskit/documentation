...
# Order of bits and qubits in Qiskit
...
- On a circuit diagram, qubit $0$ is the topmost qubit, and qubit $n$ the
  bottommost qubit (you can change this default with the `reverse_bits=True`
  argument to `QuantumCircuit.draw`; see [Changing
  ordering](#changing-ordering)).
...
- When interpreting bits as a number, bit $0$ is the least significant bit, and
  bit $n$ the most significant. This is helpful when coding because each bit
  has the value $2^\text{label}$. For example, the following circuit execution
  ends with bit $0$ being `0`, and bit $1$ being `1`. This is interpreted as
  the decimal integer `2` (measured with probability `1.0`).
...
- When displaying or interpreting a list of bits (or qubits) as a string, bit
  ...
  single-qubit states. In this case, qubit $0$ is in state $|0\rangle$, and
  qubit $1$ in state $|{+}\rangle$.
...
- When representing a state vector as a list of complex numbers (amplitudes),
