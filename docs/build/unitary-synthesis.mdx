---
title: Synthesizing unitary matrices
description: On the implementation of arbirtary unitary matrices on qubits
---

A unitary operation describes a norm-preserving change to a quantum system.
For $n$ qubits is described by a $2^n \times 2^n$ dimensional, complex matrix $U$ whose adjoint equals the inverse, that is $U^\dagger U = \mathbb{1}$.
...
Sometimes it is beneficial to re-synthesis a long series of 1- and 2-qubit gates, if the length can be reduced. For example the circuit
...
is currently using three 2-qubit gates. However, after re-synthesizing it via
...
global phase: 3.2522
        ┌────────────┐           ┌────────────┐
q_0: ───┤ U(π/2,0,0) ├─────■─────┤ U(π/2,0,0) ├───
     ┌──┴────────────┴──┐┌─┴─┐┌──┴────────────┴──┐
q_1: ┤ U(1.3496,π/2,-π) ├┤ X ├┤ U(π/2,2.9204,-π) ├
     └──────────────────┘└───┘└──────────────────┘
```

Qiskit's [transpile](../api/qiskit/qiskit.transpile) function automatically performs this re-synthesis for a sufficiently high optimization level.
