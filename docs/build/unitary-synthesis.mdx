...
Sometimes it is beneficial to re-synthesis a long series of 1- and 2-qubit gates, if the length can be reduced. For example, the following circuit uses three 2-qubit gates.

![output](/images/build/unitary-synthesis/unitary_target.png)

However, after re-synthesizing it using the following code, it only needs a single CX gate.
...
see also the [transpiler docs](../transpile/index).
