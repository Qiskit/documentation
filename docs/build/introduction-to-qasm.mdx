...
OpenQASM (open quantum assembly language), an imperative programming language for describing quantum circuits, is a platform-agnostic (or is "machine-independent" better?) programming interface for IBM Quantum systems. OpenQASM uses the measurement-based quantum circuit model to express quantum programs as ordered sequences of parameterized operations (such as gates, measurements, and resets) and real-time classical computation. OpenQASM can also describe circuits intended to characterize, validate, or debug quantum systems.
