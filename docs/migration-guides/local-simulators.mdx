...
With `qiskit-ibm-runtime` 0.22.0 or later, you can use local testing mode to replace cloud simulators. Depending on your needs, there are several ways to use local testing mode.  To
use local testing mode, specify one of the fake backends in ``qiskit_ibm_runtime.fake_provider`` or specify a Qiskit Aer backend when instantiating a primitive or a session.

### Guidance for Choosing a Simulator

If you are perpelexed by the multiple choie of simulators, feel free to refer the following table as a guide to the applicablity of the simulators. 
...
| **Circuit Size**    | Limited to the capabilities of the mimicked QPU  | Capable of handling larger circuits               | Suitable for extremely large circuits (e.g., hundreds of qubits) |
| **Results**         | Moderate runtime for QPU-specific tests          | Lesser runtime for a wide range of simulations    | Extremely fast (suitable) for stabilizer circuits  |
...
*Note: For most users, **AerSimulator** serves as a good default choice due to its flexibility and performance. However, if your work targets a specific QPU, please refer to the table 
above. For more information about the type of simulators and exmaples read below.*
