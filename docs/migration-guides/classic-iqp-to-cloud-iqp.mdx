...
This guide is for users who do not have Premium access and want to start using the updated version of IBM Quantum&reg; Platform (which runs on IBM Cloud&reg;). If you have Premium access, contact your administrator for instructions.

<Admonition type="caution" title="Important">
The [updated version of IBM Quantum Platform](https://quantum.cloud.ibm.com/) is in **early access** mode.  Therefore, you can log on, create instances, run jobs, and so on, but some functions are not available yet.  If you need to use functions that are not yet available, use the [classic IBM Quantum Platform](https://quantum.ibm.com/) or [Qiskit Runtime on IBM Cloud](https://cloud.ibm.com/quantum) instead.
</Admonition>
...
_Instances_ are virtual servers that manage your workload execution, including executing quantum programs and classical compute tasks (such as processing error mitigation).  Instances must be specified in the request header by their Cloud Resource Name (CRN).

You can see the instances you have access to on the dashboard or by clicking the Instances tab from the dashboard. Each instance is listed with its CRN identifier. You need to include this identifier in the header for most of your requests to the REST API.

## Available plans

The following plans are available to non-Premuim users:
...
Instead of accessing the [classic IBM Quantum Platform](https://quantum.ibm.com/), you will now access the [updated version of IBM Quantum Platform](https://quantum.cloud.ibm.com/), which is integrated with IBM Cloud.  Because of this integration, all users need to follow the onboarding instructions before using the new platform.
...
2. Create an instance.
   1. Log on to the new [IBM Quantum Platform.](quantum.cloud.ibm.com) Choose **IBMid** and enter your IBM Cloud account information. The dashboard opens.
   1. If you do not already have an instance, Click **Create instance** on the dashboard page.
      <Admonition type="note">
      If you have already created an instance, go to your account [administration page](https://quantum.cloud.ibm.com/admin/instances) to create new ones.
      </Admonition>
...
1. Create an API key from the [IBM Quantum Platform dashboard.](https://quantum.cloud.ibm.com/)
   <Admonition type="note">
   This API key is only visible for a short amount of time so be sure to store it in a safe place.  To manage your API key, including copying it again, you need to use IBM Cloud.  See [Managing user API keys](https://cloud.ibm.com/docs/account?topic=account-userapikey) for instructions.
   </Admonition>
...
1. <span id="untrusted-machine"></span>**Avoid executing code on an untrusted machine or an external cloud Python environment to minimize security risks.** If you must use an untrusted environment (for example, a public computer), change your API key after each use by creating a new one on the [IBM Quantum Platform dashboard](https://quantum.cloud.ibm.com/) to reduce risk. To initialize the service in this situation, you can use code like the following:
    ...
    <Admonition type="caution">
      ...
      If you accidentally share your token with someone or include it in version control like Git, immediately revoke your token by creating a new one on the [IBM Quantum Platform dashboard](https://quantum.cloud.ibm.com/) to reduce risk.
    </Admonition>
