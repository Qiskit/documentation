...
* When activating the service, an instance is required, and the channel must be set to `ibm_cloud` or `ibm_quantum_platform` (The `ibm_cloud` channel will be deprecated and removed in a future release). The instance is specified by using its cloud resource name (CRN).
...
You can see the instances you have access to on the dashboard or by clicking the Instances tab from the dashboard. Each instance is listed with its CRN identifier. You can include this identifier when connecting to Qiskit Runtime via the `QiskitRuntimeService` object when using `qiskit-ibm-runtime` in a Python environment. When an instance is passed in, only backends and jobs from the instance passed in will be available. Alternatively, if an instance is not included, then all backends and jobs across all instances in your account will be available. In this case, when a backend is selected, an instance with the backend availble will be selected. 
