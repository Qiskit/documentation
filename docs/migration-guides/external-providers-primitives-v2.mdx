...
The existing collection of migration guides helps users [transition to the runtime provider](/migration-guides/qiskit-runtime) and
...
as long as the final inputs and outputs conform to the established standards from the primitive interfaces.
...
wrapper might not be the optimal route for implementing the primtives. Instead, you should implement a particular
...
The mechanics to implement a custom Estimator are analogous to the Sampler, but might require a different pre or post-processing
