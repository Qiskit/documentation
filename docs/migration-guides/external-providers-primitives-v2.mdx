...
evolved to the updated V2 primitives (link to v2 primitives guide?), the `backend.run` model was finally deprecated in favor of
these newer interfaces to streamline quantum workload execution.

The existing collection of migration guides (link relevant guides) allow users to transition to the runtime provider and
update their user code to the new primitives V2 interface in time for the upcoming removal of `backend.run`.
To complement that, this guide focuses on supporting external providers (both existing and upcoming)
in the implementation their own flavor of the primitives V2 interface (Sampler or Estimator) to integrate seamlessly into the
