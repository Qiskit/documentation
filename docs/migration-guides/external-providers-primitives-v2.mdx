...
After a period of overlap between `backend.run` and primitives, during which users have gradually transitioned to the 
primitives access model, the `qiskit-ibm-runtime` provider has discontinued access to IBM&reg; QPUs through `backend.run`.
...
Once you know which primitive to implement, review the PUB data model for input and output handling. See the following 
