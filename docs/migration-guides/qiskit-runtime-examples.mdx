...
Both `SamplerV2` and `backend.run()` take in circuits as inputs. The main
difference is the format of the output: `backend.run()` outputs
**counts**, while `SamplerV2` returns **per-shot measurements** (but has convenience methods to also return counts).
