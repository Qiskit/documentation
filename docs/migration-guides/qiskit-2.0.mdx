...
Qiskit 2.0 is the next step within the Qiskit SDK's newly stabilized lifecycle. 
...
The [`Delay`](/api/qiskit/qiskit.circuit#delay) instruction still contains these fields, as does the new-in-2.0 [`BoxOp`](/api/qiskit/qiskit.circuit#box), because these instructions have a _variable_ delay.
