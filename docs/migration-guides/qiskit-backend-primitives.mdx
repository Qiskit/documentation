...
`Estimator` primitives, you can find guidance [here](link).  
...
2. the use of `options`, which cointains one more level of nesting compared to provider-specific primitives and
...
If the use of the third-party backend requries a more fine-grained option handling
