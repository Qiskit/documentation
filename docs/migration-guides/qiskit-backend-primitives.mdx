...
The `backend.run()` interface, previously the primary access point for IBM&reg; QPUs, has been superseded 
...
1.  The `backend` input passed during the "backend primitive" instantiation. This input argument allows to 
...
In the first level of nesting, `BackendSamplerV2` includes `default_shots`, `seed_simulator` and `run_options`;
...
The following is an example usage of the [`BackendEstimatorV2`](/api/qiskit/qiskit.primitives.BackendEstimatorV2) 
