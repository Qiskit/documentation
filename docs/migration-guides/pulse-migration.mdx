...
While the `qiskit.pulse` module contained more functionality than executing single- and two-qubit rotations more efficiently, much of the control over device physics can be modeled using the [Qiskit Dynamics](https://qiskit-community.github.io/qiskit-dynamics/) package found in the Qiskit ecosystem. The Qiskit Dynamics package documentation includes a few helpful tutorials:
