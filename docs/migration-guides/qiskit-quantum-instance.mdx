...
| --- | --- |
| [`qiskit.utils.QuantumInstance.execute`](/api/qiskit/0.46/qiskit.utils.QuantumInstance#execute) | [`qiskit.primitives.Sampler.run`](/api/qiskit/primitives-sampler#run) or [`qiskit.primitives.Estimator.run`](/api/qiskit/primitives-estimator#run) |
| [`qiskit.utils.QuantumInstance.transpile`](/api/qiskit/0.46/qiskit.utils.QuantumInstance#transpile) | [`qiskit.compiler.transpile`](/api/qiskit/compiler#qiskit.compiler.transpile) or [`qiskit.transpiler.pass_manager.PassManager.run`](/api/qiskit/transpiler-pass-manager#run)|
...
<Admonition type="caution">
  **Background on the Qiskit primitives**
  ...
  Qiskit provides reference implementations in [`qiskit.primitives.Sampler`](/api/qiskit/primitives-sampler) and [`qiskit.primitives.Estimator`](/api/qiskit/primitives-estimator). Additionally,
  [`qiskit.primitives.BackendSampler`](/api/qiskit/primitives-backend-sampler) and [`qiskit.primitives.BackendEstimator`](/api/qiskit/primitives-backend-estimator) are
  ...
  - *Primitives* - Any Sampler or Estimator implementation using base classes [`qiskit.primitives.BackendSampler`](/api/qiskit/primitives-backend-sampler) and a [`qiskit.primitives.BackendEstimator`](/api/qiskit/primitives-backend-estimator).
  - *Reference primitives* -  [`qiskit.primitives.Sampler`](/api/qiskit/primitives-sampler) and [`qiskit.primitives.Estimator`](/api/qiskit/primitives-estimator) are reference implementations that come with Qiskit.
  ...
  - *`Backend` primitives* - Instances of [`qiskit.primitives.BackendSampler`](/api/qiskit/primitives-backend-sampler) and [`qiskit.primitives.BackendEstimator`](/api/qiskit/primitives-backend-estimator). These allow any processor to implement primitive interfaces.
...
return a [`qiskit.result.QuasiDistribution`](/api/qiskit/result-quasi-distribution) object with the resulting quasi-probability distribution.
...
Basic simulation implemented using the [`qiskit.quantum_info`](/api/qiskit/quantum-info) module. If shots are
...
<Admonition type="note">
  The [`qiskit.result.QuasiDistribution`](/api/qiskit/result-quasi-distribution) class that is returned as part of the [`qiskit.primitives.SamplerResult`](/api/qiskit/primitives-sampler-result)
  exposes two methods to convert the result keys from integer to binary strings / hexadecimal:

    - [`qiskit.result.QuasiDistribution.binary_probabilities`](/api/qiskit/result-quasi-distribution#binary_probabilities)
    - [`qiskit.result.QuasiDistribution.hex_probabilities`](/api/qiskit/result-quasi-distribution#hex_probabilities)
</Admonition>
...
* The only primitives that accept a custom **bound** transpiler pass manager are instances of [`qiskit.primitives.BackendSampler`](/api/qiskit/primitives-backend-sampler) or [`qiskit.primitives.BackendEstimator`](/api/qiskit/primitives-backend-estimator).
...
[`qiskit.primitives.BackendSampler`](/api/qiskit/primitives-backend-sampler) or [`qiskit.primitives.BackendEstimator`](/api/qiskit/primitives-backend-estimator), you do not have to manually assign parameters as expected in the QuantumInstance workflow.
