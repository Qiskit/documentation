...
- If you provide a `name` and an `instance` when initializing a service, the service tries to load the saved details from `name` and connect to the specified instance. If there is any conflict, a warning is raised.
    ...
    ```
- The service tries to load the default saved account only if you don't provide a `token` or `name` when initializing a service.  However, if an `instance` is provided explicitly, the service try to connect to that instance by using the default credentials. If there is any conflict, a warning is raised.
