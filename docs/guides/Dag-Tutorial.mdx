...
In Qiskit, circuits are represented internally using a Directed Acyclic Graph (DAG). The advantage of this representation over a pure list of gates (that is, netlist) is that the flow of information between operations is explicit, making it easier for passes to make transformation decisions without changing the semantics of the circuit.

This guide demonstrates how to work with DAGs and use them to write custom transpiler passes. You'll start by building a simple circuit and examining its DAG representation, then explore basic DAG operations and implement a custom BasicMapper pass.

## Build a circuit and examine its DAG

Start by building a simple circuit and examining its DAG representation:
...
This section demonstrates common operations with DAGs, including accessing nodes, adding operations, and substituting subcircuits. These operations form the foundation for building custom transpiler passes.
...
dag_drawer(dag)
```
After all transformations are completed, the DAG can be converted back into a regular ```QuantumCircuit object```. This is how the transpiler operates. A circuit is taken, processed in DAG form, and a transformed circuit is produced as output.
