---
title: Migrate to the IBM Quantum V2 primitives
...
# Migrate to the IBM Quantum V2 primitives
...
With the deprecation of the V1 primitives, all code should be migrated to use the V2 interfaces. This guide describes what changed in the IBM Quantum V2 primitives (available with qiskit-ibm-runtime 0.21.0) and why, describes each new primitive in detail, and gives examples to help you migrate code from using the legacy primitives to the V2 primitives.  The examples in the guide all use the IBM Quantum primitives, but in general, the same changes apply to the other primitive implementations.  The functions unique to the IBM Quantum primitives such as error mitigation remain unique to the IBM Quantum primitives.
...
```
  ...
  <TabItem value="SamplerOut" label="Convert V2 output to V1 format">
    The V1 output format was a dictionary of bitstrings (as an int) as the key and quasi-probabilities as the value for each circuit. The V2 format has the same key (but as a string) and counts as the value. To convert the V2 format to V1, divide the counts by the number of shots, where the number of shots selected is described in the [Sampler options](/docs/guides/sampler-options#shots) guide.
...
5. Update Estimator's `run()` method to pass in the list of PUBs. For example, `run([(circuit1, observable1, parameter_set1)])`.
...
5. Update Sampler's `run()` method to pass in the list of PUBs. For example, `run([(circuit1, parameter_set1)])`.
...
<Admonition type="tip" title="Recommendations">
    - Learn more about setting options in the [Sampler options](/docs/guides/sampler-options), [Estimator options](/docs/guides/estimator-options), and [Executor options](/docs/guides/executor-options) guides.
    - Learn more details about [Primitive inputs and outputs](/docs/guides/primitive-input-output#pubs).
