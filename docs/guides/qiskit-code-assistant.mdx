...
Qiskit Code Assistant aims to make quantum computing more accessible to new Qiskit adopters and to improve the coding experience for current users. It is a generative AI code assistant powered by [watsonx](https://www.ibm.com/products/watsonx-ai). It is trained using millions of text tokens from Qiskit SDK v1.x, years of Qiskit code examples and IBM Quantum&trade; features. Qiskit Code Assistant can help your quantum development workflow by offering LLM-generated suggestions based on [Mistral-Small-3.2-24B-Qiskit ](https://huggingface.co/Qiskit/mistral-small-3.2-24b-qiskit) which incorporate the latest features and functionalities from IBM&reg;.
...
* Accelerates Qiskit code generation by leveraging generative AI based on the `mistral-small-3.2-24b-qiskit` model.
...
To provide code suggestions, Qiskit Code Assistant uses a Large Language Model (LLM). In this case, Qiskit Code Assistant relies on the model `mistral-small-3.2-24b-qiskit`, built on the [Mistral-Small-3.2-24B-Instruct-2506 model.](https://huggingface.co/mistralai/Mistral-Small-3.2-24B-Instruct-2506) The `mistral-small-3.2-24b-qiskit` model improves the `Mistral-Small-3.2-24B-Instruct-2506` model's code generation capabilities for Qiskit through extended pretraining and fine-tuning it on high-quality Qiskit data, as well as Python commits and chat. For more information about the Mistral AI models family, refer to [Mistral AI documentation.](https://docs.mistral.ai/getting-started/models) For more details about the `.*-qiskit` models, see [Qiskit Code Assistant: Training LLMs for generating Quantum Computing Code.](https://arxiv.org/abs/2405.19495)

The model `mistral-small-3.2-24b-qiskit` model is open source and available [here.](https://huggingface.co/Qiskit/mistral-small-3.2-24b-qiskit)

## The Qiskit HumanEval benchmark

To test the `mistral-small-3.2-24b-qiskit` model, we created, in collaboration with Qiskit Advocates and experts, and ran the execution-based benchmark called Qiskit HumanEval (QHE). This benchmark is similar to [HumanEval](https://arxiv.org/abs/2107.03374), including multiple challenging code problems to solve, all based on the official Qiskit libraries.
