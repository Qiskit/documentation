...
Keep up with the latest and greatest from Qiskit and IBM Quantum&reg;! Gathered here are the most recent Qiskit package release summaries, documentation updates, blogs, community events, and more.

##  IBM Quantum Platform

### The next evolution of IBM Quantum Platform: How to prepare for the transition

We are excited to announce a major evolution coming to IBM Quantum Platform, one that will bring a host of new features and provide Open Plan users with access to our state-of-the-art Heron processor.

We are upgrading the Platform as part of our effort to deliver an enterprise-grade cloud experience that leverages new backend infrastructure for enhanced features, increased performance, and the latest data privacy and security measures. These changes lay a foundation to support the scaling of our services alongside the [IBM Quantum Roadmap,](https://www.ibm.com/quantum/technology#roadmap) and to enable the increasingly complex workloads that are required to achieve quantum advantage. 
...
**Highlights from the 0.37.0 (13 March 2025) version release**
...
qiskit-ibm-runtime is now compatible with Qiskit 2.0. This means that classes and methods removed in Qiskit 2.0 have also been removed or handled accordingly.  The `pulse` and `channel` related changes are highlighted below. See the [Pulse migration guide](/migration-guides/pulse-migration) for details.
...
See the full [0.37.0 release notes](/api/qiskit-ibm-runtime/release-notes#00370-2025-03-13).
...
We’re pleased to share that the beta release of the Qiskit Transpiler Service is now available to all IBM Quantum&reg; Premium Plan users.
...
Early Access release of the [new IBM Quantum Platform](https://quantum.cloud.ibm.com/)

In this release, Open and Pay-As-You-Go users can begin the onboarding experience, update their existing code to use the new IBM Cloud channel, and explore the platform. Refer to the [migration guide](/migration-guides/classic-iqp-to-cloud-iqp) for help.

As an incentive to migrate, all users, including Open Plan, can access an IBM Quantum Heron QPU (IBM Torino) when using the new IBM Quantum Platform.

<Admonition type="note">
Premium Plan, Dedicated service, and Startup program users will receive guided migration support through their administrators in the coming months. In the meantime, they can create a free Open plan account to explore the new platform.
</Admonition>

Read more details in the [blog.](https://www.ibm.com/quantum/blog/iqp-upgrade)

### 2 February 2025

A new version of [dynamic circuits](/guides/classical-feedforward-and-control-flow) will start rolling out on 2 June 2025. This new version lets you run dynamic circuits at utility scale and comes with improvements in speed, quality, and usability that will gradually be added throughout the year.  Improvements include the following:

- Using the Gen3 engine stack that gives a 75x speedup in execution
- Executing branches in parallel to reduce circuit duration and improves result quality
- Improving mid circuit measurements
- Improving integration with dynamical decoupling
- Improving circuit timing information visibility

To accelerate their roll-out, the following capabilities are now deprecated and will be no longer supported on or around 2 June 2025:

- The while, for, and switch control flow constructs
- The ability to use control flow instructions inside the body of a branch (that is, nested control flow)
...
## IBM Quantum blog: Two new blogs in January
...
The first IBM Quantum blog post of the year takes a look back at some of the amazing tools released in 2024 that help make quantum software development easier and more accessible than ever before.
The blog provides recaps of major releases from last year, including Qiskit v1.x, Qiskit addons, Qiskit Transpiler Service, and more - all with code examples and documentation links to help readers get started using them.

*Blog: [A year of simplifying quantum software development tools with Qiskit](https://www.ibm.com/quantum/blog/dev-tools)*
...
*Blog: [Using dynamic circuits to efficiently implement quantum states with long-range entanglement](https://www.ibm.com/quantum/blog/long-range-entanglement)*
