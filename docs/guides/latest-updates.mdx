...
- Updated navigation to remove duplicated pages:
  - The Administration section has been renamed "Access management" and focuses on managing users and access groups.
  - There is only one page each for managing instances and workloads; there is no longer a separate page for administrators.  Instead, what you can view and manage on each page is dependent on your access and permissions.
...
- Launched two new QPUs: ibm_aachen (eu-de) and ibm_kingston (us-east)
- Individuals who can view the analytics page can now filter the data to view information per plan.
- Administrators can easily add multiple account members to an access group.
