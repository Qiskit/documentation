...
- Instance limits: Individuals can view [available time and usage](https://quantum.cloud.ibm.com/instances), which displays alerts when approaching account or instance limits.
- Admin permissions: Admins can view overall usage, cycle period, and remaining time per plan in the instances page.
- Search: Search is now integrated and available for filtering documentation content including guides and API reference.
- Billing: Standard/Pay-as-you-go users can find a link to billing by clicking the three dots on each instance on the [Instances](https://quantum.cloud.ibm.com/instances) page.
- New sign-in option: Sign-in with Google now available on the [sign-in page](https://quantum.cloud.ibm.com/signin).
- Bug fixes:
  - Addressed failed batch sessions for Open Plan users
  - Addressed batch sessions not closing properly
  - Improved platform stability due to proper error handling and better recovery
  - Addressed an issue that was preventing QPU metrics to be displayed for the QPUs in the eu-de region
...
- The latest Qiskit version, 1.4.0, has been released on GitHub and is available on [PyPi](https://pypi.org/project/qiskit/). Even if you recently upgraded to v1.3.3, you should still upgrade to Qiskit v1.4, because this is the last minor version of the v1.x series, and contains all of the deprecation warnings for API changes in Qiskit v2.0. Install it now, and see what to change to be ready for 2.0!
  ...
  Wondering what will change? Read the [changelog](https://github.com/Qiskit/qiskit/releases/tag/1.4.0) and the [v1.4 release notes.](/docs/api/qiskit/release-notes/1.4)
...
- Deprecation
  ...
  - The `IBMBackend` attributes `max_shots` and `max_experiments`, as well as the property `max_circuits`, have all been deprecated. Previously, these attribute values represented the maximum number of shots and circuits that could be submitted in one job. See [job limits](/docs/guides/job-limits#job-limits) for details.

- New features

  - `qiskit-ibm-runtime` is now compatible with Qiskit v2.0. This means that classes and methods removed in Qiskit v2.0 have also been removed or handled accordingly.  The `pulse`- and `channel`-related changes are highlighted in the following list. See the [Pulse migration guide](/docs/migration-guides/pulse-migration) for details.
...
- See the full [0.37.0 release notes.](/docs/api/qiskit-ibm-runtime/release-notes#0370-2025-03-13)
