...
- Updated navigation to remove duplicated pages:
  - The Administration section has been paired down to focus on the management of users and access groups, and is now called “Access management”.
  - There is now only one page each for managing instances and workloads, and what you can view and manage on each page is dependent on your access and permissions.
  - The analytics page for account administrators has been moved into the main navigation menu, to make it easier to find.
- Enhancements in the access management section:
  - Administrators can now add or remove multiple users in your account at once from an access group
- Launched two new QPUs: ibm_aachen and ibm_kingston
- Individuals with access to the analytics page can now filter the data to view info per plan (this is not yet there, but it will be soon)
- Administrators of Premium plan instances can now “set as limit” the instance allocation, so that usage in those instances cannot exceed the time alloted.
