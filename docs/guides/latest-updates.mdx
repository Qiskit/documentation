...
New pages include documentation for [Qiskit Functions](/guides/functions), [Qiskit addons](/guides/addons), and [Qiskit Code Assistant](/guides/qiskit-code-assistant), as well as a guide on [job limits](/guides/job-limits). 

Qiskit Functions guides:

- [Introduction](/guides/functions)
- Circuit functions
...
Qiskit Code Assistant guides:

- [Overview](/guides/qiskit-code-assistant)
...
- Added content to the [Execution modes](/guides/execution-modes) documentation guides and improved explanatory content in the[Introduction to primitives](/guides/primitives) page

- Clarity around how the [Estimator primitive calculates error](/guides/primitive-input-output#how-the-estimator-calculates-error)
...
- Information on [benchmarking and the Benchpress package](/guides#benchmarking-and-the-benchpress-package)

- The [Introduction to Qiskit](/guides#introduction-to-qiskit) page is now more navigable with the addition of a right-hand nav 
...
Updates to the infrastructure and design of the docs include the following:

- We fixed GitHub source code links for functions using decorators - thanks to the external contributor for opening an issue for this!
- When you click on "Go to the latest version" in the API reference docs, you arrive at the equivalent page in the latest version - and if the page does not exist in the new version, you land on the index page. We also fixed broken links in historical api refs
- We improved the design and color of the warning banner for when you access outdated API pages to make it easier to realize that newer docs are available - and the banner persists as you scroll down the page
- Source code links and release notes have been added to the [Qiskit Transpiler Service Client API docs](/api/qiskit-ibm-transpiler)
...
- Tips on how to use the Qiskit Code Assistant now appear throughout the guides
- Narrow tables no longer take up extra wasted space
