...
The a marking of "support" in the two columns means:
...
| arithmetic expressions        |                                      | 🟡         | 🟡                 | [4](#4), [10](#10) |
| comparisons                   |                                      | ✅         | ✅                 | [4](#4)     |
...
8. Qiskit SDK can represent structured control flow and export this to OpenQASM 3.  The `continue`
   ...
   within Qiskit SDK, and are not eligible for execution on hardware.  `for` loops in Qiskit v2.1.0
   are not well supported.  `while` loops are not eligible for execution on hardware.  Nested control
   flow (such as an `if` inside another `if`, or an `else if` statement) is not eligble for execution
   on hardware.
...
<span id="10"></span>
10. Qiskit Runtime supports arithmetic expressions that can be computed at run
    time. It does not support arithmetic expressions computed during program execution.
