...
1. <span id="save-account"></span>Authenticate to the service by calling `QiskitRuntimeService` with your IBM Quantum API token and CRN. [Set up an environment variable](#securely-manage) rather than copying and pasting your API token into your code.

   ```python
   from qiskit_ibm_runtime import QiskitRuntimeService

   service = QiskitRuntimeService(channel="ibm_quantum", token=<your-environment-variable>)

   ```

   Or, optionally use the `save_account()` method to save your credentials for easy access later on, before initializing the service. *Note that account credentials are saved in plain text. **Never include your token in a notebook.** Instead, use an environment variable when using `save_account()`; see the [Securely manage your token](#securely-manage) section for more information.*
    ...
    QiskitRuntimeService.save_account(
        channel="ibm_quantum",
        token=<your-environment-variable>,
        set_as_default=True,
    ...
    * The `channel` parameter allows you to distinguish between different account types. When initializing the account, IBM Cloud is the default account used if have saved credentials for an IBM Quantum Platform and an IBM Cloud account.
    ...
    <Admonition type="caution">
      Account credentials are saved in plain text. To more securely manage your token, it is recommended that you use an environment variable; see the [Securely manage your token](#securely-manage) section for more information.
    </Admonition>
...
1. <span id="cloud-save"></span>Authenticate to the service by calling `QiskitRuntimeService` with your saved credentials or with your IBM Cloud API key and CRN. [Set up an environment variable](#securely-manage) rather than copying and pasting your API token into your code.

   ```python
   from qiskit_ibm_runtime import QiskitRuntimeService

   service = QiskitRuntimeService(channel="ibm_cloud", token=<your-environment-variable>, instance="<IBM Cloud CRN>")
   ```

   You can optionally use the `save_account()` method to save your credentials for easy access later on, before initializing the service. *Note that account credentials are saved in plain text. **Never include your token in a notebook.** Instead, use an environment variable when using `save_account()`; see the [Securely manage your token](#securely-manage) section for more information.* 
    ...
    * If you don't save your credentials, you must specify them every time you start a new session.
    ...
    <Admonition type="caution">
      *Note that account credentials are saved in plain text. **Never include your token in a notebook.** Instead, use an environment variable when using `save_account()`; see the [Securely manage your token](#securely-manage) section for more information.*
    </Admonition>
...
<Tabs>
  ...
  <TabItem value="runtime" label="Within Qiskit Runtime">
  ```python
  import os
  from qiskit_ibm_runtime import QiskitRuntimeService

  # Gather the token from your environment variables.
  # We use `.pop()` to ensure any subprocesses started by
  # our program cannot access the value.
  iqp_api_token = os.environ.pop("IQP_API_TOKEN")

  QiskitRuntimeService.save_account(token=iqp_api_token)
  ```
  </TabItem>

  <TabItem value="functions" label="Within Qiskit Functions">
  ```python
  import os
