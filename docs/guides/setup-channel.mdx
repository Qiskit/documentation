...
1. Retrieve your API token from the [IBM Quantum account page](https://quantum.ibm.com/account) and authenticate to the service. Remember that your API token should be kept confidential, so do not share it in a notebook or in any public code.
    ...
    service = QiskitRuntimeService(
      channel="ibm_quantum",
      token="<MY_IBM_QUANTUM_TOKEN>" # Your token is confidential.
      # Take care not to share your token in public code.
      )
    ```

1. <span id="save-account"></span>If you want to save your token locally, set the `IQP_API_TOKEN` environment variable in your system by adding the following line to your shell profile (for example, `.bashrc`, `.zshrc`) or by setting it directly in your terminal. **Credentials are saved locally as plain text. Never include your token in a notebook!**
    ...
    When you authenticate to the service, use `token=iqp_api_token` in your code to retrieve the token from the environment variable.
    ...
    ```
      ...
      Or, optionally use the `save_account()` method to save your credentials for easy access later on, before initializing the service. **Credentials are saved locally as plain text. Never include your token in a notebook!**
...
1. <span id="cloud-save"></span>Authenticate to the service by calling `QiskitRuntimeService` with your saved credentials or with your IBM Cloud API key and CRN. Remember that your credentials should be kept confidential, so do not share it in a notebook or in any public code.
    ...
    You can optionally use the `save_account()` method to save your credentials for easy access later on, before initializing the service. See these [instructions for creating an environment variable](#save-account) to use in the following code. **Credentials are saved locally as plain text. Never include your token in a notebook or public code!**
