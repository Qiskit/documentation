...
1. <span id="save-account"></span>If you want to save your token locally, set the `IQP_API_TOKEN` environment variable in your system by adding the following line to your shell profile (for example, `.bashrc`, `.zshrc`) or by setting it directly in your terminal. **Credentials are saved locally as plain text. Never include your token in a notebook!**
    ...
    <details>
      ...
      * If you have saved credentials for both an IBM Quantum Platform account and an IBM Cloud account, IBM Cloud is the default account used when you initialize your account, unless you specify otherwise with the `channel` parameter.
