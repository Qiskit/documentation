...
* **ID error** - Identity gate error, measured from randomized benchmarking. The identity gate is a delay of duration equal to the duration of the √x (SX) and X gates. The randomized benchmarking sequence includes SX, ID, and X gates, and it is assumed their errors are the same.
* **Single-qubit gate length (ns)** - Duration of a single-qubit gate operation.
* **RX error** - RX gate error, averaged over the RX angles using a variant of randomized benchmarking.
* **Z-axis rotation (RZ) error** - Error of the virtual RZ gate. Reported as all 0 since these are performed in software.
* **√x (sx) error**:
* **Pauli-X error** - Error in the X gate, assumed to be the same error as the √x (SX) gate measured from randomized benchmarking.
...
* **CZ error** (Heron) - Error in the CZ gate averaged over the CZ angles using a variant of randomized benchmarking for arbitrary unitaries.
* **ECR error** (Eagle) - Error in the ECR gate averaged over the ECR angles using a variant of randomized benchmarking for arbitrary unitaries.
