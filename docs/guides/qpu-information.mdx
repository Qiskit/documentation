...
*   **EPLG** - Average error per layered gate in a chain of 100 qubits. Average error per layered gate measures the average gate error in a layered chain of $N$ qubits ($N$=100 here). It is derived from a similar quantity known as the layer fidelity (LF) where EPLG$_{100}$ = 4/5(1-LF$^{\frac{1}{99}}$) and layer fidelity is the process fidelity of the layered chain of $N$ qubits. For details, see the paper [Benchmarking quantum processor performance at scale](https://arxiv.org/abs/2311.05933). Note that in the paper EPLG is defined for process error and here we quote average gate error for consistency with the individually reported gate errors.
...
![An expanded card for a sample QPU.](/images/guides/exp-card.png "Expanded card for a sample QPU")
