...
Each qubit on a QPU has specific properties that characterize its performance and behavior:

- **T1**: The longitudinal relaxation time, representing how long it takes (in seconds) for a qubit to decay from the |1⟩ state to the |0⟩ state.
- **T2**: The transverse relaxation time, representing how long (in seconds) quantum phase information is preserved.
- **Frequency**: The operating frequency of the qubit (in Hz).
- **Readout error**: The average probability of a measurement error for the qubit, calculated as:  
  `(prob_meas1_prep0 + prob_meas0_prep1)/2`
...
- **Readout length**: The duration (in seconds) of the measurement operation, from when the measurement pulse starts until the system is ready for another measurement.

<Admonition type="note" title="Measurement errors">
These measurement probabilities (`prob_meas0_prep1` and `prob_meas1_prep0`) represent what are known as SPAM errors (State Preparation And Measurement). While they primarily reflect measurement errors in IBM Quantum systems, they may include small contributions from imperfect state preparation.
</Admonition>

<Admonition type="tip">
You can access these properties programmatically using:
```python
backend.properties().qubit_property(qubit_index)
