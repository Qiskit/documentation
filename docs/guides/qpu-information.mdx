...
<LegacyContent>
  <Admonition type="note">
    This documentation is relevant to IBM Quantum&reg; Platform Classic. If you
    need the newer version, go to the new [IBM Quantum Platform
    documentation.](https://quantum.cloud.ibm.com/docs/guides/qpu-information)
  </Admonition>
</LegacyContent>
<CloudContent>
  <Admonition type="note">
    This documentation is relevant to the new IBM Quantum&reg; Platform. If you
    need the previous version, return to the [IBM Quantum Platform Classic
    documentation.](https://docs.quantum.ibm.com/guides/qpu-information)
  </Admonition>
</CloudContent>
...
The following is a subset of QPU configuration values available in IBM Quantum Platform and from [Qiskit](/docs/api/qiskit/1.4/qiskit.providers.models.BackendConfiguration).
...
<Admonition type="note" title="Average gate fidelity">
  Note that the metric for median ECR error (for Eagle processors) and median CZ
  error (for Heron processors) is the [average gate
  fidelity](/docs/api/qiskit/quantum_info#average_gate_fidelity), which is the
  average over all possible input states of the fidelity between the state
  produced by the actual operation and the state produced by the ideal
  operation.
</Admonition>
...
- **Calibration data** - Download the calibration data as a CSV file or click the arrow to display the Topology diagram, Individual qubit readout graph, or the Calibration data table. You can customize the data that is shown, depending on the view you have open. For example, on the Topology diagram, you can choose the data you want to see for connections and qubits. The colored bars associated with the diagram or graph indicate the range that is shown, with the average value marked. The color maximum and minimum change depending on the QPU.
  ...
  > - <span id="coupling-map">**Topology diagram** or **coupling map** </span> -
  >   A diagram that indicates the pairs of qubits that support two-qubit gate operations
  >   between them. This is also called the coupling map or connectivity. Qubits
  >   are represented as circles and the supported two-qubit gate operations are
  >   displayed as lines connecting the qubits.
  > - **Individual qubit properties** - Shows the following properties:
  >   - **T1 (Relaxation Time)**: The T1 time represents the average duration a qubit remains in its excited state ($|1\rangle$) before decaying to its ground state ($|0\rangle$) due to energy relaxation. This parameter is crucial for understanding the qubit's energy relaxation behavior and is typically measured in microseconds (µs).
  >   - **T2 (Dephasing Time)**: The T2 time denotes the timescale over which a qubit maintains phase coherence between the $|0\rangle$ and $|1\rangle$ states. It accounts for both energy relaxation and pure dephasing processes, providing insight into the qubit's coherence properties. T2 is also measured in microseconds (µs).
  >   - **frequency**: This parameter specifies the resonant frequency of the qubit, indicating the energy difference between the $|0\rangle$ and $|1\rangle$ states. It is typically expressed in gigahertz (GHz) and is essential for calibrating control pulses and understanding qubit behavior.
  >   - **anharmonicity**: Anharmonicity in superconducting qubits refers to the difference in energy between the first and second excited states of the qubit. It is typically expressed in frequency units, such as gigahertz (GHz) or hertz (Hz).
...
<LegacyContent>
  To find your available QPUs, open the [Compute
  resources](https://quantum.ibm.com/services/resources) page. Click a QPU to
  view its details.
</LegacyContent>
<CloudContent>
  To find your available QPUs, open the [Compute
  resources](https://quantum.cloud.ibm.com/computers) page. Click a QPU to view
  its details.
</CloudContent>
You can also view your available QPUs by using the [backends
API.](/docs/api/qiskit-ibm-runtime/qiskit-runtime-service#backends)

## View QPU configuration

<LegacyContent>
  View QPU configuration values by selecting a QPU on the [Quantum processing
  units](https://quantum.ibm.com/services/resources) page from the Compute
  resources tab. The three tabs in the Calibration data section let you choose
  how to view the calibration data; the Map view tab is automatically selected.
</LegacyContent>
<CloudContent>
  View QPU configuration values by selecting a QPU on the [Compute
  resources](https://quantum.cloud.ibm.com/computers) page. The three tabs in
  the Calibration data section let you choose how to view the calibration data;
  the Map view tab is automatically selected.
</CloudContent>
