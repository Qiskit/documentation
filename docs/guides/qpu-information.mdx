...
The following is a subset of QPU configuration values available in IBM Quantum Platform and from [Qiskit](/api/qiskit/providers-models-backend-configuration).
...
Note that the metric for median ECR error (for Eagle processors) and median CZ error (for Heron processors) is the [average gate fidelity](/api/qiskit/quantum-info#average_gate_fidelity), which is the average over all possible input states of the fidelity between the state produced by the actual operation and the state produced by the ideal operation.
</Admonition>
