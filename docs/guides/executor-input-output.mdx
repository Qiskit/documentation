...
The inputs and output of the Executor primitive are very different from those of the [Sampler](/docs/guides/sampler-input-output) and [Estimator](/docs/guides/estimator-input-output) primitives. For example, instead of taking a list of PUBs as the input, Executor takes a `QuantumProgram`, which contains a list of `QuantumProgramItem` objects. `QuantumProgramItem`s can be of two types:
- `CircuitItem`, which typically stores a circuit and its parameters.
- `SamplexItem`, which typically stores a circuit and a `Samplex` object, capable of generating (randomized) sets of parameters at runtime.
These container classes give you more flexibility than a PUB, which is a simple tuple data structure. Indeed, they allow you to specify where and how a set of circuit instructions should be randomized before execution, for example to perform twirling or injecting noise.
...
[`QuantumProgramItem`](https://qiskit.github.io/qiskit-ibm-runtime/stubs/qiskit_ibm_runtime.quantum_program.QuantumProgramItem.html). Each of these items represents a
...
* A parametrized `qiskit.circuit.QuantumCircuit` with a `samplomatic.samplex.Samplex`. A Samplex is a core type of the [Samplomatic library](https://github.com/Qiskit/samplomatic) and encodes all the information about the randomization process itself.

Each of these items, including instructions to add them to a `QuantumProgram`, is explained in more detail below.

### Example: Create a `QuantumProgram` with three different tasks

You must first initialize your quantum program, then append program items to it using either `append_circuit_item` or `append_samplex_item` (if samplex is present), as shown in the following examples.

The below cell initializes a `QuantumProgram` and specifies that it should run 1024 shots for every configuration of each item in the program. Then it appends a version of the target circuit with parameters, transpiled according to the backend's instruction set architecture (ISA).
...
Circuit items are executed without any sort of randomization. On the contrary, samplex items allows users to specify how to randomize their content. In the next cell, we show how to generate a samplex items for the GHZ circuit above that adds twirling to all the gates and measurements in it.

In particular, setting `shape`
to `(2, 14, 10)` specifies to randomize each of the 10 sets of parameters 28 (`2 x 14`) times, and to arrange the randomized parameter sets in an array of shape `(2, 14, 10)`.
...
```python
# Transpile the circuit and group gates and measurements into annotated boxes
preset_pass_manager = generate_preset_pass_manager(
    backend=backend, optimization_level=0
)
# the boxing pass manager is a convenient way to group gates
# and measurements into boxes and request that they be annotated; in this example, we request that these boxes
# be annotated with a`Twirl` annotation
...
The result for the previous example contains three items:
...
classical register.  The information is stored in an `np.ndarray` of shape `(shots, register bits)`, which is `(1024, 3)` for the above example.
