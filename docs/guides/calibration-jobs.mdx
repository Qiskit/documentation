...
This calibrates the CZ gate as a slower version of the CZ gate in the standard calibration. That is, it causes increased CZ duration.  This targets increased processor performance for error mitigation workflows by improving noise model agreement. However, longer gate times causes some additional decoherence.

In addition to containing the slower CZ, this calibration also has AC TLS bias mode, which stabilizes the device's noise model. See [Observable Estimation in the Absence of Classical Verification](https://arxiv.org/pdf/2607.25998#page=[52]) for details.
