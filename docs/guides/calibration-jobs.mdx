...
Brief parameter monitoring jobs are executed approximately once an hour, automatically interleaved between user jobs, using the full IBM Quantum Compute Service software stack. The results of these jobs are analyzed to check whether any parameters have begun to deviate from acceptable ranges, ideally catching issues before they become significant enough to affect performance to a noticeable level.
...
In addition to jobs monitoring individual parameters are jobs that monitor the performance of the quantum computer more holistically, such as tests that look at the fidelity of generated Bell states, as well as tests of fractional gates and dynamic circuits on quantum computers that support those features. The goal of these tests, which also run through the full Quantum Compute stack interleaved with user jobs, is to efficiently validate the overall behavior of the hardware and software. If these tests detect a significant drop in performance, the quantum computer will automatically pause the job queue until the issue is resolved, ensuring that user jobs won't run until the device is performing as expected again.
...
Default calibrations are triggered whenever monitoring jobs indicate that parameters such as pulse amplitudes or angles have deviated from their ideal values. They run throughout the day in between user jobs, and therefore there is no set time period at which calibrations start and finish. These run only on the qubits/gates for which parameter monitoring has identified specific issues, along with any qubits required to run at the same time according to specific batching rules. On Heron QPUs, the total time spent on calibrations is typically under two hours per day.
...
### Slow gate calibration

This calibrates the CZ gate as a slower version of the CZ gate in the standard calibration. That is, it causes increased CZ duration.  This targets increased processor performance for error mitigation workflows by improving noise model agreement. Note that longer gate times causes some additional decoherence.

In addition to containing the slower CZ, this calibration also has AC TLS bias mode, which stabilizes the device's noise. See [Observable Estimation in the Absence of Classical Verification](https://arxiv.org/pdf/2607.25998#page=[52]) for details. This mode is useful for any PEA or PEC workflow.

<Admonition type="note">
Slow gate calibration is not available on all QPUs.
</Admonition>

#### Run a slow gate calibration

To start a slow gate calibration, run the following command:

`backend = service.backend(<backend_name>, calibration_id="slow_gates")`

#### When to use slow gate calibration

This calibration is especially useful for any PEA or PEC workflow, but is also useful for  any workflow that involves measuring observables.

#### When NOT to use slow gate calibration

This calibration degrades results when the circuits involved have bitstrings that can be classically distilled. That is, peaked circuits (circuits engineered to concentrate probability weight on a single bitstring), and [SQD or SKQD](/learning/courses/integrating-quantum-and-high-performance-computing/sqd-skqd) workflows.
