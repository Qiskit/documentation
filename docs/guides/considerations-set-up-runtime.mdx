...
IBM Quantum Platform is the dashboard for your IBM Cloud&reg; account's Qiskit Runtime service instances, workloads, and a simplified view of access management. An organization's IBM Cloud account can have multiple users and multiple Qiskit Runtime service instances, each with its own allocation. Identity and Access Management (IAM) controls which users can access which service instances, so you can enable collaboration while restricting visibility where needed. Access management becomes more relevant once you have service instances on paid plans, since those incur cost. See [IBM Cloud account structure](/docs/guides/cloud-account-structure) for an overview of how accounts, users, instances, and access fit together, and the [IBM Cloud IAM documentation](https://cloud.ibm.com/docs/account?topic=account-iamoverview) for full details on the IAM concepts referenced in this guide, such as access groups, policies, roles, and resource groups.
...
If your organization has multiple IBM Cloud accounts - for example, separate accounts per business unit, each with its own Qiskit Runtime service instances - you can link them under a single [IBM Cloud Enterprise account](https://cloud.ibm.com/docs/account?topic=account-what-is-enterprise), which has one main account responsible for billing and one or more child accounts. To redistribute Premium or Flex Plan allocation between child accounts, contact IBM Quantum support. See the IBM Cloud Enterprise documentation for full details.
</Admonition>
...
* **How many service instances do you need, and what will each be used for?** Plan your instance names carefully. Whenever you create a service instance through the IBM Quantum Platform UI, the platform makes an additional call to IAM on your behalf to create a matching access group (named after the instance, with "Collaborators" appended) that grants write access to that instance, so instance names end up as access group names too. This extra step only happens when you create an instance through the IBM Quantum Platform UI - it does not happen if you create the instance using Terraform, the IBM Cloud CLI, or the IBM Cloud API.
...
* **Will you use the access group that's automatically created for each instance, create additional access groups of your own, assign access to individual users directly, or organize instances into resource groups?**
   ...
   * **Resource groups** are used only when you need to maintain a clear separation of service instances. When creating a service instance from IBM Quantum Platform, you can select which resource group it belongs to (and add tags), but you must use the IBM Cloud console to create or manage the resource groups themselves. If more service instances are created in a resource group, all users who have access to the resource group see them automatically, without updating access groups. If you choose to use resource groups, you will create access groups and then assign them to resource groups.
...
Access applies to service instances. Thus, users with write access to an instance (including through the "Collaborators" access group automatically created for instances made through the IBM Quantum Platform UI) can cancel their own workloads, but can also view and cancel other users' workloads in that instance. This is a function of how IAM works and cannot be changed.
