...
Unlike the existing Python addon, this code is written in modern C++17 and is designed to enable HPC workflows and applications, particularly those that require a single binary to be compiled for use with MPI.
...
- M3 (or mthree) is a package for scalable quantum measurement error mitigation that need not explicitly form the assignment matrix, or its inverse - and is thus a matrix-free measurement mitigation (M3) routine.
