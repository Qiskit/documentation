...
shape is `(4, 3)`. The followins code uses a circuit with 1024 shots and a 3-bit classical register:
...
program.append(
    ...
    # shape defaults to (10,) - one randomized execution per configuration
)
...
# Get all shots for randomization (0, 7) and parameter configuration 3
specific = meas_data[0, 7, 3, :, :]  # shape (1024, 3)

# Average over all randomizations for parameter configuration 5 on bit 2
