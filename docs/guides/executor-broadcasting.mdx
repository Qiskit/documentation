...
shape is `(4, 3)`. The following code uses a circuit with 1024 shots and a 3-bit classical register (as defined in the previously defined [Quickstart](#quickstart) example):
...
```python
# Using shape=(2, 14, 10) with input extrinsic shape (10,), and 1024 shots
# and 3 classical registers, as defSined in the Quickstart example.
result = executor.run(program).result()
