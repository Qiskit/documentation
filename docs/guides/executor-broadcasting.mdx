...
The data provided to the Executor primitive can be arranged in a variety of shapes to provide flexibility in a workload through [broadcasting](/docs/guides/primitive-input-output#broadcasting). This guide explains how the Executor handles array inputs and outputs using broadcasting semantics. Understanding these concepts will help you efficiently sweep over parameter values, combine
multiple experimental configurations, and interpret the shape of returned data.

<Admonition type="note">
The examples in this topic use pseudocode and assume you have defined appropriate circuits, used the Samplomatic pass manager to add boxes and annotations, and used the Samplomatic `build` method to get a template and samplex for each code block, as necessary.
</Admonition>
...
The rest of this guide explains how this works and how to build more complex sweeps, including Samplomatic-based randomization and inputs.
...
program.append.circuit_item(circuit, circuit_arguments=parameter_values)
...
shape is `(4, 3)`. The following code uses a circuit with 1024 shots and a 3-bit classical register:
