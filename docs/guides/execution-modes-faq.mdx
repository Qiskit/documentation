...
description: Answers to commonly-asked questions about execution modes

---
# Execution modes FAQs

<Accordion>
  <AccordionItem title="Does `qiskit-ibm-runtime` local testing mode support different execution modes?">
...
- There is a slight overhead for running each job, so you end up paying a little more for batched jobs versus when all circuits are executed in a single job.
