...
Finally, if the `twirling` options `num_randomizations` or `shots_per_randomization` are specified, those values take precedence over `default_shots`  (but not `shots` values specified as part of the `run` call).
...
3. If `num_randomizations` or `shots_per_randomization` are specified as twirling options, use those values.
