...
[IBM Quantum&reg; Composer](https://quantum.cloud.ibm.com/composer) is a graphical quantum programming tool that lets you drag and drop operations to build quantum circuits and run them on real quantum hardware or simulators.
...
![IBM Quantum Composer window.](/docs/images/tutorials/explore-composer/52af25d0-aafe-4a4d-b92b-91cd3a3c54a1.avif)
...
6. **Visualizations** - Visualizations characterize your circuit as you build it. They use a single-shot statevector simulator, which is different from the system specified in the Run circuit settings. Note that the visualizations ignore any measurement operations you add. Sign in and click **Run circuit** to get results from the specified backend instead.
...
1. (Optional) If you are not currently signed in to IBM Quantum, select **Sign in** in the upper right corner. Then, you can either sign in or **Create an IBM Cloud account**.
   ...
   If you don’t sign in, the visualizations automatically show simulated results for up to four qubits. If you want to run your circuit on a quantum computer or simulator, or if you want to visualize a circuit that has more than four qubits, you must sign in.
...
![A set of selected operations.](/docs/images/tutorials/explore-composer/3c5c063d-b15f-43cb-bb65-9728187454d8.avif)

Select Copy from the contextual menu to copy the group.

![Right-click menu options.](/docs/images/tutorials/explore-composer/611ee5a8-cd08-401c-ba8c-6b40a7a55c21.avif)
...
![Create a custom operation by grouping.](/docs/images/tutorials/explore-composer/6ebf7cf5-dc36-49d4-ba78-f1af9ea55972.avif)
...
![control modifier.](/docs/images/tutorials/explore-composer/c5bb3c7b-3b9b-480b-8b17-e8c5fdf54543.avif)
...
![Gates in the custom operation](/docs/images/tutorials/explore-composer/763d3b42-4b6e-4409-817f-7d15abae2d76.avif)

b) The code for the new operation:

![Customized new operation](/docs/images/tutorials/explore-composer/db5856ca-751c-4f12-9f25-6060b9003687.avif)

c) The new operation in the graphical editor:

![New operation](/docs/images/tutorials/explore-composer/d7fb8ec5-5eae-4ad4-9972-1e3665b39b7f.avif)

### 4. Inspect your circuit, step-by-step

The Inspect mode de-mystifies the inner workings of the circuits you create. It steps through a simulation of your circuit, one layer at a time, so that you can see the state of the qubits as the computation evolves.

![Circuit Inspector.](/docs/images/tutorials/explore-composer/138134c2-cdfb-4a8b-918d-98df70fb8e3c.avif)
...
![Controls to step forward and backward, and to jump to a circuit's beginning or end.](/docs/images/tutorials/explore-composer/5f2dfb7c-a244-405c-8950-f422a4408fcf.avif)
...
Click **Run circuit** in the upper right corner. In the window that opens, select an available QPU (quantum processing unit) or simulator. QPUs send the circuit to real quantum hardware, and simulators have the word ‘simulator’ in the name. You can also choose an instance, which is associated with a plan (such as the Open, Flex, or Premium Plans). The instance you choose affects which QPUs are available to you. Click the View details link in the table of QPUs to see more information on each QPU.

Next, you can set the number of shots (executions) of your circuit that the backend will perform.

You can optionally name the job and add tags in this panel as well. This will not change the name of the circuit. A "Composer" tag has been pre-populated to make it easier to filter your Workloads table by Composer jobs. You can remove this tag.


![Window displaying job options.](/docs/images/tutorials/explore-composer/21dd1126-b0e7-4d35-b86a-6ec0ef2c7385.avif)

> **Note**
> When you run a circuit, it is automatically sent to the least busy QPU, unless you specify a QPU in the Run settings. If you run the same circuit again, the system selection window will default to your previous choice.
...
![probability the qubit is in the 1 state](/docs/images/tutorials/explore-composer/3e4140c6-9da0-46b0-85e4-a1509078d3e9.avif)
...
![phase of the local qubit state](/docs/images/tutorials/explore-composer/7f047fde-c19e-46db-b596-be5da55e697e.avif)

**Example: phase disks for two different qubits**

![phase disk examples](/docs/images/tutorials/explore-composer/0207fba8-8b4b-4611-9638-6cbe169e10c4.avif)
...
![purity of the qubit state](/docs/images/tutorials/explore-composer/425d4355-6a67-45ec-989c-afc2aa800448.avif)
...
![a quantum circuit](/docs/images/tutorials/explore-composer/58f83286-9707-4e43-893e-2bd161c744cd.avif)
...
![measurement probabilities view](/docs/images/tutorials/explore-composer/4d79349d-dadd-4e9c-8571-4617601d22f3.avif)
...
![q-sphere view](/docs/images/tutorials/explore-composer/55d58744-c95c-44b3-ba29-3c7e24ec4dcc.avif)
...
![statevector view](/docs/images/tutorials/explore-composer/1f828b26-e332-4ef2-b8f0-649030fabab4.avif)
...
| ------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![x\_gate](/docs/images/tutorials/explore-composer/1d52e1a3-ca53-4caa-9700-259e35f0dbaf.avif) | `x q[0];`          | ![image19](/docs/images/tutorials/explore-composer/1392cb96-3d66-4a9e-9ed1-687a937bc3cc.avif)                              | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![CX-Gate](/docs/images/tutorials/explore-composer/e74dca0d-3f6b-42c9-9471-b65fd03ef5b5.avif) | `cx q[0], q[1];`          | ![image19](/docs/images/tutorials/explore-composer/40abd3fa-ab1e-4ea2-ade7-49a06073b37f.avif)                              | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                  | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| --------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![SWAP gate](/docs/images/tutorials/explore-composer/61f641e0-fb8f-4a67-af60-9b1bf792f2c6.avif) | `swap q[0], q[1];` | ![swap\_qsph](/docs/images/tutorials/explore-composer/e2090577-7a13-484a-a642-2c56f42119cb.avif)                        | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| -------------------------------------------------- | -------------- |
| ![Identity gate](/docs/images/tutorials/explore-composer/f3c1f16a-d5b4-4f1c-9f18-2d9b6dcfd2fe.avif) | `id q[0];`     |
...
| Composer reference                                | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![t\_gate](/docs/images/tutorials/explore-composer/b5f1b90c-d3b7-4f60-82f9-259dc40cef5b.avif) | `t q[0];`          | ![image29](/docs/images/tutorials/explore-composer/c43496bb-89ff-4518-9afb-d276e400b032.avif)                              | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![z\_gate](/docs/images/tutorials/explore-composer/941c8820-65bc-444c-890b-b445b9958085.avif) | `z q[0];`          | ![image23](/docs/images/tutorials/explore-composer/84b92ecb-3b62-4cb3-9c01-549866681816.avif)                              | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![T-dagger gate](/docs/images/tutorials/explore-composer/8e348308-7e1f-4e61-a002-ff84d04d89b1.avif) | `tdg q[0];`        | ![image31](/docs/images/tutorials/explore-composer/b79890e8-e6f1-4c22-8546-6322bc6ed57b.avif)                            | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![S-dagger gate](/docs/images/tutorials/explore-composer/ae59613c-0c65-447c-869d-81c7ffb8b2ad.avif) | `sdg q[0];`        | ![image27](/docs/images/tutorials/explore-composer/1de10ec2-2026-40f6-8f96-cd332f79780d.avif)                            | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                              | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ----------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![p\_gate](/docs/images/tutorials/explore-composer/2b9eec00-ecbf-45ce-ae09-8e3015dc6c98.avif) | `p(theta) q[0];`   | ![p\_qsph](/docs/images/tutorials/explore-composer/a02007e1-4c68-42af-b10c-49cbbaaed8ef.avif)                             | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![rz\_gate](/docs/images/tutorials/explore-composer/112f2dec-4fa2-42f5-b51c-2bd2205ebd30.avif) | `rz(angle) q[0];`  | ![image17](/docs/images/tutorials/explore-composer/8d60318b-72f9-4188-a07a-ad4d3e82b4e8.avif)                             | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| ------------------------------------------- | ------------------ |
| ![0-opera](/docs/images/tutorials/explore-composer/6a1605ff-fb88-4bf7-ac81-43b0cce918c9.avif) | `reset q[0];`      |
...
| Composer reference                                       | OpenQASM reference |
| -------------------------------------------------------- | ------------------ |
| ![Measurement in the standard or z basis](/docs/images/tutorials/explore-composer/9fca7f1e-ba46-400d-a3d1-312ee011d705.avif) | `measure q[0];`    |
...
| Composer reference                                          | OpenQASM reference |
| ----------------------------------------------------------- | ------------------ |
| ![Control modifier](/docs/images/tutorials/explore-composer/243c0fad-8236-4b03-a71a-bacaed4055c8.avif) | `c`                |
...
| Composer reference                               | OpenQASM reference |
| ------------------------------------------------ | ------------------ |
| ![barrier](/docs/images/tutorials/explore-composer/96cd10ae-e3cf-40ea-9ca7-49a2d68a8c07.avif) | `barrier q;`       |
...
| Composer reference                                    | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ----------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![h-gate](/docs/images/tutorials/explore-composer/fd58c6d3-f0af-4566-bf2b-105ed850c946.avif) | `h q[0];`          | ![image1](/docs/images/tutorials/explore-composer/e797de24-e6de-44cb-9137-bb8931d763df.avif)                               | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![sx\_gate](/docs/images/tutorials/explore-composer/67933b13-85f0-45e9-bc31-dcea4684dd6f.avif) | `sx q[0];`         | ![sx\_qsph](/docs/images/tutorials/explore-composer/fd300da1-512c-4193-8e77-b15e5e378d34.avif)                         | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                    | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ----------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![sxdg\_gate](/docs/images/tutorials/explore-composer/4d5d9638-30af-48c3-a8c9-d506a0041f24.avif) | `sxdg q[0];`       | ![sxdg\_qsph](/docs/images/tutorials/explore-composer/3e258992-baa9-42bb-92b1-6e082eb674c9.avif)                     | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![rx\_gate](/docs/images/tutorials/explore-composer/f943edee-7a79-43a1-9e18-87eba141f787.avif) | `rx(angle) q[0];`  | ![image13](/docs/images/tutorials/explore-composer/7640c11d-253a-42b7-a3cd-63a8fa98e446.avif)                             | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ------------------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![ry\_gate](/docs/images/tutorials/explore-composer/77c50d8c-b2d1-4de8-9657-572ede30e02f.avif) | `ry(angle) q[0];`  | ![image15](/docs/images/tutorials/explore-composer/3c1efa46-e05f-486d-8e6a-df5fd0add674.avif)                             | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                 | OpenQASM reference       | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| -------------------------------------------------- | ------------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![rxx-gate](/docs/images/tutorials/explore-composer/d4328ec9-dc63-4c5c-b7ac-8ecc7cbcd5b6.avif) | `rxx(angle) q[0], q[1];` | ![rxx\_qsph](/docs/images/tutorials/explore-composer/206828a8-0392-498e-98c2-1c89ece527bb.avif)                          | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                  | OpenQASM reference       | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| --------------------------------------------------- | ------------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![rzz\_gate](/docs/images/tutorials/explore-composer/50bddfba-198c-4d70-b978-d044da32c36e.avif) | `rzz(angle) q[0], q[1];` | ![rzz\_qsph](/docs/images/tutorials/explore-composer/013918f1-8973-499b-8b2b-ee150d628852.avif)                          | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                                   | OpenQASM reference         | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ---------------------------------------------------- | -------------------------- | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![u\_gate](/docs/images/tutorials/explore-composer/628ae112-fcf8-4be7-a04b-568c390f7c4d.avif) | `u(theta, phi, lam) q[0];` | ![u3\_qsph](/docs/images/tutorials/explore-composer/3e5212aa-a78c-48d8-b49a-4af362e5b899.avif)                            | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
...
| Composer reference                        | OpenQASM reference | Q-sphere | Note about q-sphere representations                                                                                                                                                                                                     |
| ----------------------------------------- | ------------------ | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![rc3x-gate](/docs/images/tutorials/explore-composer/5cebe71b-7864-4ff6-b669-ae2ec7049901.avif) | `rc3x a, b, c, d;` | ![rc3x\_qsph](/docs/images/tutorials/explore-composer/86b6a4bc-36cd-483c-a225-bc1d0b3207b3.avif)                        | The q-sphere representation shows the state after the gate operates on the initial equal superposition state $\frac{1}{\sqrt{2^{n}}}\sum_{i=0}^{2^{n}-1}\vert i\rangle, $ where $n$ is the number of qubits needed to support the gate. |
