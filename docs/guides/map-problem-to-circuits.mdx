...
![All Qiskit pattern steps are shown (Map problem, Optimize for hardware, Execute on hardware, Post-process results). The Map problem step is emphasized.  This step uses the Qiskit SDK.](/images/qiskit-patterns/map-problem.svg)
