...
Qiskit Serverless provides a simple interface to run workloads across quantum-classical resources. This includes deploying programs to the IBM Quantum Platform, running workloads remotely, and easy resource management for multi-cloud and quantum-centric supercomputing use cases.
...
This package requires `python` 3.11+. For users and organisations who want to run Serverless on custom infrastructure, follow the [deploy Qiskit Serverless on custom cloud resources guide](https://qiskit.github.io/qiskit-serverless/cloud.html).
