...
implements the Estimator V2 interface introduced in the Qiskit SDK 1.0, and offers additional input vectorization
features in comparison with the [`Estimator`](../api/qiskit/qiskit.primitives.Estimator) class, which implements the
deprecated Estimator V1 interface. (For more information, see the [V2 primitives migration guide](/migration-guides/v2-primitives).) Both can take circuits, observables, and parameters as inputs and return the locally
