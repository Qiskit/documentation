...
The reference implementation of `Estimator` in `qiskit.primitives` that runs on a local statevector
...
primitive implementation should target for expectation values estimates.  This is the sampling overhead and is defined exclusively in the `.run()` method. This allows you to fine-tune the option all the way to the PUB level.
...
The reference implementations of `Sampler` in `qiskit.primitives` is the [`StatevectorSampler`](../api/qiskit/qiskit.primitives.StatevectorSampler) class. It takes circuits and parameters as inputs and returns the results from sampling from the output probability distributions as a quasi-probability distribution of output states.
