...
In the following image, based on the example output, `bar` corresponds to the stretches on Qubit 3. The first stretch delay that uses `bar` starts at the end of the `init_play` (1365). The stretch duration is 30, so that delay ends when the `x` gate starts (1365+30 = 1395).
