...
- **Remainder:** The remainder from resolving the stretch
...
'stretch_values': 
 [{'name': 'x_0',
   'value': 5,
   'remainder': 1,
   'expanded_values': [[803, 6], [810, 5]]}
   ]
```   

You can also use a visualization (requires plotly) to help understand the timing. Use the sliders at the bottom, the controls at the top (hover over the image to reveal these), and the legend on the side of the output to customize the view.

![The output from the `draw_circuit_schedule_timing` command is shown.](/docs/images/guides/stretch/stretch-visualization.png)
