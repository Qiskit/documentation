...
You can specify a single circuit and multiple observables (if using Estimator) and parameter value sets for that circuit.
...
It receives an array of primitive unified blocs (PUBs) as the inputs and returns the computed expectation values per pair, along with their
