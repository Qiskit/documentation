...
*This section applies to Premium users, who have access to multiple instances.* When determining which jobs to run, the [fair-share scheduler](./fair-share-scheduler) takes into account an instance's usage of the QPU in question. For example, an instance that has already run many jobs on a certain QPU might not have as high a priority on that QPU, when compared to an instance that has very low usage.

The jobs you run, as well as the jobs run by other collaborators in the same instance, count toward the reported usage for that instance. You can see the usage for a selected instance on your [Dashboard](https://quantum.ibm.com/) (use the [instance switcher](#instance-switcher) at the top of the page to change which instance is reflected on your Dashboard).  
