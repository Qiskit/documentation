...
After you have an IBM Cloud&reg; account, you can create an instance in the Open Plan and access up to 10 minutes total of quantum time for free per 28-day rolling window.
</CloudContent>
...
<CloudContent>
You must use the Cloud Resource Name (CRN)  to specify an instance to use; either when initializing the service or when sending the workload to a QPU.  The instance's CRN is listed on the [Platform dashboard](https://quantum.cloud.ibm.com/). Before running the below code, follow the steps to [save an account.](/guides/cloud-setup#cloud-save)
</CloudContent>
<LegacyContent>
```python
...
# Optional: List all the instances you can access. This assumes you have already saved your account credentials.
service = QiskitRuntimeService()
print(service.instances())

# Optional: Specify an instance at service level. This becomes the default unless overwritten.
service = QiskitRuntimeService(instance="<CRN>")
backend1 = service.backend("<QPU-name>")
```
</CloudContent>
