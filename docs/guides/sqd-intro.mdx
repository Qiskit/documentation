...
The most straightforward way to install the `qiskit-addon-sqd` packge is via PyPI.
...
Users who wish to contribute to this package or who want to install it manually may do so by first cloing the repository
...
The bulk of the SQD workflow lies here where each of these subspace Hamiltonians are diagonalized. The ground states obtained from each of these subspaces, $|\psi^{(K)}\rangle$, are used to obtain an estimate of a reference vector of occupancies $\mathbb{n}^{(K)}$ averaged over each of the $K$ subspaces and sent back to the configuration recovery step. Then a new set of subspaces are obtained and diagonalized, and this procedure iterates in a self-consistent loop until the ground state energy estimate has converged.
