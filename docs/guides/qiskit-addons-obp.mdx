...
where $S$ represents the total number of slices and $\mathcal{U}_s$ denotes a single slice of the circuit $U_C$. Each of these slices are then analytically applied in sequence to measure the backpropagated operator $O'$, and might or might not contribute to the overall size of the sum, depending on if the slice is a Clifford versus non-Clifford operation.
...
<Admonition type="tip" title="Recommendations">
    - [Get started with OBP.](/guides/qiskit-addons-obp-get-started.ipynb)    
