...
An API token is a long-lived credential used to [authenticate to the IBM Quantum Compute Service](/docs/guides/hello-world#install-and-authenticate).  A bearer token is a temporary credential that is used to [authenticate to the REST API in an untrusted environment](/docs/guides/cloud-setup-untrusted#authenticate-to-the-rest-api-in-an-untrusted-environment).  A bearer token cannot be used to authenticate to the IBM Quantum Compute Service.
...
IBM retains your (undeleted) job and usage data for three years while you continue to use the Quantum Compute Service.
