...
``` python
service = QiskitRuntimeService()
with Session(backend="ibm_sherbrooke"):
    ...
```
...
You can define the maximum session timeout with the `max_time` parameter. This should exceed the longest job's execution time and be within the QPU's limits.
