...
The maximum session time to live (TTL) determines how long a session can run before expires.  You can set this value with the `max_time` parameter. This should exceed the longest job's execution time.
