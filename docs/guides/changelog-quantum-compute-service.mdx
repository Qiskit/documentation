...
- **The first Nighthawk r2 QPU is now available** &mdash; Premium, Flex, and Pay-As-You-Go Plans now have access to `ibm_phoenix`, the first IBM Quantum Nighthawk r2 QPU and our fastest processor yet. `ibm_phoenix` is a high-connectivity, square-lattice 120-programmable-qubit device that delivers up to 25× higher throughput than Heron while matching its fidelity. The gain, which comes from independent high-speed reset on every programmable qubit, provides a lower default repetition delay of 1 µs (previously 250 µs), pushing throughput above 100,000 Max Circuits Per Second (MCPS).

  On average, the increased speed and connectivity make it possible to do three to four times more usable work from the same capacity, with the largest, most demanding workloads seeing benefits of up to ten times more usable work. Larger experiments, broader parameter sweeps, and methods that were once too costly at scale are now practical to run routinely.
