...
<CloudContent>
Select any QPU on the [Compute resources](https://quantum.cloud.ibm.com/computers) tab. The default gates for that QPU are listed under Details. Note that the non-unitary operations are not listed here; use the method in Qiskit described above to see all native gates and operations for a QPU.
</CloudContent>
