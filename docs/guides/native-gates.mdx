...
You can view the native gates and operations for a QPU either [with Qiskit](#native-gates-with-qiskit) or on the IBM Quantum&trade; Platform [Compute resources page](#native-gates-on-platform).
...
| Name |  Notes |
| :--- | :--- |
| [CZ](/api/qiskit/circuit-library-cz-gate) | two-qubit gate |
| [RZ](/api/qiskit/circuit-library-rz-gate) | single-qubit gate |
| [SX](/api/qiskit/circuit-library-sx-gate)  |single-qubit gate |
| [X](/api/qiskit/circuit-library-x-gate)  | single-qubit gate |
| [ID](/api/qiskit/circuit-library-i-gate) | single-qubit gate wait cycle |
...
| [if_else](/api/qiskit/circuit-if-else-op) | control flow for classical feedforward |
| [for_loop](/api/qiskit/circuit-for-loop-op) | control flow for classical feedforward |
| [switch_case](/api/qiskit/circuit-switch-case-op) | control flow for classical feedforward |
...
| Name |  Notes |
| :--- | :--- |
| [ECR](/api/qiskit/circuit-library-ecr-gate) | two-qubit gate |
| [RZ](/api/qiskit/circuit-library-rz-gate) | single-qubit gate |
| [SX](/api/qiskit/circuit-library-sx-gate)  |single-qubit gate |
| [X](/api/qiskit/circuit-library-x-gate)  | single-qubit gate |
| [ID](/api/qiskit/circuit-library-i-gate) | single-qubit gate wait cycle |
...
| Name |  Notes |
| :--- | :--- |
| [CX](/api/qiskit/circuit-library-cx-gate)  | two-qubit gate |
| [RZ](/api/qiskit/circuit-library-rz-gate) | single-qubit gate |
| [ID](/api/qiskit/circuit-library-i-gate) | single-qubit gate wait cycle |
| [reset](/api/qiskit/circuit#qiskit.circuit.Reset)  | single-qubit gate, non-unitary; not the same as the initialization done at the start of a circuit to prepare the all 0's state |
| [if_else](/api/qiskit/circuit-if-else-op) | control flow for classical feedforward |
