...
For example, if you have a PUB with one circuit, observables with shape (1, 6), and parameters with shape (4, 1), this would render 4 * 6 = 24 circuits (or fewer, if the observables are commutable). If you requested 2,000 shots, then the total number of executions is 24 * 2,000 = 48,000.
...
This ensures that the job can be manipulated within the memory limits of the low-level software stack. The actual limit depends on the options selected in the Sampler or Estimator query. However, the two-qubit gate limit per job lower bound is **10,000 two-qubit gates**. That is, if a job has fewer than 10,000 two-qubit gates, it typically passes validation. Some jobs might allow more, and the service will calculate the limit allowed for the user-selected options and communicate that in the validation error message.