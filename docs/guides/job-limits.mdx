...
When the observables commute, the number of executions can be further reduced. Generally, we calculate number of observables that commute by using Qiskit as follows:
...
After determining the number of commuting groups, the total execution is given by:
$Total executions = number_of_commuting_groups * number_of_parameter_set * number_of_shots$

If `Total executions` is more than 10 million, that workload cannot be handled by the quantum software stack and usually will fail due to timeout or `1500` errors.

Additionally, if you use error mitigation or error correction techniques such as twirling, the number of actual executions during runtime are greater than the `Total executions` calculated.
