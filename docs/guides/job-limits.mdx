...
- The `backend.configuration()` fields, `max_shots` and `max_experiments`, are no longer relevant and will be removed in the near future.
</Admonition>
...
The maximum number of two-qubit gates per circuit is 5 million. This ensures that the job can be manipulated within the memory limits of the low-level software stack.