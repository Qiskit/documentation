...
This requires additional steps to the standalone C usage; for more details, see the [Install the Qiskit C API guide](/docs/guides/install-c-api).
...
<Admonition type="note">
  These instructions have only been tested on UNIX-like systems. Windows instructions are in progress.
</Admonition>

## Requirements

Start by ensuring you have installed the [Qiskit C API](/docs/guides/install-c-api). Next, install the Qiskit Python interface, as follows:
```bash
...
it to Python. You can convert a C-side `QkObs*` to a Python-side `SparseObservable` object, using the provided converter
`qk_obs_to_python`:
```c
// file: extension.c
