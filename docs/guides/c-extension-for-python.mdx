...
<Admonition type="warning">
  Qiskit's C API is still experimental, and has not yet committed to a fully stable programming or 
  binary interface.  Extension modules built against Qiskit are only guaranteed to work against 
  the exact version of Qiskit used in the build, though we do not plan to deliberately break 
  either interface.
</Admonition>
