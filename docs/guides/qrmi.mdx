---
title: Quantum resource management interface (QMRI)
...
# Quantum resource management interface (QMRI)
...
The source code to build and deploy this interface can be found in this [GitHub repository](https://github.com/qiskit-community/qrmi).

There is also an optional `task_runner` command line tool to execute quantum payloads against quantum hardware which is included in the Python package. The full documentation can be found [here](https://github.com/qiskit-community/qrmi/blob/main/python/qrmi/tools/task_runner/README.md).

## Build the QRMI libraries

At its core, QRMI is a set of libraries to control state for a set of quantum resources. Written in Rust with a C and Python API exposed for ease of integration to any compute infrastructure. 
...
Then to build the Python package, first set up a python environment and install the required dependencies.
```shell-session
. ~/.cargo/env
cargo clean
...
And lastly, build the python wheels for distribution to your hosts.
```shell-session
source ~/py312_qrmi_venv/bin/activate
...
And the C API documentation can be created using doxygen
```shell-session
doxygen Doxyfile
```
