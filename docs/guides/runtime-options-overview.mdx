...
| experimental         |                          |                         |                           |                        |                    |
|                      | execution_path           |                         | `<new_flag>` *([see note](#experimental-option-note))*            | None                   |                    |
  </TabItem>
...
Setting `<new flag>` enables the next-generation execution path. This is currently experimental and is only available on `ibm_torino`.
</Admonition>
...
<Tabs>
  ...
  <TabItem value="PLACEHOLDER" label="Experimental option NEEDS NAME">
  Incompatible with:
  - Dynamic circuits
