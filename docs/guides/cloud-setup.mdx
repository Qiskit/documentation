...
If you have an administrator, they will set up instances and invite you to join an account. When this happens, you will be notified by email. If you have an emailed invitation to join an account, follow the steps in [Set up your IBM Cloud account - invited users](/docs/guides/cloud-setup-invited) instead of using this guide.
...
1. Log in to [IBM Quantum Platform](https://quantum.cloud.ibm.com) with an IBMid or Google account.  **The profile that you use to log in to IBM Cloud or IBM Quantum Platform in this step is not your IBM Cloud account.**
...
### 1. Create an instance
    ...
    1. Choose whether you want an access group to be created for this instance and click **Create instance**.   This access group allows other users (collaborators) to share the time allocated to this instance.  Collaborators can be added on the Users page.
       ...
       You can add or remove access groups at any time from IBM Cloud.  See [Setting up access groups](https://cloud.ibm.com/docs/account?topic=account-groups&interface=ui) for instructions.
        ...
        <Admonition type="note">
          If you are an administrator who needs to set up Qiskit Runtime on Cloud for your organization, refer to [Plan Qiskit Runtime for an organization](/docs/guides/considerations-set-up-runtime).
        </Admonition>
...
1. Activate the Python virtual environment, run Python in your virtual environment, and enter the following:
    ...
    ```
2. Save your credentials. Run the appropriate code block once per account you want to save.
...
- **`plans_preference`**: Optionally set the types of plans to prioritize. This is ignored if the instance is specified. Available options are `open` and `premium`. Instances of a certain plan type are excluded if the plan name is not specified. For example, if [`open`] is passed in, only open plan instances are available. This is ignored if `instance` is specified.
...
See [Initialize the service](/docs/guides/initialize-account) for instructions.
