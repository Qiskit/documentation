...
### 1. Create an instance
    ...
    1. Make sure that you have the correct account and region are selected from the account and region switchers in the top header.

        The region controls where your jobs are run and where the jobs' data is kept. You can access either region by using the same API key, but you can only see and access the instances that were created in the region that you're logged in to.
