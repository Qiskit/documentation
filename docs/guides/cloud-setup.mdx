...
This example saves credentials for open and premium access.  The open credentials are set as the default.

```python
from qiskit_ibm_runtime import QiskitRuntimeService

QiskitRuntimeService.save_account(channel="ibm_quantum_platform", token="<API_TOKEN>",
                                  instance="<CRN_for_premium_instance>", name="premium")
QiskitRuntimeService.save_account(channel="ibm_quantum_platform", token="<API_TOKEN>",
                                  instance="<CRN_for_open_instance>", name="open", set_as_default=True)
 ```
...
QiskitRuntimeService.save_account(
    ...
    # Set instance "filters" instead of specifiying the instance:
    region="eu-de",
...
See the [Initialize the service](/docs/guides/initialize-account) guide for instructions.
