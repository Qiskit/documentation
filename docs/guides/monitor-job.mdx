...
<Admonition type="note">
  service.jobs() returns only Qiskit Runtime jobs. To retrieve jobs run from IBM Provider (deprecated), use [qiskit-ibm-provider](../api/qiskit-ibm-provider/qiskit_ibm_provider.IBMBackend#ibmbackend) instead.  See [Migrate from qiskit_ibm_provider](/migration-guides/qiskit-runtime-from-ibm-provider) for instructions to migrate.
</Admonition>
