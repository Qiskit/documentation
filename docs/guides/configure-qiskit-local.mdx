...
- `QISKIT_SABRE_ALL_THREADS`: Is used to control the behavior of the layout and routing pass in Qiskit's preset pass manager. When set to `1` or `TRUE` this will use all available CPUs for running multiple random trials. This can improve the quality of the results, especially for systems with > 20 CPUs/cores, but comes with the tradeoff of results not being reproducible when run on different local hardware.
