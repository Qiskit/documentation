...
Batch jobs have an interactive timeout value (iTTL) between jobs and a maximum time to live (TTL) value. If you don't explicitly close a batch, it is deactivated after the iTTL expires and can be reactivated at any time until it reaches its  maximum TTL (usually 8h).

To determine the maximum timeout value for a batch, use the [`batch.details()`  method](run-jobs-batch#batch-details) and look for the `max_time` value.
