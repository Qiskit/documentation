...
![The bottom status bar is shown, with `mistral-small-3.2-24b-qiskit` listed.](/images/guides/qiskit-code-assistant/jupyterlab-selected-model.png)

The first time you use the `mistral-small-3.2-24b-qiskit` model a modal opens, listing some major restrictions that you should be aware of when using the model. Clicking `Accept` will accept the disclaimer and enable the model for code generation.
