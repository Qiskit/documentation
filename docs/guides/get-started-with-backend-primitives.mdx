...
`backend` object, as long as it implements the [`Backend`](/docs/api/qiskit/qiskit.providers.Backend) interface.  Some providers implement primitives natively.  See the [Qiskit Ecosystem page](https://www.ibm.com/quantum/ecosystem?tag=provider) for details.
