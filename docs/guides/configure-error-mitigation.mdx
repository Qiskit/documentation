...
The Estimator primitive supports several error mitigation techniques, including [TREX](/api/qiskit-ibm-runtime/options-resilience-options-v-2#measure_mitigation), [ZNE](/api/qiskit-ibm-runtime/options-resilience-options-v-2#zne), [PEC](/api/qiskit-ibm-runtime/options-resilience-options-v-2#pec), and [PEA](/api/qiskit-ibm-runtime/options-zne-options). See [Error mitigation and suppression techniques](error-mitigation-and-suppression-techniques) for an explanation of each. When using primitives, you can turn on or off individual methods. See the [Custom error settings](#advanced-error) section for details.
