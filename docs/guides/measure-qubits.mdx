...
Use the [`measure`](/api/qiskit/circuit-quantum-circuit#measure) method to measure a [`QuantumCircuit`](/api/qiskit/circuit-quantum-circuit#quantumcircuit-class).
...
To measure all qubits into the corresponding classical bits, use the [`measure_all`](/api/qiskit/circuit-quantum-circuit#measure_all) method. By default, this method adds new classical bits in a `ClassicalRegister` to store these measurements.
...
To measure all qubits that are not idle, use the [`measure_active`](/api/qiskit/circuit-quantum-circuit#measure_active) method. This method creates a new `ClassicalRegister` with a size equal to the number of non-idle qubits being measured.
...
- [`measure_all`](/api/qiskit/circuit-quantum-circuit#measure_all) method
- [`measure_active`](/api/qiskit/circuit-quantum-circuit#measure_active) method
