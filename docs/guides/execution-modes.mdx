...
Usage is a measurement of the amount of time the QPU is locked for your workload. It is important to understand how usage is calculated because it impacts how much you are charged for jobs (including failed jobs).
...
When a job is failed or canceled, you are charged for usage as follows:

* Job or batch mode: you are charged for all the time the QPU was locked for the execution, regardless of cancellations and errors. So if the error occurs before the lock, you are charged nothing. If the error/cancellation occurs after the system lock, you are charged based on the value that  _channels service_ returns as `consumed`. Thus, some jobs failed jobs have no cost and others do.

* Session mode: You are charged all the wall-clock time from the first job execution until the session goes inactive or is closed, regardless of numbers of jobs, cancellations, or errors.
