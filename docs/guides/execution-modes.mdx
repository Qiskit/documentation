...
<LegacyContent>
3. If the maximum TTL value is reached, the workload ends and any remaining queued jobs fail. In IBM Quantum&reg; Platform, any jobs already running will run to completion.  However, if you are using Qiskit Runtime on IBM Cloud&reg;, jobs won't run to completion if doing so would exceed the instance's cost limit.
</LegacyContent>
<CloudContent>
3. If the maximum TTL value is reached, the workload ends and any remaining queued jobs fail. Any job currently running won't run to completion if doing so would exceed the instance's cost limit.
</CloudContent>
