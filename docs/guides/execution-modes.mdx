...
3. If the maximum TTL value is reached, the workload ends and any remaining queued jobs fail. In IBM Quantum Platform, any jobs already running will run to completion.  However, if you are using Qiskit Runtime on IBM Cloud, jobs won't run to completion if doing so would exceed the instance's cost limit.
