...
The reported usage is the time a QPU is locked for your workload, including jobs that failed.
...
* Job or batch mode: the reported usage is the time the QPU was locked for executing your workload until the time it failed or was canceled. Therefore, if the workload had not locked the QPU by the time of failure, the reported usage is zero. Otherwise, the workload's reported usage is the value that Qiskit Runtime returns as `consumed`.
