...
* Job or batch mode: you are charged for all the time the QPU was locked for the execution, regardless of cancellations and errors. So if the error occurs before the lock, you are charged nothing. If the error/cancellation occurs after the system lock, you are charged based on the value that   Qiskit Runtime returns as `consumed`. Thus, some jobs failed jobs have no cost and others do.
