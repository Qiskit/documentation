...
<CardGroup>
    ...
    <Card
      title="AQC-Tensor"
      description="A Qiskit addon that uses tensor network methods to compile the initial portion of a circuit into a nearly equivalent approximation, with many fewer layers. It simulates the time evolution of a quantum system, and is also useful in cases where a circuit's tensor-network simulation achieves a great intermediate state - or a circuit prepares an approximation to the target state, but with fewer layers when compiled to the QPU."
    ...
    <Card
      ...
      description="To get started building quantum workloads to solve optimization problems, refer to this community repository, which contains guidelines, best practices, and reference implementations for running quantum optimization algorithms. It uses the Quantum Approximate Optimization Algorithm (QAOA) workflow as a point of reference."
...
<CardGroup>
    ...
    <Card
      ...
      description="Dynamic circuits are powerful tools for measuring qubits in the middle of quantum circuit execution - and based on the outcome of those mid-circuit measurements, you can then perform classical logic operations within the circuit. Some common uses cases include efficient quantum state preparation, long-range entanglement, and sampling of instantaneous quantum polynomial-like circuits. "
    ...
    <Card
      ...
      description="This Qiskit addon utilizes a technique to reduce circuit depth by trimming operations from its end at the cost of more operator measurements. However, this increased cost can be lessened by dropping operators with small coefficients, which don't contribute much to the final estimation."
...
<CardGroup>
    ...
    <Card
      ...
      description="NEAT analyzes and predicts the performance of your queries when you are gauging the expected performance of estimator-based workloads. This tool uses Qiskit Aer to simulate the estimation task classically, either exactly or with a noise model.  NEAT can also convert your Primitive Unified Blocs (PUBs) into Cliffordized circuits."
...
<CardGroup>
    ...
    <Card
      ...
      description="Execution modes are scheduling strategies for execution to use when building quantum workloads. Batch mode is a multi-job manager for efficiently running experiments comprising multi-job workloads, and session mode is a dedicated window for running a multi-job workload. (Open Plan instances cannot run in session mode.)"
    ...
    <Card
      ...
      description="Executor allows you to fine-tune error mitigation and other techniques without sacrificing performance, by providing the ingredients to capture design intents on the client-side, and shifting the costly generation of circuit variants to the server-side."
...
<CardGroup>
    ...
    <Card
      title="Samplomatic"
      description="Samplomatic is the client-side library for the directed execution model, providing the ability to create sampling randomizations of your quantum circuits in exactly the way that you specify. It utilizes the Qiskit SDK `Box` annotations to specify regions of a circuit that should have similar noise profiles, and can also group collections of gates to twirl, or use the same noise model from `NoiseLearner`."
    ...
    <Card
      ...
      description="This error mitigation technique returns an unbiased estimate of the expectation value, at the expense of a large sampling overhead. It reproduces the output of the ideal circuit by executing different noisy circuit instances drawn from a random ensemble defined by the linear combination."
    ...
    <Card
      title="Gate folding-based noise amplification"
    ...
    <Card
      ...
      description="A technique for suppressing decoherence errors, DD is useful in regions within a circuit in which one or more qubits sit idle while other instructions are being executed. Use Sampler and Estimator's built-in support to apply DD, try the `PadDynamicalDecoupling` transpiler pass, or build your own using the new stretch instruction."
    ...
    <Card
      ...
      description="This Qiskit addon implements a post-processing technique to measure ground state energies. It processes samples from a given circuit to project and diagonalize a target Hamiltonian in a subspace. Use when you want to obtain the eigenvalues and eigenvectors of quantum systems (such as chemical or lattice models)."
    ...
    <Card
      title="SQD for HPC"
      description="This HPC-ready implementation of the SQD addon is written in modern C++17 standards and is designed to create a single compiled binary for use with MPI.  This library builds upon existing interfaces in the C++ standard template library (STL)."
    ...
    <Card
      ...
      description="The noise models produced by `NoiseLearner` might not be accurate for some Heron backends due to non-Markovian noise on these devices. This can be alleviated with measurement-based post-selection. The technique adds a series of RX gates followed by a measurement to the circuit, and in post-processing throws away the bad bitstrings. Implement this with the Qiskit addon utilities package, or with `NoiseLearnerV3`. "
