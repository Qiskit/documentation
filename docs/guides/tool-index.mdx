...
<CardGroup>
    ...
    <Card
      ...
      description="Explore the different execution modes - scheduling strategies for execution - when building quantum workloads, to determine which one is most efficient for a specific workload. Batch mode is a multi-job manager for efficiently running experiments comprising multi-job workloads, and session mode is a dedicated window for running a multi-job workload. (Open Plan instances cannot run in session mode.)"
