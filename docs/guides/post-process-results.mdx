...
![All Qiskit pattern steps are shown (Map problem, Optimize for hardware, Execute on hardware, Post-process results). The Post-process results step is emphasized.  This step uses the Qiskit SDK.](/images/qiskit-patterns/post-process-results.svg)
