...
*   **Access share:** (This documentation uses the simplified term “share”.) A relative amount of access to IBM Quantum computing services assigned to a specific hub, group and project. The portion of access is determined by the specific allotment of minutes divided by the total number of allocation distributed. IBM&reg; assigns time to each hub based on their allotted capacity contracted under the Premium Plan subscription. Hub administrators then assign fractions of their allocation in minutes to each of their groups. Finally, group administrators assign fractions of their allocation in minutes to each of their projects.
...
*   **Time used:** For every group and project, during the scheduling window, we account for all usage on all the QPUs of the IBM Quantum Premium Plan. These include all successful jobs, as well as jobs returning known select errors. It does not account for canceled jobs, even when partially executed.
...
The scheduler combines a group’s allocation with the alotted time of its hub, to determine the total fraction of computational power allocated to that group.
