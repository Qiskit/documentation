...
For example, assume only two groups maintain jobs in one QPU queue. With all else being equal, a group with twice the number of shares will have twice the number jobs execute.
