...
To use the AI transpiler passes, install the `qiskit-ibm-transpiler` package (see instructions [here](./qiskit-transpiler-service#install-transpiler-service)).

## Running the AI transpiler passes on local or cloud

If you want to use the AI-powered transpiler passes in your local environment for free you will need install the `qiskit-ibm-transpiler` with some extra dendencies as follows:

```sh
pip install qiskit-ibm-transpiler["ai-local-mode"]
```

Without these extra dependencies, the AI-powered transpiler passes will run on cloud through the Qiskit Transpiler Service (available only for IBM Quantum&trade; Premium Plan users). After installing the extra dependencies, the default mode to run the AI-powered transpiler passes will be using your local machine.
...
- `local_mode`: This flag determines where your AIRouting pass will run, if `False`, AIRouting will run on cloud through the Qiskit Transpiler Service. If `True` the package will try to run on your local environment with a fallback to the cloud mode if the required the extra dependencies needed to run the AI-powered transpiler passes on local are not found.
...
## Hybrid Qiskit-AI circuit transpilation

The `qiskit-ibm-transpiler` offers the option to configure an hybrid pass manager combining best of Qiskit and the AI-powered transpiler passes, so you can run a whole transpilation on your own. This featres behaves similarly to the Qiskit `generate_pass_manager` method. A typical way to use this `generate_ai_pass_manager` is as follows:
...
Here, the `coupling_map` determines which coupling map needs to be used for the transpilation. The `ai_optimization_level` refers to the level of optimization (1-3) to get using the AI components of the PassManager, while the `optimization_level` refers to the optimization to get from the Qiskit heuristic algorithms. The `ai_layout_mode` refers to how the AI routing part of the PassManager will handle the layout. Refer to the #AI-routing-pass section to review the different configuration options for this parameter.
