...
- `local_mode`: This flag determines where the AIRouting pass runs, if `False`, AIRouting runs on cloud through the Qiskit Transpiler Service. If `True`, the package tries to run on your local environment with a fallback to cloud mode if the required the extra dependencies are not found.
...
The `qiskit-ibm-transpiler` offers the option to configure a hybrid pass manager that combines the best of Qiskit and the AI-powered transpiler passes, so you can run a whole transpilation on your own. This feature behaves similarly to the Qiskit `generate_pass_manager` method. A typical way to use `generate_ai_pass_manager` is as follows:
