...
Without these extra dependencies, the AI-powered transpiler passes run on cloud through the Qiskit Transpiler Service (available only for IBM Quantum&trade; Premium Plan users). After installing the extra dependencies, the default mode to run the AI-powered transpiler passes is to use your local machine.
