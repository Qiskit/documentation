...
Using an open-source suite of over 1,000 benchmarking tests developed by leading universities, national labs, and researchers at IBM, Qiskit led the results in speed and quality for most test tasks. See the [IBM Quantum blog article](https://www.ibm.com/quantum/blog/qiskit-performance) for more details.

The benchmarking suite used for these tests, named Benchpress, is now available as [an open-source package](https://github.com/qiskit/benchpress). You can now use the Benchpress package to perform your own analysis of quantum SDK performance.
