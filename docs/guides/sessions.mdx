...
To find the maximum timeout or ITTL value for a session, follow the instructions in [Determine session details](/guides/run-jobs-session#session-details). To set the maximum timeout value for a session, follow the instructions in [Session length.](/guides/run-jobs-session#specify-length)

The default settings are listed in the [Session length](/guides/run-jobs-session#session-length) section.
