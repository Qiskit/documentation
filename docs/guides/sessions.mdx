...
To find the maximum timeout or ITTL value for a session, follow the instructions in [Determine session details](./run-jobs-session#session-details). To set the maximum timeout value for a session, follow the instructions in [Session length.](./run-jobs-session#specify-length)

The default settings are listed in the [Introduction to execution modes](./execution-modes#ittl-by-mode) guide.
