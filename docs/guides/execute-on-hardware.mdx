...
![All Qiskit pattern steps are shown (Map problem, Optimize for hardware, Execute on hardware, Post-process results), with Execute on hardware emphasized. Executing on hardware uses only the Qiskit Runtime service.](/images/qiskit-patterns/execute.svg)
