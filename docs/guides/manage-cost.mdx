...
<Admonition type="note">
  The information in this topic only applies to those who are using the Standard plan for Qiskit Runtime on IBM Cloud&reg;.  There are no costs associated with the IBM Quantum&trade; Platform Open Plan.
</Admonition>
...
You can estimate how long a job will run, and therefore its cost, by estimating the job run time.  For details, see the [Workload usage](estimate-job-run-time) topic.
