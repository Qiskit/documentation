---
title: Manage cost
description: How to manage costs of running jobs on QPUs when using the Premium, Flex, or Pay-As-You-Go Plan on IBM Quantum Platform.

---

# Manage cost

<Admonition type="note" title="The Open Plan is free">
This guide only pertains to plans with associated costs: Premium, Flex, and Pay-As-You-Go Plans. There is no cost associated with the Open Plan.
</Admonition>
...
* **IBM Quantum Platform**: Open the [Instances & allocation tab.](https://quantum.cloud.ibm.com/instances) Find the instance you want to limit, click the overflow menu at the end of the row, and then click **Edit details**. Set the total cost limit or the usage limit, then click **Save changes**.
   <Admonition type="note">
   You can also set the cost or usage limit when creating a new instance.
   </Admonition>
...
* **IBM Quantum Platform**: The Max cost limit is shown on the Pay-As-You-Go tab of the [Instances & allocation](https://quantum.cloud.ibm.com/instances) tab.

* **API**:
   * IBM Quantum Platform users: [`GET /v1/instances/configuration`](/docs/api/qiskit-runtime-rest/tags/instances).
...
* **API**: Set the instance limit to `null`.
   * IBM Quantum Platform users: [`PUT /v1/instances/configuration "instance_limit":null`](/docs/api/qiskit-runtime-rest/tags/instances).
...
You or your account owner will receive a monthly invoice that provides details about your resource charges. You can check how much has been spent at any time on the [IBM Cloud Billing and usage page](https://cloud.ibm.com/billing).
