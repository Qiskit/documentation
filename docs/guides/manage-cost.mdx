...
* **IBM Cloud CLI**: For instructions to use the CLI, refer to [Getting started with the IBM Cloud CLI.](https://cloud.ibm.com/docs/cli?topic=cli-getting-started)
   ...
   1. Update an instance's cost limit:
      ...
      \<`seconds`\> is the maximum number of seconds that can be consumed by the instance. For example, to establish a limit of 1000 seconds $(1000 * \$1.6 = \$1,600)$, enter this command:
      ```cli
      `ibmcloud resource service-instance-update <instance_CRN> -p '{"instance_limit_seconds":"1000"}'`
      ```
   1. View the instance details to verify that it was updated:
      ```cli
         `ibmcloud resource service-instance <instance_CRN> -o json`
      ```
      This command returns a JSON object.  For example:
      ```json
          [
            {
            ...
