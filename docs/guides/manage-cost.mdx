...
* **API**:
   * New IBM Quantum Platform users: [`GET /v1/instances/configuration`](/api/qiskit-runtime-rest/tags/instances#tags__instances__operations__get_instance#tags__instances__operations__get_instance).
...
* **IBM Cloud CLI**: For instructions to use the CLI, refer to [Getting started with the IBM Cloud CLI.](https://cloud.ibm.com/docs/cli?topic=cli-getting-started)
   ...
   ```cli
   ibmcloud resource service-instance $INSTANCE_CRN_OR_NAME -o JSON
   ```
