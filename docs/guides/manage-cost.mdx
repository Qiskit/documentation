...
* **IBM Cloud CLI**: For instructions to use the CLI, refer to [Getting started with the IBM Cloud CLI.](https://cloud.ibm.com/docs/cli?topic=cli-getting-started)
   1. Log in to the CLI: `ibmcloud login --sso`.
      ...
      2. After logging in to the browser that opens, a code is shown.  Click the copy button.
   ...
   1. Update an instance's cost limit:
      ...
      \<`SECONDS`\> is the maximum number of seconds that can be consumed by the instance. For example, to establish a limit of 1000 seconds (1000 * $1.6 = $1,600)
