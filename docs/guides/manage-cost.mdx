...
Learn how to limit your costs when using the Standard Plan on IBM Quantum&reg; Platform.
...
## Set, change, and view a cost limit or instance limit
...
* **IBM Cloud CLI**: For instructions to use the CLI, refer to [Getting started with the IBM Cloud CLI.](https://cloud.ibm.com/docs/cli?topic=cli-getting-started)
   1. Log in to the CLI: `ibmcloud login --sso`.
      * When you press return, you are asked whether you want to open a browser.  Type `Y`.
      * Enter your email and password in the browser that opens, then a code is shown.  Click the copy button.
      * Go back to the CLI window and paste in the code.
   1. Establish the API: `ibmcloud api cloud.ibm.com`
   1. Update the max cost limit of an instance:
      ```cli
         ibmcloud resource service-instance-update <INSTANCE_CRN> -p
         '{"instance_limit_seconds":"<SECONDS>"}'
      ```
      where:
      \<`INSTANCE_CRN`\>: The CRN of the instance you want to update. You can get it from the [IBM Quantum Platform dashboard.]((https://quantum.cloud.ibm.com))
      \<`SECONDS`\>: The maximum number of seconds that can be consumed by the instance. For example, to establish a limit of 1000 seconds (1000 * $1.6 = $1,600)
   1. View the limit to verify that it was updated: `ibmcloud resource service-instance <INSTANCE_CRN> -o json`
      This command returns a JSON.  For example:
      ```json
          [
            {
            ...
...
{/* * **User interface**: Navigate to the <LegacyContent>[IBM Cloud Instances page](https://cloud.ibm.com/quantum/instances)</LegacyContent><CloudContent>[Instances page](https://quantum.cloud.ibm.com/instances)</CloudContent>, then click the instance and set the cost limit. */}
