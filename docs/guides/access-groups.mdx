...
The [Compare pre-defined service role actions](#compare-actions) section provides a comparison of the pre-defined Manager, Editor, and Viewer roles.
</Admonition>
...
The following table displays which permissions are granted to account owners and access groups.

| Permissions | Account owner | Qiskit Runtime administrator (access group) | Instance collaborator (access group) |
| --- | --- | --- | --- |
...
| Configure Account Single Sign-on (SS0) using IBM Cloud App ID | | | | 

<span id="compare-actions"></span>
## Compare pre-defined service role actions

The following table displays which actions can be taken by the pre-defined service role actions: Manager, Editor, and Viewer.

| Action | Manager | Editor | Viewer | 
| --- | --- | --- | --- |
| Create a Session/Batch | | | | 
| Submit a Job | | | | 
| Read a result | | | | 
| Cancel a Job | | | | 
| Delete a Job | | | | 
| Read QPU calibrations | | | | 
| See account analytics | | | | 
| See instance usage and remaining time | | | | 
| Update instance allocation and limits | | | | 
