...
<Admonition type="caution" title="The backends field may be outdated">
  ...
  Instead, the current workaround is to use the [Qiskit Runtime REST API](/docs/api/qiskit-runtime-rest) with the [`GET /v1/backends`](/docs/api/qiskit-runtime-rest/backends#tags__backends__operations__list_backends) endpoint. (Make sure that you set the `Service-CRN` header to your instance's CRN.)
</Admonition>
