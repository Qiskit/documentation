...
* A C compiler: for example, Microsoft Visual C++ (MSVC) and the native command prompt offering the ``cl`` command
...
Examples of good workspace paths: `C:\WORKSPACE_PATH`, `D:\WORKSPACE_PATH`, `C:\Users\john\WORKSPACE_PATH`
...
1. Open VS Code
2. Click File -> Open Folder, then select your workspace (for example, `C:\WORKSPACE_PATH`).
...
```powershell
$<workspace_abbreviation> = "C:\<your_workspace>"   # change to your workspace path
mkdir $<workspace_abbreviation> -Force
cd $<workspace_abbreviation>
```

Example:

```powershell
$WORKSPACE = "C:\WORKSPACE_PATH"   # change to your workspace path
...
If the download didn't work, or you prefer manual download, comment the code lines that install Python 3.12, then download Python 3.12 from the [Python](https://www.python.org/downloads/).  Run the executable to install Python.  Select "Add Python to PATH" during the install, then rerun the above snippet to make sure it's being found.
...
cd $<workspace-abbreviation> # example: cd: $WORKSPACE
& $PYTHON_EXE -m venv .venv --prompt <workspace> # example: --prompt WORKSPACE_PATH
...
Your prompt should now show `(_workspace_)`. 
...
Run the following code each time you restart the kernel. It locates and loads the MSVC developer environment automatically so you don't need the x64 built-in command prompt.
...
(root / "pyWORKSPACE_PATH.toml").write_text("""
[build-system]
...
| Path length errors | Use a short root path on C:\ (for example, `C:\WORKSPACE_PATH`, or the path you set in the setup step), or enable Long Paths: Settings -> System -> For developers -> Long Paths |
| Verify cell fails with "conda environment still active" | Run `conda deactivate` until `CONDA_DEFAULT_ENV` and `CONDA_PREFIX` are both gone from the environment. Check with `$env:CONDA_PREFIX`. If they persist, open a fresh PowerShell (not an Anaconda prompt) and retry from Step 2. |
| Verify cell fails with "venv was created from a conda-managed Python" | The virtual environment inherited conda's C runtime and cannot be fixed in place. Delete it and rebuild from a python.org Python. Run `Remove-Item -Recurse -Force .venv`, then rerun the VS Code install step to set `$PYTHON_EXE`, then recreate the virtual environment (step 2). |
...
| Build fails with garbled paths or `C1083` | Your username or workspace path contains non-ASCII characters. Move the workspace to a short ASCII-only path (for example, `C:\WORKSPACE_PATH`) |
| Build or import randomly fails, works on retry | The workspace folder is synced by OneDrive. Move it to a local path like `C:\WORKSPACE_PATH` |
