...
Examples of good workspace paths: `C:\project`, `D:\project`, `C:\Users\john\project`
...
1. Open VS Code
2. Click File -> Open Folder, then select your workspace (for example, `C:\project`).
...
```powershell
$PROJ = "C:\project"   # change to your workspace path
mkdir $PROJ -Force
cd $PROJ
```
...
cd $<workspace-abbreviation> # example: cd: $PROJ
& $PYTHON_EXE -m venv .venv --prompt <workspace> # example: --prompt project
...
| Path length errors | Use a short root path on C:\ (for example, `C:\project`, or the path you set in the setup step), or enable Long Paths: Settings -> System -> For developers -> Long Paths |
...
| Build fails with garbled paths or `C1083` | Your username or workspace path contains non-ASCII characters. Move the workspace to a short ASCII-only path (for example, `C:\project`) |
| Build or import randomly fails, works on retry | The workspace folder is synced by OneDrive. Move it to a local path like `C:\project` |
