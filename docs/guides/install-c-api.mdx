...
This section provides build instructions for UNIX systems.
...
* Make: this is optional but is recommended to use automated install processes
...
<Admonition type="caution">
  Windows support is currently experimental. These instructions are a work in progress.
</Admonition>
...
```
This will write the MSVC-compatible header in the same directory as the library.

Now you can use `cl` to compile the C program. For simplicity, here it is assumed the file is
located in `target/release`.
```bash
cl example.c qiskit_cext.dll.lib -I.
```
If your source file has another location,  be sure to update the
...
If you get an error that reports libraries are missing, you can try updating the local `PATH`
