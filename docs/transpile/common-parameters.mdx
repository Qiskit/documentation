...
As an example, we generate a random 2-qubit `UnitaryGate` which will be synthesized in the initial stage. Setting the `approximation_degree` less than 1.0 may generate an approximate circuit. We must specify the `basis_gates` as well to let the synthesis method know which gates it can use for the approximate synthesis.
