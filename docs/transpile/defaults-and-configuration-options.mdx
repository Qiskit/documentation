...
These parameters describe the system that the input circuit will be run on.  They are used to ensure that the returned circuit can be run on a backend with the specified properties. 
