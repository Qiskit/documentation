...
Constuct a 12-qubit circuit with circular entanglement:
...
<Admonition>
Although the routing stage it intertwined with the _layout_ stage, which selects the actual qubits, we will comsider them as separate stages for simplicity. The combination of routing and layout is called _qubit mapping_. (TODO: insert link) 
</Admonition>
...
Several SWAP gates were inserted (each consisting of three CX gates), which will cause a lot of errors on current devices. To see which qubits are selected on the actual qubit topology, use `plot_circuit_layout` from Qiskit Visualizations:
...
```
![Circuit Layout for optimiation level 0](/images/transpile/representing_quantum_computers/circ_layout_lv0.png)
...
** Question: why aren't there any SWAPs instarted?  It looks like it still uses the coupling_map that's specified in qc_t_cm_lv0. **
