...
The simplest use of the transpiler is to provide all the backend infomation by providing the `backend` or `target`. To better understand how the transpiler works, construct a circuit and transpile it with different information:
...
This show that our virtual qubits 0-11 were trivially mapped to the line of physical qubits 0-11. Let's return to the default (`optimization_level=1`), which uses `VF2Layout` if any routing is required.
...
If we wish to consider the error rates of the qubits in addition to the `coupling_map` and `basis_gates`, we will need to construct a `target`.  While the `target` object contains everything we need to target a backend, here we show how to build one that just contains limited amount of information.

We retrieved the `target` from `backend.target` earlier, which contains lots of backend information, including error rates. For example, the instruction properties of the echoed cross resonance gate between qubit 0 and 1 (not that `ecr` is directional) is shown is retrieved via
...
The above result shows that the gate is 533μs with an error of 0.7%. To reveal error information to the transpiler, we will build our own target model with the `basis_gates` and `coupling_map` above and populate it with error values from the actual backend `FakeSherbrooke`. 
...
Note that by including the error information, the `VF2PostLayout` pass attempts to find the optimal qubits to use, and we have returned to the circuit found at the top with the same physical qubits.
