...
```
![The transpiled 12-qubit anstatz using Target information](/images/transpile/representing_quantum_computers/qc_t_target.png)
...
This show that our virtual qubits 0-11 were trivially mapped to the line of physical qubits 0-11.
...
```
![Ansatz transpiled to coupling map with optimization level 2](/images/transpile/representing_quantum_computers/qc_t_cm_lv2.png)

Now we do not have any SWAP gates inserted, and the physical qubits selected are the same when using the Target class *(TODO: actually they should probably be different).*
...
Now the layout is in a ring.  Because this layout respects the circuit's connectivity, there are no SWAP gatess. This is a much better circuit for execution.

## Basis gates

Every quantum backend supports a limited instruction set, called its _basis gates_.  Every gate in the circuit must be translated to the elements of this set. This set should consist of single- and two-qubit gates that provide a universal gates set, meaning that any quantum operation can be decomposed into those gates.  This is done by the _basis translator_ (*link*), and the basis gates can be specified as a keyword argument to the transpiler to provide this information.
...
**question: the statement "the default single-qubit gates..." makes it sound like you can choose a different set of gates somehow.  That doesn't seem possible, so what does that statement mean?**
Technically you can choose whatever gates you like as long as you provide the instruction or add pulse gates (see custom transpiler pass doc). The default basis gates are those that calibrations have been done for on the backend, so no further instrcution/pulse gate needs to be provided. For example, on some backends `cx` is the default two-qubit gates and `ecr` on others.
...
```
![Ansatz transpiled to coupling map and basis gates with optimization level 2](/images/transpile/representing_quantum_computers/qc_t_cm_bg_lv2.png)

Note that the `CXGate`s have been decomposed to `ecr` gates and single-qubit basis gates.

## Build your own target

While the `target` object contains everything we need to target a backend, this section shows how to build your own target. We will add the error rates from the instruction properties. The target already contains this information, for example, the instruction properties of the echoed cross resonance gate between qubit 0 and 1 is shown in the retrieved visualization.  However, the information is not shared with the transpiler by default. 

**something seems off in the sentence about the echoed cross resonance gate.  I tried to fix it, but there is no visualization, so I'm not sure what it's supposed to say.**
...
The above result shows that the gate is 533μs with an error of 0.7%. To reveal this information to the transpiler, we will build our own target model and populate it with value from the actual backend `FakeSherbrooke`.
...
```
![Ansatz transpiled to our target model with optimization level 2](/images/transpile/representing_quantum_computers/qc_t_cm_bg_et_lv2.png)

### Backend properties

Older backends do not have the `target` attribute.  In such cases, we can achieve the same effect by passing `backend.properties()` to the transpiler directly:
