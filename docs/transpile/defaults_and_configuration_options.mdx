...
Since CX's are the noisiest gate, we can quantify the transpilation's "hardware efficiency" by counting the CX's in the resulting circuit. We will compare the default transpilation levels given the same circuit. 
