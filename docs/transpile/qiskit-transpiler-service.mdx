...
*Note*: This package requires Qiskit SDK 1.X by default.
...
To get more info about the `qiskit-transpiler-service` available methods, check out the [qiskit-transpiler-service API reference](/api/qiskit-transpiler-service). To know more about the service API, check out the [Qiskit Transpiler Service REST API documentation](/api/qiskit-transpiler-service-rest)
...
3. Produce a similar circuit and transpile it while letting the service to decide if using the AI-powered transpiling passes.
...
If you use any AI feature from the Qiskit transpiler service in your research, here is the recommended citation
